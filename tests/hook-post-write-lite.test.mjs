...
check(
  "HPWL1. capturePreimage stores typeEscapes without raw source text leakage",
  () => {
    const fx = fixture();
    try {
      ...
      const raw = readFileSync(
        preimagePath(fx.auditRoot, fx.sid, "tool_a"),
        "utf8",
      );
...
check(
  "HPWL2. processPostWriteLite appends silent-new event for new escape",
  () => {
    const fx = fixture();
    try {
      ...
      const result = processPostWriteLite(
        payload(fx, [editCall("tool_a", "src/a.ts")]),
        {
...
check(
  "HPWL3. duplicate occurrence key merges into existing event count",
  () => {
    const fx = fixture();
    try {
      writeSource(
        fx.root,
        "src/a.ts",
        "export function f(raw) { return raw as any; }\n",
      );
      captureForTool(fx, "tool_a", "src/a.ts");
      const beforeKey = readPreimage(fx.auditRoot, fx.sid, "tool_a").fingerprint
        .typeEscapes[0].occurrenceKey;
      ...
      assert.equal(
        entries.find((entry) => entry.dedupe_key === beforeKey)
          .occurrence_count,
        2,
      );
    } finally {
...
check(
  "HPWL4. same-file group uses first preimage and cleans all preimages",
  () => {
    const fx = fixture();
    try {
      writeSource(fx.root, "src/a.ts", "export const value = raw;\n");
...
check("HPWL5. missing first preimage over-warns from empty baseline", () => {
  ...
  try {
    writeSource(fx.root, "src/a.ts", "export const value = raw as any;\n");
    const result = processPostWriteLite(
      payload(fx, [editCall("tool_missing", "src/a.ts")]),
      {
        maxChars: 0,
      },
    );
