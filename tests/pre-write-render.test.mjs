...
function assertAllClaimsCited(md, fixtureName) {
  ...
  assert(
    `${fixtureName}: every claim-bearing line is cited`,
...

{
  ...
  assert(
    'A1. fixture A contains "pre-write advisory" title',
    md.includes("pre-write advisory"),
  );
  assert(
    'A2. fixture A contains "EXISTS" row for formatDate',
    /EXISTS.*formatDate|formatDate.*EXISTS/.test(md),
  );
  assert(
    "A3. fixture A contains ownerFile path",
    md.includes("src/utils/date.ts"),
  );
  assert(
    "A4. fixture A cites fanInByIdentity grounded",
    md.includes(
      `symbols.json.fanInByIdentity['src/utils/date.ts::formatDate']`,
    ),
  );
  assert(
    "A4b. fixture A renders value/type/broad fan-in space",
    md.includes("fan-in 8 (value 7, type 1, broad 0)"),
    md,
  );
  assert(
    "A4c. fixture A cites fanInByIdentitySpace grounded",
    md.includes(
      `symbols.json.fanInByIdentitySpace['src/utils/date.ts::formatDate']`,
    ),
  );
...

{
  ...
  const md = renderMarkdown(advisory);
  assert(
    'B1. fixture B renders under "any-contaminated" section',
    md.includes("Already exists — but any-contaminated"),
  );
  assert(
    "B2. fixture B surfaces raw measurements (anyFieldRatio 0.85)",
    md.includes("0.85"),
  );
  assert(
    "B3. fixture B separates recommendation caution from measurement confidence",
  ...
  assert(
    "B4. severely-any-contaminated label present in rendered output",
...

{
  ...
  const md = renderMarkdown(advisory);
  assert(
    "C1. fixture C contains NOT_OBSERVED for formatTimestamp",
    md.includes("NOT_OBSERVED"),
  );
  assert(
    'C2. fixture C has "Search hints (not reuse candidates)" sub-section',
    md.includes("Search hints (not reuse candidates)"),
  );
  assert("C3. fixture C lists near-name formatDate", md.includes("formatDate"));
  ...
  const alreadyExistsSection = sections.find((s) =>
    s.startsWith("Already exists (reuse candidates)"),
  );
  assert(
    'C4. "Already exists (reuse candidates)" section does NOT contain near-name rows',
  ...
  assert(
    'C5. near-name citation includes "search hint only"',
...

{
  ...
  const md = renderMarkdown(advisory);
  assert(
    "D1. fixture D cites canonical declaration",
    md.includes(`canonical/type-ownership.md:L7`),
  );
  assert(
    "D2. fixture D cites AST absent via [확인 불가]",
    md.includes("확인 불가"),
  );
  ...
  assert(
    'D3. Markdown does NOT contain literal "CANONICAL DRIFT:"',
...

{
        ...
        identities: [
          {
            ...
            resolverConfidence: "high",
            citations: [
              `[grounded, symbols.json.fanInByIdentity['apps/admin/types.ts::User'] = 5]`,
            ],
          },
          {
            ...
            resolverConfidence: "high",
            citations: [
              `[grounded, symbols.json.fanInByIdentity['apps/blog/types.ts::User'] = 2]`,
            ],
          },
        ],
        canonicalClaim: null,
  ...
  const md = renderMarkdown(advisory);
  assert(
    "E1. EXISTS_MULTIPLE header or marker present",
    md.includes("EXISTS_MULTIPLE"),
  );
  assert(
    "E2. BOTH owner files rendered",
    md.includes("apps/admin/types.ts") && md.includes("apps/blog/types.ts"),
  );
  assert(
    "E3. per-identity fan-ins preserved (= 5 and = 2)",
...

{
  ...
  const md = renderMarkdown(advisory);
  assert(
    'F1. "Planned type escapes" section present',
    md.includes("Planned type escapes (from Step 2 intent)"),
  );
  assert(
    'F2. escapeKind "as-unknown-as-T" rendered',
    md.includes("as-unknown-as-T"),
  );
  assert(
    "F3. locationHint rendered",
    md.includes("src/vendor/wrapper.ts::adaptResponse"),
  );
  assert(
    "F4. codeShape rendered",
    md.includes("response as unknown as ThirdPartyShape"),
  );
  assert("F5. reason rendered", md.includes("upstream SDK lacks type exports"));
  assert(
    "F6. alternativeConsidered rendered (at least the prefix)",
    md.includes("unknown + decoder"),
  );
  assert(
    "F7. grounded citation on the planned escape (intent extracted at pre-write Step 2)",
  ...
  assert(
    "F8. Already exists section omitted when names list empty",
...

{
  ...
  const md = renderMarkdown(advisory);
  assert(
    "G1. empty planned escapes renders the default zero-planned text",
    md.includes("0 escapes planned"),
  );
  // Canonical empty-list text references fact-model.md §3.9 enumeration.
  assert(
    "G2. empty-list text references all 11 escapeKinds or fact-model §3.9",
...

{
  ...
  const md = renderMarkdown(advisory);
  assert(
    "G1. New code candidates section present",
    md.includes("### New code candidates"),
  );
  assert(
    "G2. NEW_FILE row mentions the intent file",
    md.includes("src/utils/time.ts"),
  );
  assert(
    "G3. NEW_FILE grounded topology citation rendered",
    md.includes("topology.meta.complete = true"),
  );
  assert(
    'G4. boundary sub-line cites "not evaluated"',
    /boundary.*not.evaluated|not.evaluated.*boundary/i.test(md),
  );
  assert(
    'G5. "CANONICAL DRIFT:" literal never appears',
    !md.includes("CANONICAL DRIFT:"),
  );
}
...

{
  ...
  const md = renderMarkdown(advisory);
  assert(
    "H1. FILE_STATUS_UNKNOWN row rendered under New code candidates",
    md.includes("### New code candidates") &&
      md.includes("FILE_STATUS_UNKNOWN"),
  );
  assert("H2. [확인 불가] citation present", md.includes("확인 불가"));
  assert(
    'H3. literal "NEW_FILE" claim NEVER appears in this fixture',
...

{
  ...
  const md = renderMarkdown(advisory);
  assert(
    "I1. DEPENDENCY_AVAILABLE rendered under Already exists",
    md.includes("### Already exists (reuse candidates)") &&
      md.includes("DEPENDENCY_AVAILABLE"),
  );
  // Pinning: dependency-available NEVER appears under New code candidates.
  const sections = md.split(/^### /m);
  const newCodeSection = sections.find((s) =>
    s.startsWith("New code candidates"),
  );
  assert(
    'I2. "DEPENDENCY_AVAILABLE" does NOT appear under New code candidates',
    !newCodeSection || !newCodeSection.includes("DEPENDENCY_AVAILABLE"),
  );
  assert(
    "I3. package.json citation rendered",
...

{
  ...
  const md = renderMarkdown(advisory);
  assert(
    "I2a. unavailable import graph renders as unavailable, not zero",
    md.includes("import graph unavailable") &&
      !md.includes("0 observed consumer"),
    md,
  );
  assert(
    "I2b. unavailable dependency lookup keeps [확인 불가] citation",
...

{
  ...
  const md = renderMarkdown(advisory);
  assert("J1. Watch-for section rendered", md.includes("### Watch-for"));
  assert(
    "J2. shape-hash + P4 substrings both present",
    md.includes("shape-hash") && md.includes("P4"),
  );
  assert("J3. [확인 불가] citation on the shape row", md.includes("확인 불가"));
}
...

{
  ...
  assert(
    "K1. all three escape items render",
  ...
  assert(
    "K2. renderJson preserves original plannedTypeEscapes order",
...

{
  ...
  const md = renderMarkdown(advisory);
  assert(
    "L1. file appears under Already exists (reuse candidates)",
    md.includes("### Already exists (reuse candidates)") &&
      md.includes("src/old/orphan.ts"),
  );
  assert("L2. Watch-for hub signal also present", md.includes("### Watch-for"));
  assert(
    "L3. hub citation includes count AND threshold",
...

{
  ...
  const md = renderMarkdown(advisory);
  assert(
    "L2a. NEW_FILE remains under New code candidates",
    md.includes("### New code candidates") &&
      md.includes("NEW_FILE") &&
      md.includes("lib/cardNewsService.js"),
  );
  assert(
    "L2b. domain cluster also appears under Watch-for",
    md.includes("### Watch-for") &&
      md.includes("DOMAIN_CLUSTER_DETECTED") &&
      md.includes("lib/cardNews*"),
  );
  assert(
    "L2c. existing sibling examples render",
    md.includes("lib/cardNewsGenerator.js") &&
      md.includes("lib/cardNewsPlanner.js"),
  );
}
...

{
  const advisory = {
    ...
    boundaryChecks: [],
    drift: [],
    capabilities: { identityFanIn: true, anyContamination: false },
    failures: [],
  };
  const md = renderMarkdown(advisory);
  const count = (
    md.match(/producer did not emit anyContamination capability/g) ?? []
  ).length;
  assert(
    "L3a. capability-absent note renders once, not once per identity",
    count === 1,
    `count=${count}\n${md}`,
  );
  assert(
    "L3b. fan-in citations still render for both identities",
...

{
  ...
  assert(
    'M1. "### Canonical drift" section rendered',
    md.includes("### Canonical drift"),
  );
  assert(
    'M2. "CANONICAL DRIFT:" literal appears in the drift section',
    md.includes("CANONICAL DRIFT:"),
  );
  assert(
    "M3. Both owners cited (canonical src/models/User.ts + AST apps/legacy/user.ts)",
    md.includes("src/models/User.ts") && md.includes("apps/legacy/user.ts"),
  );
  assert(
    "M4. Canonical line number cited",
  ...
  assert(
    'M5. "CANONICAL DRIFT:" is contained within the Canonical drift section',
    driftSection && driftSection.includes("CANONICAL DRIFT:"),
  );

  const nonDriftSections = sections.filter(
    (s, i) => i > 0 && !s.startsWith("Canonical drift"),
  );
  const literalInOtherSections = nonDriftSections.some((s) =>
    s.includes("CANONICAL DRIFT:"),
  );
  assert(
    'M6. "CANONICAL DRIFT:" appears ONLY in the drift section (not in Already exists etc)',
...

{
  ...
  const md = renderMarkdown(advisory);
  assert(
    "Ma1. ast-absent drift renders CANONICAL DRIFT:",
    md.includes("### Canonical drift") && md.includes("CANONICAL DRIFT:"),
  );
  assert(
    'Ma2. ast-absent drift cites "not observed" / "ast" language',
...

{
  ...
  const md = renderMarkdown(advisory);
  assert(
    'Me1. empty drift → "### Canonical drift" section omitted',
    !md.includes("### Canonical drift"),
  );
  assert(
    'Me2. "CANONICAL DRIFT:" literal absent when drift empty',
    !md.includes("CANONICAL DRIFT:"),
  );
}
...

{
  ...
  const json = renderJson(advisory);
  assert(
    "J1. renderJson returns an object (not string)",
    typeof json === "object" && json !== null,
  );
  assert(
    "J2. renderJson preserves invocationId",
    json.invocationId === "json-test",
  );
  assert(
    "J3. renderJson preserves intent",
    Array.isArray(json.intent?.names) && json.intent.names.includes("x"),
  );
  assert(
    "J4. renderJson includes empty-array defaults for optional collections",
    Array.isArray(json.lookups) &&
      Array.isArray(json.boundaryChecks) &&
      Array.isArray(json.drift),
  );
  assert(
    "J5. renderJson preserves intentWarnings",
    Array.isArray(json.intentWarnings) && json.intentWarnings.length === 2,
    JSON.stringify(json.intentWarnings),
  );
  assert(
    "J6. renderMarkdown suppresses benign compact intent defaults",
...

{
  ...
  const md = renderMarkdown(advisory);
  assert(
    "L4a. semantic hint routes to Search hints",
    md.includes("### Search hints") && md.includes("loadIfExists"),
  );
  assert(
    "L4b. semantic hint remains degraded search hint, not reuse claim",
    md.includes("intent-token match") &&
      md.includes("NOT a grounded reuse claim"),
  );
  assert(
    "L4c. matched tokens render for reviewer inspection",
...

{
  ...
  const md = renderMarkdown(advisory);
  assert(
    "O1. Grounded facts section renders SAFE_CUE claim",
  ...
  assert(
    "O2. Agent review cues section renders review cue separately",
    md.includes("### Agent review cues") && md.includes("near exported name"),
    md,
  );
  assert(
    "O3. Muted cue details are not rendered by default",
    !md.includes("domain-token-overlap") && !md.includes("Muted noise"),
    md,
  );
  assert(
    "O4. Unavailable evidence section renders lane status",
    md.includes("### Unavailable evidence") && md.includes("shape-index.json"),
    md,
  );
  assert(
    "O5. renderer avoids semantic/reuse-forcing wording",
...

{
  ...
  const md = renderMarkdown(advisory);
  assert(
    "O6. exact cue-covered candidate appears once in default Markdown",
...

{
  ...
  assert(
    ...
    !/\b(reuse|equivalent|safe|exists|should call|blocking failure)\b/i.test(
      md,
    ),
    md,
  );
...

{
  const advisory = {
    ...
    evidenceAvailability: {
      ...
      output: ".audit",
      guidance:
        "Run a baseline audit with the same `--output`, or rerun pre-write without `--no-fresh-audit`.",
      artifacts: [
        {
          artifact: "symbols.json",
          status: "missing",
  ...
  const json = renderJson(advisory);
  assert(
    "O7. evidence availability warning renders before lookup sections",
  ...
  assert(
    "O8. evidenceAvailability survives JSON render",
...

{
  ...
  const md = renderMarkdown(advisory);
  assert(
    "P2b local. local operation cue renders explicit review wording",
    md.includes(
      "Review related local service operation: `getWorld` inside `createRepository` in `src/repository.ts`.",
    ) &&
      md.includes(
        "shared domain tokens: `world`; operation family: `read-query`; locality: sameFile, sameDir.",
      ) &&
      md.includes(
        "supporting local-operation reasons: `local-operation-same-file-domain-overlap`.",
      ),
      !md.includes("supporting local-operation reasons: `unknown`."),
    md,
  );
  assert(
    "P2b local. local operation cue cites the policy evidence path",
    md.includes(
  ...
  assert(
    "P2b local. muted local operation details remain hidden by default",
    !md.includes("local-operation-operation-family-mismatch"),
    md,
  );
  assert(
    "P2b local. local operation renderer avoids strong action wording",
    !/\b(reuse|equivalent|safe|exists|should call|blocking failure)\b/i.test(
      md,
    ),
    md,
