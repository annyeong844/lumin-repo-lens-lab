...
check('HED2. renderer includes silent-new evidence and ACK instruction', () => {
  ...
  assert.match(rendered.text, /\[audit · observed in this\/previous tool batch\]/);
  assert.match(rendered.text, /src\/a\.ts:12/);
  assert.match(rendered.text, /value as any/);
  assert.match(rendered.text, /Event id evt_a/);
  assert.match(rendered.text, /AUDIT_ACK <event id> <intentional\|fixed\|noted>/);
  assert.deepEqual(rendered.eventIds, ['evt_a']);
  assert.equal(rendered.omittedCount, 0);
});

check('HED3. aggregate renderer uses matching-escapes wording instead of exact line assertion', () => {
  const rendered = renderEventReminderContext([
    snapshot({
      id: 'evt_many',
      occurrence_count: 3,
...
check('HED4. renderer budget omits whole event blocks and reports omittedCount', () => {
  const rendered = renderEventReminderContext([
...
check('HED5. drain missing store emits nothing and creates no event-store directory', () => {
  const fx = fixture();
  try {
    ...
    assert.equal(existsSync(path.join(fx.auditRoot, 'sessions', fx.sid, 'event-store')), false);
  } finally {
    rmSync(fx.root, { recursive: true, force: true });
  }
});

check('HED6. drain due event returns hook output and marks event delivered before return', () => {
  const fx = fixture();
  try {
    ...
    assert.match(result.output.hookSpecificOutput.additionalContext, new RegExp(appended.eventId));
