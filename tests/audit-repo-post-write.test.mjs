...
function buildFixture(fx) {
  write(
    fx,
    "package.json",
    JSON.stringify({ name: "aupost-fx", type: "module" }),
  );
...

{
  ...
  try {
    ...
    assert(
      'T1b. stdout contains "## post-write delta"',
      res.stdout.includes("## post-write delta"),
    );

    const m = readManifest(out);
    assert(
      "T1c. manifest.postWrite.requested === true",
      m.postWrite?.requested === true,
    );
    assert("T1d. manifest.postWrite.ran === true", m.postWrite?.ran === true);
    assert(
      "T1e. manifest.postWrite.deltaPath is absolute path to an existing file",
    ...
    assert(
      "T1f. manifest.postWrite.silentNew is a number",
      typeof m.postWrite?.silentNew === "number",
    );
    assert(
      "T1g. manifest.postWrite.requiredAcknowledgementCount is a number",
      typeof m.postWrite?.requiredAcknowledgementCount === "number",
    );
    assert(
      "T1h. manifest.postWrite.baselineStatus is available|missing",
      ["available", "missing"].includes(m.postWrite?.baselineStatus),
    );
    assert(
      "T1i. manifest.postWrite.scanRangeParity is ok|mismatch|baseline-missing",
      ["ok", "mismatch", "baseline-missing"].includes(
        m.postWrite?.scanRangeParity,
      ),
    );
    assert(
      "T1j. manifest.postWrite.afterComplete is boolean",
    ...
    assert(
      "T1k. silentNew matches delta.summary.silentNew",
      m.postWrite.silentNew === delta.summary.silentNew,
    );
    assert(
      "T1l. requiredAcknowledgementCount matches silent-new entry count",
      m.postWrite.requiredAcknowledgementCount ===
        (delta.entries ?? []).filter((e) => e.label === "silent-new").length,
    );
    assert(
      "T1m. baselineStatus matches delta.baseline.status",
...

{
  ...
  try {
    ...
    assert(
      "T2b. stderr mentions --pre-write-advisory",
      /pre-write-advisory/.test(res.stderr),
    );

    const m = readManifest(out);
    assert(
      "T2c. manifest.postWrite.requested === true",
      m.postWrite?.requested === true,
    );
    assert("T2d. manifest.postWrite.ran === false", m.postWrite?.ran === false);
    assert(
      "T2e. reason mentions --pre-write-advisory missing",
      /pre-write-advisory/.test(m.postWrite?.reason ?? ""),
    );
    assert(
      "T2f. no summary fields when ran===false",
  ...
  try {
    ...
    const res = spawnSync(
      ...
      [
        ...
        "--post-write",
        "--pre-write-advisory",
        path.join(out, "some.json"),
      ],
      { encoding: "utf8" },
    );

    assert(
      "T3. both flags → exit 2",
      res.status === 2,
      `status=${res.status}, stderr=${res.stderr.slice(0, 300)}`,
    );
    assert(
      "T3b. stderr mentions mutually exclusive",
      /mutually exclusive/.test(res.stderr),
    );

    const m = readManifest(out);
    assert(
      "T3c. manifest.preWrite recorded the conflict",
      m.preWrite?.ran === false &&
        /mutually exclusive/.test(m.preWrite?.reason ?? ""),
    );
    assert(
      "T3d. manifest.postWrite recorded the conflict",
  ...
  try {
    ...
    const res = spawnSync(
      ...
      [
        ...
        "--post-write",
        "--pre-write-advisory",
        path.join(out, "does-not-exist.json"),
      ],
      { encoding: "utf8" },
    );

    assert("T4. exit 0 (graceful)", res.status === 0, `status=${res.status}`);

    const m = readManifest(out);
    assert("T4b. manifest.postWrite.ran === false", m.postWrite?.ran === false);
    assert(
      'T4c. reason starts with "post-write.mjs exited non-zero:"',
...

{
  ...
  try {
    ...
    const m = readManifest(out);
    assert(
      "T5. manifest.postWrite undefined (field omitted)",
...

{
  ...
  try {
    ...
    const inv = JSON.parse(
      readFileSync(path.join(out, "any-inventory.json"), "utf8"),
    );
    assert(
      "T6. --include-tests forwarded → meta.includeTests === true",
...

{
  ...
  try {
    ...
    assert(
      "T7. --no-include-tests forwarded → meta.includeTests === false",
...

{
  ...
  try {
    ...
    assert(
      "T8. --production forwarded → meta.includeTests === false",
      inv.meta.includeTests === false,
    );
  } finally {
    rmSync(fx, { recursive: true, force: true });
...

{
  ...
  try {
    ...
    assert(
      "T9a. manifest.postWrite.deltaPath starts with --delta-out dir",
      m.postWrite?.deltaPath?.startsWith(deltaOut),
      `deltaPath=${m.postWrite?.deltaPath}, deltaOut=${deltaOut}`,
    );
    assert(
      "T9b. delta file exists at --delta-out location",
      existsSync(deltaFile),
    );
    assert(
      "T9c. default output dir does NOT contain a post-write-delta.latest.json",
...

{
  ...
  try {
    ...
    const res = runPostWrite(fx, out, advisory);
    assert(
      "T10. path with spaces + $ survives end-to-end via audit-repo",
      res.status === 0 &&
        res.stdout.includes("## post-write delta") &&
        existsSync(path.join(out, "post-write-delta.latest.json")),
    );
...

{
  ...
  try {
    ...
    assert(
      "T11a. stdout has step log BEFORE post-write delta BEFORE wrote-line",
      stepIdx >= 0 &&
        deltaIdx >= 0 &&
        wroteIdx >= 0 &&
        stepIdx < deltaIdx &&
    ...
    assert(
      "T11b. stdout has NO [post-write] diagnostic prefix",
...

{
  ...
  try {
    ...
    assert(
      "T12. --strict-post-write + bogus advisory → exit 2",
      res.status === 2,
      `status=${res.status}, stderr=${res.stderr.slice(0, 200)}`,
    );

    const m = readManifest(out);
    assert(
      "T12b. manifest.postWrite.ran === false (unchanged from non-strict)",
      m.postWrite?.ran === false,
    );
    assert(
      "T12c. reason still starts with post-write.mjs exited non-zero:",
      (m.postWrite?.reason ?? "").startsWith("post-write.mjs exited non-zero:"),
    );
  } finally {
    rmSync(fx, { recursive: true, force: true });
...

{
  ...
  try {
    ...
    assert(
      "T12a. audit-repo --post-write accepts absolute anyInventoryPath",
      res.status === 0,
      `status=${res.status}, stderr=${res.stderr.slice(0, 300)}`,
    );
    const m = readManifest(postOut);
    assert(
      "T12b. manifest baselineStatus === available for absolute before path",
...

{
  ...
  try {
    ...
    assert(
      "T13. --strict-post-write + happy path → exit 0",
      res.status === 0,
      `status=${res.status}`,
    );
    const m = readManifest(out);
    assert("T13b. manifest.postWrite.ran === true", m.postWrite?.ran === true);
  } finally {
    rmSync(fx, { recursive: true, force: true });
    rmSync(out, { recursive: true, force: true });
  }
...

{
  ...
  try {
    ...
    assert(
      "T14. --strict-post-write without --post-write → exit 0 (no effect)",
      res.status === 0,
      `status=${res.status}`,
    );
    const m = readManifest(out);
    assert("T14b. manifest.postWrite undefined", m.postWrite === undefined);
  } finally {
    rmSync(fx, { recursive: true, force: true });
    rmSync(out, { recursive: true, force: true });
  }
...

{
  ...
  try {
    ...
    const res = runPostWrite(fx, out, advisory, ["--strict-post-write"]);
    assert(
      "T15. --strict-post-write + baseline-missing → exit 0 (ran=true)",
      res.status === 0,
      `status=${res.status}`,
    );
    const m = readManifest(out);
    assert(
      "T15b. manifest.postWrite.ran === true AND baselineStatus === missing",
...

{
  ...
  try {
    ...
    assert(
      "T16. --strict-post-write-confidence + baseline-missing → exit 2",
      res.status === 2,
      `status=${res.status}, stderr=${res.stderr.slice(0, 300)}`,
    );
    assert(
      "T16b. stderr names confidence-limited post-write delta",
    ...
    const m = readManifest(out);
    assert(
      "T16c. manifest is still written with ran=true baselineStatus=missing",
      m.postWrite?.ran === true && m.postWrite?.baselineStatus === "missing",
      JSON.stringify(m.postWrite),
    );
  } finally {
...

{
  ...
  try {
    ...
    const res = runPostWrite(fx, out, advisory);
    assert(
      "T17. post-write with planned + unexpected files → exit 0",
      res.status === 0,
      `status=${res.status}; stderr=${res.stderr.slice(0, 300)}`,
    );
    ...
    const delta = JSON.parse(readFileSync(m.postWrite.deltaPath, "utf8"));
    assert(
      "T17b. fileDelta.status computed",
      delta.fileDelta?.status === "computed",
      JSON.stringify(delta.fileDelta),
    );
    assert(
      "T17c. planned file appears under plannedNew",
      delta.fileDelta?.plannedNew?.length === 1 &&
        delta.fileDelta.plannedNew[0] === "src/planned.ts",
      JSON.stringify(delta.fileDelta),
    );
    assert(
      "T17d. unplanned file appears under unexpectedNew",
      delta.fileDelta?.unexpectedNew?.length === 1 &&
        delta.fileDelta.unexpectedNew[0] === "src/unplanned.ts",
      JSON.stringify(delta.fileDelta),
    );
    assert(
      "T17e. manifest exposes unexpected new file count",
