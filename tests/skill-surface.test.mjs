...
const INTERNAL_ENGINE = readFileSync(
  path.join(DIR, "docs/internal-engine.md"),
  "utf8",
);
const SKILL = readFileSync(path.join(DIR, "SKILL.md"), "utf8");
const WRITE_GATE_SKILL = readFileSync(
  path.join(DIR, "SKILL.write-gate.md"),
  "utf8",
);
const CANON_SKILL = readFileSync(path.join(DIR, "SKILL.canon.md"), "utf8");
const PRODUCT_SURFACE = readFileSync(
  path.join(DIR, "docs/product-surface.md"),
  "utf8",
);
const HISTORY_README = readFileSync(
  path.join(DIR, "docs/history/README.md"),
  "utf8",
);
...
const MARKETPLACE = JSON.parse(
  readFileSync(path.join(DIR, ".claude-plugin/marketplace.json"), "utf8"),
);
const REFACTOR_PLAN_COMMAND = readFileSync(
  path.join(DIR, "commands/refactor-plan.md"),
  "utf8",
);
...
const WELCOME_COMMAND = readFileSync(
  path.join(DIR, "commands/welcome.md"),
  "utf8",
);
const DEFAULT_COMMAND = readFileSync(
  path.join(DIR, "commands/lumin-repo-lens-lab.md"),
  "utf8",
);
const PRE_WRITE_COMMAND = readFileSync(
  path.join(DIR, "commands/pre-write.md"),
  "utf8",
);
const POST_WRITE_COMMAND = readFileSync(
  path.join(DIR, "commands/post-write.md"),
  "utf8",
);
const CANON_DRAFT_COMMAND = readFileSync(
  path.join(DIR, "commands/canon-draft.md"),
  "utf8",
);
const CHECK_CANON_COMMAND = readFileSync(
  path.join(DIR, "commands/check-canon.md"),
  "utf8",
);
const COMMAND_ROUTING = readFileSync(
...
const REPORT_TEMPLATE = readFileSync(
  path.join(DIR, "templates/report-template.md"),
  "utf8",
);
const REFACTOR_PLAN_TEMPLATE = readFileSync(
  path.join(DIR, "templates/refactor-plan-template.md"),
  "utf8",
);
const REVIEW_CHECKLIST = readFileSync(
...
const REFACTOR_PLAN_POLICY = readFileSync(
  path.join(DIR, "references/refactor-plan-policy.md"),
  "utf8",
);
const TEMPLATES_README = readFileSync(
  path.join(DIR, "templates/README.md"),
  "utf8",
);
const HELP = execFileSync(
  process.execPath,
  [path.join(DIR, "audit-repo.mjs"), "--help"],
  {
    cwd: DIR,
    encoding: "utf8",
...
const STRUCTURAL_REVIEW_WORKFLOW_FLAT = STRUCTURAL_REVIEW_WORKFLOW.replace(
  /\s+/g,
  " ",
);
...
const ENGLISH_PUBLIC_DOCS = [
  ...
  [
    ...
    readFileSync(path.join(DIR, "references/glossary.md"), "utf8"),
  ],
...
assert(
  "S1. package.json exposes a recommended bin pointing at audit-repo.mjs",
...
assert(
  "S2. package.json description names audit-repo.mjs as the public CLI",
...
assert(
  "S2b. package.json exposes offline skill-triggering lint but keeps live sweeps opt-in",
  PACKAGE.scripts?.["build:plugin"] ===
    "node scripts/build-plugin-package.mjs" &&
    PACKAGE.scripts?.["check:skill-triggering"] ===
      "node test-harness/lib/lint-prompts.mjs" &&
    PACKAGE.scripts?.["check:behavior"] ===
      "node test-harness/lib/verify-behavior-corpus.mjs test-harness/behavior/cases.json" &&
...
assert(
  "S3. README names audit-repo.mjs as the public CLI and lists the stable validation modes",
...
assert(
  "S3b. README leads Claude Code users through marketplace install before Codex link install",
  README.includes("## First useful run") &&
    ...
    README.indexOf("## First useful run") <
      README.indexOf("### Codex-native install"),
  README.slice(0, 2400),
);

assert(
  "S3c. README documents conservative evidence boundaries and the artifact reading path",
...
assert(
  "S4. README explicitly demotes sibling root scripts to internal engine entrypoints",
...
assert(
  "S5. SKILL.md public surface section names the audit surface and sibling surfaces",
  SKILL.includes("## Public Surface") &&
    ...
    SKILL.includes(
      "This surface owns `audit`, `welcome`, and `refactor-plan`",
    ) &&
...
assert(
  "S5b. split skill source files separate audit, write-gate, and canon responsibility",
  /description:\s+"?Audit TypeScript\/JavaScript repos for structural debt/.test(
    SKILL,
  ) &&
    SKILL.includes("dead exports, cycles, oversized modules") &&
    /description:\s+"?Use before\/after TS\/JS code changes/.test(
      WRITE_GATE_SKILL,
    ) &&
    ...
    WRITE_GATE_SKILL.includes(
      "<SKILL_ROOT>/references/pre-write-intent-shape.md",
    ) &&
    ...
    WRITE_GATE_SKILL.includes(
      "<SKILL_ROOT>/references/false-positive-index.md",
    ) &&
    !WRITE_GATE_SKILL.includes(
      "<SKILL_ROOT>/references/false-positive-patterns.md",
    ) &&
    ...
    WRITE_GATE_SKILL.includes(
      "Do not ask normal chat users to hand-write JSON",
    ) &&
...
assert(
  "S5c. shared path tokens are duplicated only where independently loaded SKILL surfaces need them",
...
assert(
  "S6. SKILL.md keeps runtime canon slim and lab dirs outside the public contract",
...

{
  ...
  assert(
    "S6a. English public docs use the English `unknown` evidence label",
    offenders.length === 0 &&
...
assert(
  ...
  !Object.hasOwn(PLUGIN, "skills") &&
    ...
    COMMANDS.every((name) =>
      existsSync(path.join(DIR, "commands", `${name}.md`)),
    ),
  `${JSON.stringify(PLUGIN, null, 2)}\n${JSON.stringify(MARKETPLACE, null, 2)}`,
);

assert(
  "S6b2. welcome command gives a gentle first-touch route without running a scan",
...
assert(
  "S6b3. pre-write command accepts natural language and hides intent JSON from normal chat users",
...
assert(
  "S6c. refactor-plan command is template-led and defaults to readable coaching output",
  REFACTOR_PLAN_COMMAND.includes("Mode: `refactor-plan`") &&
    ...
    REFACTOR_PLAN_TEMPLATE.includes(
...
assert(
  "S6d. validation modes stay distinct from refactor-plan coaching mode",
...
assert(
  "S6d2. full audit review pack is an in-session reminder surface, not an external API runner",
  COMMAND_ROUTING_FLAT.includes("audit-review-pack.latest.md") &&
    ...
    COMMAND_ROUTING_FLAT.includes(
      "translate a chosen lane into a codebase-reading task",
    ) &&
    COMMAND_ROUTING_FLAT.includes(
      "Do not paste checklist or artifact lanes wholesale",
    ) &&
...
assert(
  "S6f. default slash command runs a baseline-aware current-workspace audit instead of asking for mode selection",
  DEFAULT_COMMAND.includes("Mode: `default`") &&
    ...
    readFileSync(
      path.join(DIR, "templates/REVIEW_CHECKLIST_SHORT.md"),
      "utf8",
    ).includes("Ask the coding agent:") &&
    readFileSync(
      path.join(DIR, "templates/REVIEW_CHECKLIST_SHORT.md"),
      "utf8",
    ).includes("Truth Before Warmth") &&
...
assert(
  "S6e. SKILL.md separates short chat-facing reviews from formal report-template reports",
  SKILL.split("\n").length <= 165 &&
    ...
    STRUCTURAL_REVIEW_WORKFLOW.includes(
    ...
    STRUCTURAL_REVIEW_WORKFLOW.includes(
      "does not permit skipping the checklist pass",
    ) &&
    /open `templates\/REVIEW_CHECKLIST\.md`\s+and\s+walk it/.test(
      STRUCTURAL_REVIEW_WORKFLOW,
    ) &&
    STRUCTURAL_REVIEW_WORKFLOW_FLAT.includes(
      "For explicit full audit reports, due diligence, CI-style review",
    ) &&
    SKILL.includes("follow `templates/report-template.md`") &&
    STRUCTURAL_REVIEW_WORKFLOW.includes(
      "Before finalizing any saved formal report",
    ) &&
    STRUCTURAL_REVIEW_WORKFLOW.includes(
    ...
    TEMPLATES_README.includes(
      "Maintainer-only dogfood notes live in `docs/maintainer/`",
    ),
  SKILL,
);

assert(
  "S7. audit-repo --help presents the stable validation modes on the recommended entrypoint",
...
assert(
  "S7b. full profile is documented as the shape-index profile, not quick",
  STRUCTURAL_REVIEW_WORKFLOW.includes(
...
assert(
  "S8. audit-repo --help tells users that sibling root scripts are internal engine entrypoints",
  /internal engine entrypoints/i.test(HELP_FLAT) &&
    HELP_FLAT.includes(
      "public surface is audit-repo.mjs plus the validation modes above",
    ),
  HELP,
);

assert(
  "S8b. audit routing requires a dead-export FP screen before chat smoothing candidates",
...
assert(
  "S9. README links the productization map plus history/spec/lab staging docs",
...
assert(
  "S10. docs/product-surface.md defines keep/move/hide boundaries around the public contract",
...
assert(
  "S11. docs/history/README.md anchors history as non-public and preserves the recommended contract",
  HISTORY_README.includes(
    "This area is documentation history, not the public capability contract.",
  ) &&
...
assert(
  "S12. docs/spec/README.md anchors maintainer specs outside the public contract",
  SPEC_README.includes(
    "This directory holds maintainer-facing design references",
  ) &&
...
assert(
  "S13. docs/lab/README.md anchors reproducible lab artifacts outside the public contract",
...
assert(
  "S14. docs/README.md links the public contract plus history/spec/lab staging areas",
...
assert(
  "S15. docs/internal-engine.md groups root scripts while keeping audit-repo.mjs as the recommended public CLI",
...
assert(
  "S16. .gitignore keeps generated lab artifacts out of default tracking",
...
assert(
  "S17. phase/research docs are staged while user-facing templates/references occupy shipping surface",
...
{
  const longChecklist = readFileSync(
    path.join(DIR, "templates/REVIEW_CHECKLIST.md"),
    "utf8",
  );
  const selfAuditHandbook = readFileSync(
    path.join(DIR, "docs/maintainer/SELF_AUDIT_HANDBOOK.md"),
    "utf8",
  );
  assert(
    "S17b. long checklist stays repo-neutral; self-audit handbook stays maintainer-only",
    !longChecklist.includes("For THIS repo") &&
      ...
      selfAuditHandbook.includes(
        "Use this only when reviewing `lumin-repo-lens-lab` itself",
      ) &&
...
assert(
  "S18. maintainer skill-triggering harness is present but clearly separate from shipping surface",
