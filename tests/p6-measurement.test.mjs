...

{
  ...
  assert(
    "P6-1a. missing fix-plan => candidateCounts.available=false",
    counts.available === false,
    JSON.stringify(counts),
  );
  assert(
    "P6-1b. missing fix-plan cleanup counts are null, not zero",
    counts.reviewVisibleCleanup === null &&
      counts.safeFix === null &&
      counts.reviewFix === null,
    JSON.stringify(counts),
  );
  assert(
    "P6-1c. missing dead-classify rawTierC is null",
    counts.rawTierC === null,
    JSON.stringify(counts),
  );
  assert(
    "P6-1d. missing canon-drift total is null",
...

{
  ...
  assert(
    "P6-2a. review-visible cleanup = safeFixes + reviewFixes",
    counts.reviewVisibleCleanup === 3 &&
      counts.safeFix === 2 &&
      counts.reviewFix === 1,
    JSON.stringify(counts),
  );
  assert(
    "P6-2b. degraded/muted/rawTierC/canonDrift reported separately",
    counts.degraded === 3 &&
      counts.muted === 1 &&
      counts.rawTierC === 7 &&
...

{
  ...
  assert(
    "P6-3a. inconclusive/not_applicable excluded from SAFE_FIX denominator",
  ...
  assert(
    "P6-3b. review-visible denominator includes SAFE_FIX + REVIEW_FIX",
...

{
  ...
  assert(
    "P6-4a. unavailable candidate counts force Red",
    readiness.gate === "Red" &&
      hasReason(readiness, "candidate-counts-unavailable"),
    JSON.stringify(readiness),
  );
  assert(
    "P6-4b. missing adjudication forces fp-rate-unknown",
    hasReason(readiness, "fp-rate-unknown"),
    JSON.stringify(readiness),
  );
  assert(
    "P6-4c. schemaRoundTrip.attempted=false blocks Green",
    hasReason(readiness, "schema-roundtrip-not-attempted"),
    JSON.stringify(readiness),
  );
}

{
  ...
  assert(
    "P6-5a. dirty worktree without snapshot/contentHash blocks Green",
    readiness.gate === "Red" &&
      hasReason(readiness, "dirty-worktree-without-snapshot"),
    JSON.stringify(readiness),
  );
  assert(
    "P6-5b. unknown dirty state blocks Green",
    hasReason(readiness, "dirty-worktree-unknown"),
    JSON.stringify(readiness),
  );
}

{
  ...
  assert(
    "P6-6. clean corpus + low FP + attempted roundtrip can reach Green",
...

{
  ...
  assert(
    "P6-6b. measured-zero SAFE_FIX population blocks Green without Red",
...

{
  ...
  assert(
    "P6-7a. existing canon-drift checked source marks roundtrip attempted",
    rt.attempted === true,
    JSON.stringify(rt),
  );
  assert(
    "P6-7b. parse-error source records known schema drift bug",
    rt.knownSchemaDriftBugs.some(
      (b) => b.source === "naming" && b.status === "parse-error",
    ),
    JSON.stringify(rt),
  );
}

{
  ...
  assert(
    "P6-7c. direct canon-drift artifact overrides stale manifest.checkCanon",
    rt.attempted === true &&
...

{
  const merged = mergeMeasurementArtifacts([
    {
      ...
      runtime: {
        wallMs: 10,
        childProcessCount: 1,
        steps: [{ step: "audit", status: "ok", ms: 10 }],
      },
      schemaRoundTrip: {
        attempted: true,
    ...
    },
    {
      ...
      runtime: {
        wallMs: 20,
        childProcessCount: 2,
        steps: [{ step: "audit", status: "ok", ms: 20 }],
      },
      schemaRoundTrip: {
        attempted: true,
  ...
  assert(
    "P6-8a. merge sums cleanup counts and keeps per-corpus totals",
  ...
  assert(
    "P6-8b. merge prefixes schema/canon sources by corpus",
    merged.schemaRoundTrip.sources["a:naming"].status === "clean" &&
      merged.candidateCounts.canonDrift.perSource["b:topology"].status ===
        "clean",
    JSON.stringify(merged.schemaRoundTrip),
  );
  assert(
    "P6-8c. merged two-corpus low-FP baseline can reach Green",
    readiness.gate === "Green",
    JSON.stringify(readiness),
  );
}

{
  ...
  const merged = mergeMeasurementArtifacts([
    {
      ...
      adjudication: {
        entries: [
          { corpusName: "reviewed", tier: "REVIEW_FIX", verdict: "true_dead" },
        ],
      },
      schemaRoundTrip: {
        attempted: true,
  ...
  assert(
    "P6-8d. merged corpus with review-visible candidates but no adjudication is Red",
...

{
  ...
  try {
    writeFileSync(
      path.join(out, "fix-plan.json"),
      JSON.stringify({
        safeFixes: [
          { finding: { file: "src/a.ts", symbol: "A", line: 1, bucket: "C" } },
        ],
        reviewFixes: [],
        degraded: [],
        muted: [],
    ...
    writeFileSync(
      path.join(out, "canon-drift.json"),
      JSON.stringify({
    ...
    writeFileSync(
      ...
      JSON.stringify({
        entries: [
          { corpusName: "cli", tier: "SAFE_FIX", verdict: "true_dead" },
        ],
      }),
    );
    ...
    const artifact = JSON.parse(
      readFileSync(path.join(out, "p6-measurement.json"), "utf8"),
    );
    assert(
    ...
    assert(
      ...
      artifact.candidateCounts.safeFix === 1 &&
        artifact.adjudication.entries.length === 1,
      JSON.stringify(artifact),
    );
  } finally {
...

{
  ...
  try {
    const artifact = (name, tier) => ({
      ...
      adjudication: {
        entries: [{ corpusName: name, tier, verdict: "true_dead" }],
      },
    ...
    execFileSync(
      NODE,
      [CLI, "--root", root, "--output", out, "--merge", a, "--merge", b],
      { cwd: DIR, stdio: ["ignore", "pipe", "pipe"] },
    );
