...
check('HID2. session id uses safe explicit id or deterministic transcript fallback', () => {
  assert.equal(safeSessionId({ session_id: 'sid_123' }), 'sid_123');
  assert.match(safeSessionId({ transcript_path: '/tmp/transcript.jsonl' }), /^sid_[a-f0-9]{16}$/);
...
check('HID3. tool use id uses safe explicit id', () => {
  ...
  assert.notEqual(safeToolUseId({ tool_use_id: '../bad', tool_name: 'Read' }), '../bad');
});

check('HID4. fallback tool id is deterministic and ignores hook event metadata', () => {
  const payloadA = {
...
check('HID5. fallback tool id hashes content-bearing fields without raw content', () => {
  const payload = {
    tool_name: 'Edit',
    tool_input: {
      file_path: 'src/a.ts',
