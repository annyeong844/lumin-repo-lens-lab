...
function makeDelta(overrides = {}) {
  return {
    ...
    summary: {
      planned: 0,
      plannedNotObserved: 0,
      silentNew: 0,
      preExisting: 0,
      removed: 0,
      observedUnbaselined: 0,
    },
    failures: [],
    ...overrides,
  };
...

{
  ...
  try {
    ...
    assert("T2d. latest + specific byte-identical", latest === specific);
    assert(
      "T2e. specific contains deltaInvocationId",
      JSON.parse(specific).deltaInvocationId === "DEL-1",
    );
    assert(
      "T2f. specific contains preWriteInvocationId",
...

{
  ...
  try {
    const delta = makeDelta({
      preWriteInvocationId: "PRE-X",
      deltaInvocationId: "DEL-Y",
    });
    const { specificPath } = writeDelta(out, delta);
    const basename = path.basename(specificPath);
    assert(
      "T3. filename matches post-write-delta.<pre>.<delta>.json pattern",
...

{
  ...
  try {
    ...
    try {
      writeDelta(
        out,
        makeDelta({ preWriteInvocationId: "", deltaInvocationId: "X" }),
      );
    } catch {
      threw = true;
    }
    ...
    try {
      writeDelta(
        out,
        makeDelta({ preWriteInvocationId: "X", deltaInvocationId: "" }),
      );
    } catch {
      threw = true;
    }
...

{
  ...
  try {
    writeDelta(
      out,
      makeDelta({ preWriteInvocationId: "P", deltaInvocationId: "D" }),
    );
    const files = readdirSync(out);
    const tmpLeftovers = files.filter((f) => f.includes(".tmp."));
    assert(
      "T5. no .tmp.* leftovers after normal write",
...

{
  ...
  try {
    const first = makeDelta({
      ...
      deltaInvocationId: "DELTA-A",
    });
    writeDelta(out, first);
    const second = makeDelta({
      preWriteInvocationId: "PRE",
      deltaInvocationId: "DELTA-B",
      entries: [],
    });
    ...
    assert(
      "T6a. prior specific file preserved",
      files.has("post-write-delta.PRE.DELTA-A.json"),
    );
    assert(
      "T6b. new specific file written",
      files.has("post-write-delta.PRE.DELTA-B.json"),
    );
    assert(
      "T6c. latest.json present (singular)",
    ...
    const latest = readFileSync(
      path.join(out, "post-write-delta.latest.json"),
      "utf8",
    );
    const specificB = readFileSync(
      path.join(out, "post-write-delta.PRE.DELTA-B.json"),
      "utf8",
    );
    assert(
      "T6d. latest.json overwritten to match newest specific",
      latest === specificB,
    );
  } finally {
    rmSync(out, { recursive: true, force: true });
...

{
  ...
  try {
    const delta = makeDelta({
      preWriteInvocationId: "P1",
      deltaInvocationId: "D1",
    });
    const { specificPath } = writeDelta(out, delta);
    const inJson = JSON.parse(
      readFileSync(specificPath, "utf8"),
    ).deltaInvocationId;
    const filenameComponent = path
      .basename(specificPath)
      .replace(/^post-write-delta\.P1\.(.+)\.json$/, "$1");
    assert(
      "T7. delta.deltaInvocationId === filename component byte-match",
