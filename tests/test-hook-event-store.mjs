...
check('HES1. eventStoreDir rejects unsafe session ids and builds path', () => {
  ...
  try {
    ...
    assert.throws(() => eventStoreDir(fx.auditRoot, '../bad'), /unsafe session id/);
  } finally {
...
check('HES3. duplicate append merges active unacknowledged event', () => {
  const fx = fixture();
  try {
    const first = appendEventIfNotDeduped(fx.auditRoot, fx.sid, event({ occurrence_delta: 1 }));
...
check('HES4. acknowledged active event suppresses same dedupe key', () => {
  ...
  try {
    ...
    assert.equal(markAcknowledged(fx.auditRoot, fx.sid, first.eventId, 'intentional', {
...
check('HES5. until_ack tombstone suppresses same dedupe key after cleanup', () => {
  const fx = fixture();
  try {
    const first = appendEventIfNotDeduped(fx.auditRoot, fx.sid, event());
    markAcknowledged(fx.auditRoot, fx.sid, first.eventId, 'fixed');
    assert.equal(cleanupAckedEntries(fx.auditRoot, fx.sid, {
...
check('HES6. claimDueDeliveriesAndAdvanceCursor returns due active unacked entries only', () => {
  const fx = fixture();
  try {
    const first = appendEventIfNotDeduped(fx.auditRoot, fx.sid, event({ dedupe_key: 'a', diff_key: 'a' }));
    const second = appendEventIfNotDeduped(fx.auditRoot, fx.sid, event({ dedupe_key: 'b', diff_key: 'b' }));
    ...
    assert.deepEqual(claim.snapshots.map((x) => x.id), [first.eventId]);
    assert.equal(readEventStoreState(fx.auditRoot, fx.sid).cursor.lastClaimedAt, '2026-05-08T00:30:00.000Z');
  } finally {
    rmSync(fx.root, { recursive: true, force: true });
  }
});

check('HES7. markDelivered increments delivery metadata and redelivery delay', () => {
  const fx = fixture();
  try {
    const first = appendEventIfNotDeduped(fx.auditRoot, fx.sid, event());
    assert.equal(markDelivered(fx.auditRoot, fx.sid, first.eventId, {
  ...
  try {
    appendEventIfNotDeduped(fx.auditRoot, fx.sid, event({
      data: {
        ...
        enclosing_symbol: 'parse weird-name! with spaces and symbols '.repeat(4),
...
check('HES10. unsafe session id reads are safe and do not traverse', () => {
  ...
  try {
    ...
    assert.equal(claimDueDeliveriesAndAdvanceCursor(fx.auditRoot, '../bad').snapshots.length, 0);
    assert.equal(markAcknowledged(fx.auditRoot, '../bad', 'evt_bad', 'fixed'), false);
  } finally {
    rmSync(fx.root, { recursive: true, force: true });
  }
});
...
check('HES11. append times out on fresh lock without corrupting store', () => {
  ...
  try {
    ...
    assert.deepEqual(result, { appended: false, eventId: null, reason: 'lock-timeout' });
...
check('HES12. append recovers stale event-store lock', () => {
  const fx = fixture();
  try {
    const lockDir = path.join(eventStoreDir(fx.auditRoot, fx.sid), '.event-store.lock');
