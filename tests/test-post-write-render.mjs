...
function makeEntry({ label, escapeKind = 'as-any', file = 'src/a.ts', line = 1, codeShape = 'x as any', insideExportedIdentity = null, occurrenceKey = 'sha256:K', plannedEntry = null, diagnostics = [] } = {}) {
  return {
    label, escapeKind, file, line, codeShape,
    normalizedCodeShape: codeShape, insideExportedIdentity,
    occurrenceKey, plannedEntry, diagnostics,
  };
}
...
function makeDelta({
  ...
  const s = summary ?? {
    ...
    plannedNotObserved: entries.filter(e => e.label === 'planned-not-observed').length,
    ...
    observedUnbaselined: entries.filter(e => e.label === 'observed-unbaselined').length,
  };
  return {
    preWriteInvocationId, deltaInvocationId, intentHash,
    baseline, capabilityParity, scanRangeParity, inventoryCompleteness,
    entries, summary: s, capabilityFailures,
  };
}
...

{
  ...
  assert('A. header present',
    md.includes('## post-write delta (canonical/any-contamination §6 Stage 2)'));
  assert('A. "Planned and observed:" section rendered',
    md.includes('Planned and observed:'));
  assert('A. "New escape sites" section OMITTED (silentNew===0)',
    !md.includes('New escape sites'));
  assert('A. clean summary rendered (all conditions satisfied)',
    md.includes('No silent new any in the scan range.'));
  assert('A. planned row shows escape code',
    md.includes('response as unknown as ThirdPartyShape'));
  assert('A. planned row shows insideExportedIdentity',
...

{
  ...
  const md = renderMarkdown(delta);
  assert('B. "New escape sites (silent-new — REQUIRE acknowledgment)" present',
    md.includes('New escape sites (silent-new — REQUIRE acknowledgment):'));
  assert('B. silent-new row cites occurrenceKey',
    md.includes('sha256:FIX-B') && md.includes('[grounded'));
  assert('B. "Any delta" block shows +1 for as-any',
    /as any:\s+\+1/.test(md));
  assert('B. "Any delta" block shows +0 for other kinds',
    /explicit any:\s+\+0/.test(md));
  assert('B. silent-new summary rendered',
    md.includes('silent-new — REQUIRE acknowledgment: 1'));
  assert('B. "No silent new any in the scan range." NOT present',
    !md.includes('No silent new any in the scan range.'));
}
...
{
  const delta = makeDelta({
    baseline: { status: 'missing', source: null, reason: 'advisory has no anyInventoryPath' },
    scanRangeParity: { status: 'baseline-missing' },
    inventoryCompleteness: { afterComplete: true, beforeComplete: null, filesWithParseErrors: [] },
    entries: [
      makeEntry({
  ...
  const md = renderMarkdown(delta);
  assert('C. "Observed without baseline:" section present',
    md.includes('Observed without baseline:'));
  assert('C. row carries [확인 불가] citation (not [grounded])',
    md.includes('[확인 불가') && !md.includes('[grounded, any-inventory'));
  assert('C. "Any delta" shows +0 for all kinds (observed-unbaselined not counted)',
    /as any:\s+\+0/.test(md) && /explicit any:\s+\+0/.test(md));
  assert('C. caveated summary mentions before-inventory missing',
    md.includes('No silent-new acknowledgements required') &&
    md.includes('before-inventory missing'));
  assert('C. clean summary NOT present',
    !md.includes('No silent new any in the scan range.'));
}
...

{
  ...
  const md = renderMarkdown(delta);
  assert('D. "Planned but not observed:" section present',
    md.includes('Planned but not observed:'));
  assert('D. row mentions plannedEntry locationHint',
    md.includes('src/migration.ts::adapter'));
  assert('D. "Any delta" shows +0 for all kinds',
    /as any:\s+\+0/.test(md) && /ts-expect-error:\s+\+0/.test(md));
  assert('D. clean summary rendered (silentNew=0, all clean)',
    md.includes('No silent new any in the scan range.'));
}
  ...
  const delta = makeDelta({
    inventoryCompleteness: {
      ...
      filesWithParseErrors: [
        { side: 'after', file: 'src/bad.ts', message: 'Unexpected token', line: 12 },
      ],
    },
    entries: [],
  });
  const md = renderMarkdown(delta);
  assert('E. "Inventory completeness:" section present',
    md.includes('Inventory completeness:'));
  assert('E. after-inventory parse-error file listed',
    md.includes('src/bad.ts') && md.includes('Unexpected token'));
  assert('E. caveated summary mentions after-inventory incomplete',
    md.includes('No silent-new acknowledgements required') &&
    md.includes('after-inventory incomplete'));
  assert('E. clean summary NOT rendered',
    !md.includes('No silent new any in the scan range.'));
}
...
{
  const delta = makeDelta({
    scanRangeParity: { status: 'mismatch', mismatchDetail: 'includeTests: before=false after=true' },
    entries: [
      makeEntry({
        label: 'planned', escapeKind: 'as-any', file: 'src/planned.ts', line: 10,
        codeShape: 'x as any', occurrenceKey: 'sha256:FIX-F-P',
        plannedEntry: { escapeKind: 'as-any', locationHint: 'src/planned.ts', reason: 'r' },
      }),
      makeEntry({
        label: 'silent-new', escapeKind: 'as-any', file: 'src/new.ts', line: 20,
        codeShape: 'y as any', occurrenceKey: 'sha256:FIX-F-S',
      }),
      makeEntry({
        label: 'observed-unbaselined', escapeKind: 'explicit-any', file: 'src/obs.ts', line: 30,
        codeShape: 'z: any', occurrenceKey: 'sha256:FIX-F-O',
      }),
      makeEntry({
        label: 'planned-not-observed', escapeKind: 'ts-ignore', occurrenceKey: null,
        plannedEntry: { escapeKind: 'ts-ignore', locationHint: 'src/pno.ts', reason: 'r' },
      }),
      makeEntry({
        label: 'removed', escapeKind: 'as-any', file: 'src/old.ts', line: 5,
        codeShape: 'legacy as any', occurrenceKey: 'sha256:FIX-F-R',
      }),
    ],
  });
  const md = renderMarkdown(delta);
  assert('F. all 5 occurrence sections rendered',
    md.includes('Planned and observed:') &&
    md.includes('New escape sites') &&
    md.includes('Observed without baseline:') &&
    md.includes('Planned but not observed:') &&
    md.includes('Removed:'));
  assert('F. "Any delta" shows +1 for as-any (silent-new kind only)',
    /as any:\s+\+1/.test(md));
  assert('F. "Any delta" shows +0 for explicit-any (observed-unbaselined does NOT count)',
    /explicit any:\s+\+0/.test(md));
  assert('F. summary is acknowledgment-required (silentNew > 0 wins)',
    md.includes('silent-new — REQUIRE acknowledgment: 1'));
  assert('F. scanRangeParity mismatch status line rendered',
...

{
  const delta = makeDelta({
    entries: [
      makeEntry({ label: 'planned', occurrenceKey: 'k1', plannedEntry: { escapeKind: 'as-any', locationHint: 'x', reason: 'r' } }),
      makeEntry({ label: 'silent-new', occurrenceKey: 'k2' }),
      makeEntry({ label: 'observed-unbaselined', occurrenceKey: 'k3' }),
      makeEntry({ label: 'planned-not-observed', plannedEntry: { escapeKind: 'as-any', locationHint: 'y', reason: 'r' } }),
  ...
  for (const marker of order) {
    ...
    if (idx < 0) { monotonic = false; break; }
    if (idx < prev) { monotonic = false; break; }
    prev = idx;
  }
...

{
  const delta = makeDelta({
    entries: [
      makeEntry({
        label: 'pre-existing', occurrenceKey: 'k-amb',
  ...
  const delta2 = makeDelta({
    entries: [
      makeEntry({
        label: 'silent-new', occurrenceKey: 'k-amb2',
        diagnostics: ['ambiguous-planned-match'],
      }),
    ],
  });
  const md2 = renderMarkdown(delta2);
  assert('AMB. silent-new entry with ambiguous-planned-match shows suffix',
...
{
  const delta = makeDelta({
    capabilityParity: { status: 'mismatch', mismatchDetail: 'supports.typeEscapes !== true' },
    entries: [],
  });
  const md = renderMarkdown(delta);
  assert('CAP_MIS. capability mismatch: no per-occurrence sections',
    !md.includes('Planned and observed:') &&
    !md.includes('New escape sites') &&
    !md.includes('Observed without baseline:'));
  assert('CAP_MIS. capability mismatch status line rendered clearly',
    md.includes('Capability parity') && md.includes('mismatch'));
  assert('CAP_MIS. mismatchDetail surfaced',
    md.includes('supports.typeEscapes'));
}

{
  const delta = makeDelta({
    capabilityParity: { status: 'missing', mismatchDetail: 'afterInventory absent' },
    entries: [],
  });
  const md = renderMarkdown(delta);
  assert('CAP_MIS2. capabilityParity: missing rendered as distinct state',
    md.includes('Capability parity') && md.includes('missing'));
  assert('CAP_MIS2. mentions after-inventory-missing / absent',
    md.includes('after-inventory') || md.includes('afterInventory absent'));
  assert('CAP_MIS2. no per-occurrence sections',
...

{
  ...
  assert('LIT. "silent-new — REQUIRE acknowledgment" appears (section header + summary)',
    count >= 2);
  // But NOT anywhere unrelated (e.g. not in Observed without baseline section)
  const cleanDelta = makeDelta({ entries: [ makeEntry({ label: 'observed-unbaselined', occurrenceKey: 'x' }) ], baseline: { status: 'missing', reason: 'r' } });
  const cleanMd = renderMarkdown(cleanDelta);
  assert('LIT. "silent-new — REQUIRE acknowledgment" absent when silentNew=0',
...
{
  const delta = makeDelta({ entries: [makeEntry({ label: 'silent-new', occurrenceKey: 'k' })] });
  const json = renderJson(delta);
  assert('JSON. renderJson returns structurally identical delta',
