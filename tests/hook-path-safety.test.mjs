...
check("HPS2. tool target extraction covers mutating file tools only", () => {
  assert.equal(
    getToolTargetPath("Edit", { file_path: "src/a.ts" }),
    "src/a.ts",
  );
  assert.equal(
    getToolTargetPath("Write", { file_path: "src/a.ts" }),
    "src/a.ts",
  );
  assert.equal(
    getToolTargetPath("MultiEdit", { file_path: "src/a.ts" }),
    "src/a.ts",
  );
...
check(
  "HPS4. syntactic repo-relative path validation does not touch disk",
  () => {
