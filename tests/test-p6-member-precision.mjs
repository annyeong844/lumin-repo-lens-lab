...

{
  ...
  let out;
  try {
    write(fx, 'package.json', JSON.stringify({ name: 'p6-member-ns', type: 'module' }));
    write(fx, 'src/mod.ts',
    ...
    assert('P6M-1a. namespace direct member protects only the called export',
    ...
    assert('P6M-1b. unrelated namespace sibling remains a concrete dead candidate',
...

{
  ...
  let out;
  try {
    write(fx, 'package.json', JSON.stringify({ name: 'p6-member-dyn-var', type: 'module' }));
    write(fx, 'src/mod.ts',
    ...
    assert('P6M-2a. await import binding direct member protects the called export',
    ...
    assert('P6M-2b. await import direct member does not blanket-protect siblings',
...

{
  ...
  let out;
  try {
    write(fx, 'package.json', JSON.stringify({ name: 'p6-member-dyn-then', type: 'module' }));
    write(fx, 'src/mod.ts',
    ...
    assert('P6M-3a. import().then callback member protects the called export',
    ...
    assert('P6M-3b. import().then direct member does not blanket-protect siblings',
...

{
  ...
  let out;
  try {
    write(fx, 'package.json', JSON.stringify({ name: 'p6-member-degraded', type: 'module' }));
    write(fx, 'src/mod.ts',
    ...
    assert('P6M-4. degraded namespace alias keeps the conservative whole-file shadow',
      symbols.deadTotal === 2 &&
        symbols.trulyDead === 0 &&
...

{
  ...
  let out;
  try {
    write(fx, 'package.json', JSON.stringify({ name: 'p6-member-dyn-shadow', type: 'module' }));
    write(fx, 'src/mod.ts',
    ...
    assert('P6M-5a. shadowed dynamic binding keeps outer member attribution lexical',
    ...
    assert('P6M-5b. shadowed inner dynamic binding attributes to its own module',
    ...
    assert('P6M-5c. shadowed dynamic binding does not hide unrelated dead exports',
      !!deadSymbol(symbols, 'unused') &&
        !!deadSymbol(symbols, 'otherUnused'),
...

{
  ...
  let out;
  try {
    write(fx, 'package.json', JSON.stringify({ name: 'p6-member-ns-shadow', type: 'module' }));
    write(fx, 'src/mod.ts',
    ...
    assert('P6M-6a. namespace parameter shadow does not steal module attribution',
    ...
    assert('P6M-6b. namespace shadowed-only export remains a concrete dead candidate',
