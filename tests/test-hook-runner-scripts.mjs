...
function runHook(scriptRel, payloadOrRaw, { cwd = ROOT } = {}) {
  const input = typeof payloadOrRaw === 'string'
    ? payloadOrRaw
...
check('HRS2. pre-tool-use captures preimage for a mutating Edit payload', () => {
  const fx = fixture();
  try {
    writeSource(fx.root, 'src/a.ts', 'export const value = raw;\n');
    const result = runHook('hooks/pre-tool-use.mjs', preToolPayload(fx), { cwd: fx.root });
    assert.equal(result.status, 0, result.stderr);
    assert.equal(result.stdout, '');
    ...
    assert.equal(existsSync(preimagePath(fx.auditRoot, fx.sid, 'tool_runner')), true);
  } finally {
    rmSync(fx.root, { recursive: true, force: true });
  }
});

check('HRS3. post-tool-batch emits a delivered silent-new reminder from captured preimage', () => {
  const fx = fixture();
  try {
    writeSource(fx.root, 'src/a.ts', 'export const value = raw;\n');
    const pre = runHook('hooks/pre-tool-use.mjs', preToolPayload(fx), { cwd: fx.root });
    assert.equal(pre.status, 0, pre.stderr);

    writeSource(fx.root, 'src/a.ts', 'export const value = raw as any;\n');
    const post = runHook('hooks/post-tool-batch.mjs', postBatchPayload(fx), { cwd: fx.root });
    assert.equal(post.status, 0, post.stderr);
    const hookOutput = parseHookJson(post.stdout);
    assert.equal(hookOutput.hookSpecificOutput.hookEventName, 'PostToolBatch');
    assert.match(hookOutput.hookSpecificOutput.additionalContext, /AUDIT_ACK <event id>/);
...
check('HRS4. stop runner acknowledges AUDIT_ACK lines', () => {
  ...
  try {
    ...
    writeSource(fx.root, 'src/a.ts', 'export const value = raw as any;\n');
    runHook('hooks/post-tool-batch.mjs', postBatchPayload(fx), { cwd: fx.root });

    const eventId = readEventStoreState(fx.auditRoot, fx.sid).entries[0].id;
    const stop = runHook('hooks/stop.mjs', {
      cwd: fx.root,
      session_id: fx.sid,
...
check('HRS5. user-prompt-submit drains already due events', () => {
  ...
  try {
    appendEventIfNotDeduped(fx.auditRoot, fx.sid, {
      ...
      diff_key: 'sha256:1111111111111111111111111111111111111111111111111111111111111111',
      dedupe_key: 'sha256:1111111111111111111111111111111111111111111111111111111111111111',
      occurrence_delta: 1,
      data: {
        file: 'src/a.ts',
        line: 1,
    ...
    const prompt = runHook('hooks/user-prompt-submit.mjs', {
      cwd: fx.root,
      session_id: fx.sid,
    }, { cwd: fx.root });
    ...
    assert.match(hookOutput.hookSpecificOutput.additionalContext, /AUDIT_ACK <event id>/);
    assert.equal(readEventStoreState(fx.auditRoot, fx.sid).entries[0].delivered_count, 1);
  } finally {
    rmSync(fx.root, { recursive: true, force: true });
  }
});
