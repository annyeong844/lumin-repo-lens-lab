...
try {
  ...
  writeFileSync(
    path.join(root, "src", "event-dispatcher.ts"),
    [
  ...
  const fileMethods =
    symbols.classMethodIndex?.["src/event-dispatcher.ts"] ?? {};
  const handleDeleteMethods = fileMethods.handleDelete ?? [];

  assert(
    "CM1. symbols.json advertises class method pre-write surface support",
    symbols.meta?.supports?.classMethodIndex === true,
    JSON.stringify(symbols.meta?.supports, null, 2),
  );
  assert(
    "CM2. class method index records handleDelete with class owner",
    handleDeleteMethods.some(
      (m) =>
  ...
  assert(
    "CM3. class methods do not get promoted into export defIndex",
  ...
  assert(
    "CM4. pre-write nearNames can see class method handleDelete",
    near.some(
      (n) =>
        n.name === "handleDelete" &&
  ...
  assert(
    "CM5. delete-domain method outranks unrelated handle-prefix methods",
