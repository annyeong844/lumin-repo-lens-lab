...

{
  ...
  try {
    ...
    const res = spawnSync(NODE, [AUDIT_CLI,
      '--root', fx, '--output', out, '--canon-draft',
    ], { encoding: 'utf8' });
    assert('FA-1. exit 0 with topology.json present',
      res.status === 0, `stderr=${res.stderr.slice(0, 400)}`);

    const manifest = readManifest(out);
    assert('FA-2. manifest.canonDraft.requested === true',
      manifest.canonDraft?.requested === true);
    assert('FA-3. manifest.canonDraft.ran === true',
      manifest.canonDraft?.ran === true);
    assert('FA-4. perSource has all 4 keys',
      manifest.canonDraft?.perSource &&
      Object.keys(manifest.canonDraft.perSource).length === 4);
    assert('FA-5. all 4 sources ran + exitCode 0',
    ...
    assert('FA-9. first-read summary surfaces canon-draft command result',
      summaryMd.includes('## Command Result') &&
      summaryMd.includes('Canon draft wrote 4 proposal files'),
      summaryMd);
    assert('FA-10. console preview surfaces canon-draft result before JSON spelunking',
...

{
  ...
  try {
    ...
    assert('FB-1. exit 0 (advisory)',
      res.status === 0, `stderr=${res.stderr.slice(0, 400)}`);

    const manifest = readManifest(out);
    assert('FB-2. manifest.canonDraft.ran === true',
      manifest.canonDraft?.ran === true);
    assert('FB-3. topology source SUCCEEDED — pipeline produced topology.json before canon-draft',
    ...
    assert('FB-4. all 4 sources succeeded',
      ['type-ownership', 'helper-registry', 'topology', 'naming'].every((s) =>
        manifest.canonDraft?.perSource?.[s]?.ran === true));
    assert('FB-5. topology.json was produced by audit-repo pipeline',
...

{
  ...
  try {
    ...
    const res = spawnSync(NODE, [AUDIT_CLI,
      '--root', fx, '--output', out, '--canon-draft',
      '--sources', 'type-ownership,naming',
    ], { encoding: 'utf8' });
    assert('S1. exit 0 with scoped --sources',
      res.status === 0, `stderr=${res.stderr.slice(0, 400)}`);
    const manifest = readManifest(out);
    assert('S2. requestedSources = [type-ownership, naming]',
    ...
    assert('S3. perSource has EXACTLY those 2 keys (requested-only per P1-9)',
    ...
    assert('S4. topology + helper-registry NOT in perSource (not requested)',
...

{
  ...
  try {
    ...
    assert('SAL-1. --source alias scopes canon-draft',
      res.status === 0, `stderr=${res.stderr.slice(0, 400)}`);
    const manifest = readManifest(out);
    assert('SAL-2. requestedSources = [naming]',
      Array.isArray(manifest.canonDraft?.requestedSources) &&
    ...
    assert('SAL-3. perSource has only naming',
...

{
  ...
  try {
    ...
    assert('SA-1. --sources all → exit 0',
      res.status === 0, `stderr=${res.stderr.slice(0, 400)}`);
    const manifest = readManifest(out);
    assert('SA-2. requestedSources expanded to 4 named sources',
      Array.isArray(manifest.canonDraft?.requestedSources) &&
      manifest.canonDraft.requestedSources.length === 4 &&
      ['type-ownership', 'helper-registry', 'topology', 'naming'].every((s) =>
        manifest.canonDraft.requestedSources.includes(s)));
    assert('SA-3. perSource has all 4 entries',
      Object.keys(manifest.canonDraft?.perSource ?? {}).length === 4);
  } finally {
    rmSync(fx, { recursive: true, force: true });
    rmSync(out, { recursive: true, force: true });
...

{
  ...
  try {
    ...
    assert('SD-1. --sources "all,naming" → exit 0',
      res.status === 0, `stderr=${res.stderr.slice(0, 400)}`);
    const manifest = readManifest(out);
    assert('SD-2. requestedSources deduped to 4',
      Array.isArray(manifest.canonDraft?.requestedSources) &&
      manifest.canonDraft.requestedSources.length === 4);
  } finally {
    rmSync(fx, { recursive: true, force: true });
...

{
  ...
  try {
    ...
    const res = spawnSync(NODE, [AUDIT_CLI,
      '--root', fx, '--output', out, '--canon-draft',
      '--sources', 'type-ownership',
    ], { encoding: 'utf8' });
    assert('SV-1. versioned draft run exits 0',
      res.status === 0, `stderr=${res.stderr.slice(0, 400)}`);
    const manifest = readManifest(out);
    const draftPath = manifest.canonDraft?.perSource?.['type-ownership']?.draftPath;
    assert('SV-2. manifest draftPath points at type-ownership.v2.md',
      typeof draftPath === 'string' &&
      path.basename(draftPath) === 'type-ownership.v2.md',
      `draftPath=${draftPath}`);
    assert('SV-3. manifest draftPath exists on disk',
      typeof draftPath === 'string' && existsSync(draftPath));
    assert('SV-4. draftPaths uses the same versioned path',
...

{
  ...
  try {
    ...
    const res = spawnSync(NODE, [AUDIT_CLI,
      '--root', fx, '--output', out, '--canon-draft',
      '--sources', 'type-ownership,foobar,naming',
    ], { encoding: 'utf8' });
    assert('U1. unknown --sources value → exit 1',
      res.status === 1, `got=${res.status}`);
    const manifest = readManifest(out);
    assert('U2. manifest.canonDraft.ran === false with reason',
      manifest.canonDraft?.ran === false &&
      /unknown --sources/.test(manifest.canonDraft?.reason ?? ''));
    assert('U3. No partial perSource keys (validation before execution)',
      !manifest.canonDraft?.perSource ||
...

{
  ...
  try {
    ...
    assert('T1. --sources topology through orchestrator → exit 0 (pipeline guaranteed prerequisite)',
      res.status === 0, `got=${res.status}; stderr=${res.stderr.slice(0, 400)}`);
    const manifest = readManifest(out);
    assert('T2. ran=true with perSource = {topology: {ran:true, exitCode:0}}',
      manifest.canonDraft?.ran === true &&
      manifest.canonDraft?.perSource?.topology?.ran === true &&
...

{
  ...
  try {
    ...
    const res = spawnSync(NODE, [AUDIT_CLI,
      '--root', fx, '--output', out, '--canon-draft',
      '--source', 'naming',
      '--canon-output', canonOut,
    ], { encoding: 'utf8' });
    assert('CO-1. --canon-output through orchestrator → exit 0',
      res.status === 0, `got=${res.status}; stderr=${res.stderr.slice(0, 400)}`);
    assert('CO-2. custom canon-output receives naming draft',
      existsSync(path.join(canonOut, 'naming.md')));
    assert('CO-3. custom canon-output avoids default root/canonical-draft write',
      !existsSync(path.join(fx, 'canonical-draft', 'naming.md')));
    const manifest = readManifest(out);
    assert('CO-4. manifest draftPath points at custom canon-output',
      manifest.canonDraft?.draftPaths?.length === 1 &&
      path.resolve(manifest.canonDraft.draftPaths[0]) === path.join(canonOut, 'naming.md'));
  } finally {
...

{
  ...
  assert('P1. PROFILE validation keeps quick|full|ci only (no canon-draft profile)',
    profileSectionRe.test(stripped));

  // --canon-draft must never appear as DEFAULT for profile.
  const suspect = /canon-?draft.*['"]quick['"]|canon-?draft.*['"]full['"]|canon-?draft.*['"]ci['"]/;
  assert('P2. no profile-default includes --canon-draft',
...

{
  ...
  assert('W1. audit-repo.mjs does NOT import source-logic (thin wrapper)',
    flagged.length === 0,
...
{
  const src = readFileSync(path.join(DIR, 'audit-repo.mjs'), 'utf8');
  const helper = readFileSync(path.join(DIR, '_lib', 'audit-canon-draft.mjs'), 'utf8');
  const stripped = src
    .replace(/\/\*[\s\S]*?\*\//g, '')
    .replace(/\/\/[^\n]*/g, '');
  ...
  const helperHasImport = /CANON_DRAFT_SOURCES/.test(strippedHelper) &&
  ...
  assert('C2. audit-repo.mjs and helper do NOT locally declare CANON_DRAFT_SOURCES',
...

{
  ...
  try {
    ...
    assert('SH1. path with spaces + $ survives orchestrator',
