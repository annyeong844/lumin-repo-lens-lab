...
import { chmodSync, writeFileSync, readFileSync, mkdirSync, mkdtempSync, rmSync, existsSync } from 'node:fs';
...
function buildFixture(fxDir) {
  write(fxDir, 'package.json', JSON.stringify({ name: 'au-fx', type: 'module' }));
...

{
  ...
  try {
    ...
    const res = spawnSync(NODE, [AUDIT_REPO,
      '--root', fx, '--output', out, '--pre-write', '--profile', 'quick',
    ], { encoding: 'utf8' });

    assert('T1. --pre-write without --intent → exit code 2',
      res.status === 2, `status=${res.status}, stderr=${res.stderr.slice(0, 300)}`);

    assert('T1b. stderr contains helpful message',
      /--pre-write requested but skipped.*--intent/.test(res.stderr),
      `stderr=${res.stderr.slice(0, 300)}`);

    const manifest = JSON.parse(readFileSync(path.join(out, 'manifest.json'), 'utf8'));
    assert('T1c. manifest.preWrite.requested === true',
      manifest.preWrite?.requested === true);
    assert('T1d. manifest.preWrite.ran === false',
      manifest.preWrite?.ran === false);
    assert('T1e. manifest.preWrite.reason mentions intent',
      /intent/i.test(manifest.preWrite?.reason ?? ''),
      `reason=${manifest.preWrite?.reason}`);
    assert('T1e2. pre-write-only missing intent does not run the base quick audit',
      !(manifest.commandsRun ?? []).some((cmd) =>
        ['triage-repo.mjs', 'measure-topology.mjs', 'build-symbol-graph.mjs'].includes(cmd.step)),
      JSON.stringify(manifest.commandsRun));

    // No advisory should have been written.
    assert('T1f. pre-write-advisory.latest.json NOT created',
...

{
  ...
  try {
    ...
    const res = spawnSync(NODE, [AUDIT_REPO,
      '--root', fx, '--output', out, '--pre-write', '--intent', intentPath, '--profile', 'quick',
    ], { encoding: 'utf8' });

    assert('T2. --pre-write + --intent → exit 0',
      res.status === 0, `status=${res.status}, stderr=${res.stderr.slice(0, 300)}`);

    const manifest = JSON.parse(readFileSync(path.join(out, 'manifest.json'), 'utf8'));
    assert('T2b. manifest.preWrite.ran === true',
      manifest.preWrite?.ran === true,
      `preWrite=${JSON.stringify(manifest.preWrite)}`);

    assert('T2c. pre-write-advisory.latest.json written',
      existsSync(path.join(out, 'pre-write-advisory.latest.json')));
    assert('T2c2. manifest.preWrite.advisoryPath points at invocation-specific advisory',
      path.basename(manifest.preWrite?.advisoryPath ?? '').startsWith('pre-write-advisory.') &&
      path.basename(manifest.preWrite?.advisoryPath ?? '') !== 'pre-write-advisory.latest.json' &&
      existsSync(manifest.preWrite.advisoryPath),
      JSON.stringify(manifest.preWrite));
    assert('T2c3. manifest.preWrite.latestAdvisoryPath keeps latest as convenience pointer only',
      path.basename(manifest.preWrite?.latestAdvisoryPath ?? '') === 'pre-write-advisory.latest.json' &&
      existsSync(manifest.preWrite.latestAdvisoryPath),
      JSON.stringify(manifest.preWrite));
    assert('T2d. audit-repo --pre-write skips the base quick-audit producer chain',
      !(manifest.commandsRun ?? []).some((cmd) =>
        [
          'triage-repo.mjs',
      ...
      JSON.stringify(manifest.commandsRun));
    assert('T2e. names-only pre-write path creates symbols but not triage/topology/fix-plan',
    ...
    assert('T2e2. manifest.preWrite mirrors advisory evidence availability',
      manifest.preWrite?.evidenceAvailability?.status === 'available' &&
      manifest.preWrite.evidenceAvailability.artifacts?.some((entry) =>
    ...
    assert('T2f. pre-write-only path emits a command-result summary, not a repo-wide audit summary',
...

{
  ...
  try {
    ...
    assert('T3. default audit exits 0',
      res.status === 0);

    const manifest = JSON.parse(readFileSync(path.join(out, 'manifest.json'), 'utf8'));
    assert('T3b. manifest.preWrite is absent or requested:false',
      !manifest.preWrite || manifest.preWrite.requested !== true,
      `preWrite=${JSON.stringify(manifest.preWrite)}`);

    assert('T3c. no pre-write advisory artifact',
      !existsSync(path.join(out, 'pre-write-advisory.latest.json')));
  } finally {
    rmSync(fx, { recursive: true, force: true });
    rmSync(out, { recursive: true, force: true });
...

{
  ...
  try {
    ...
    const res = spawnSync(NODE, [AUDIT_REPO,
      '--root', fx, '--output', out, '--pre-write', '--intent', '-', '--profile', 'quick',
    ], {
      encoding: 'utf8',
      input: JSON.stringify(intent),
    });

    assert('T3s1. --pre-write --intent - → exit 0',
      res.status === 0, `status=${res.status}, stderr=${res.stderr.slice(0, 300)}`);

    const manifest = JSON.parse(readFileSync(path.join(out, 'manifest.json'), 'utf8'));
    assert('T3s2. stdin intent path records preWrite ran=true',
      manifest.preWrite?.ran === true,
      `preWrite=${JSON.stringify(manifest.preWrite)}`);
    assert('T3s3. stdin intent writes advisory',
      existsSync(path.join(out, 'pre-write-advisory.latest.json')));
    assert('T3s4. stdin pre-write-only path still skips base quick audit',
      !(manifest.commandsRun ?? []).some((cmd) =>
        ['triage-repo.mjs', 'measure-topology.mjs', 'build-symbol-graph.mjs'].includes(cmd.step)),
      JSON.stringify(manifest.commandsRun));
    const summaryMd = readFileSync(path.join(out, 'audit-summary.latest.md'), 'utf8');
    assert('T3s5. stdin pre-write path surfaces command result in the first-read summary',
      summaryMd.includes('## Command Result') &&
      summaryMd.includes('Pre-write ran and wrote an advisory'),
      summaryMd);
  } finally {
...

{
  ...
  try {
    ...
    assert('T3f1. malformed stdin intent propagates non-zero exit',
      res.status === 1, `status=${res.status}, stderr=${res.stderr.slice(0, 500)}`);

    const manifest = JSON.parse(readFileSync(path.join(out, 'manifest.json'), 'utf8'));
    assert('T3f2. failed stdin intent records preWrite ran=false',
      manifest.preWrite?.requested === true && manifest.preWrite?.ran === false,
      `preWrite=${JSON.stringify(manifest.preWrite)}`);
    assert('T3f3. failed stdin intent manifest names pre-write child failure',
      /pre-write\.mjs exited non-zero/.test(manifest.preWrite?.reason ?? ''),
      `reason=${manifest.preWrite?.reason}`);
    assert('T3f4. failed stdin intent does not create advisory',
      !existsSync(path.join(out, 'pre-write-advisory.latest.json')));
  } finally {
    rmSync(fx, { recursive: true, force: true });
    rmSync(out, { recursive: true, force: true });
...

{
  ...
  try {
    ...
    assert('T4. space + $ path: audit-repo --pre-write exits 0',
      res.status === 0, `status=${res.status}, stderr=${res.stderr.slice(0, 300)}`);
    assert('T4b. advisory written under space+$ path',
      existsSync(path.join(out, 'pre-write-advisory.latest.json')));
  } finally {
...

{
  ...
  try {
    write(fx, 'package.json', JSON.stringify({ name: 'au-pre-scope', type: 'module' }));
    write(fx, 'src/prod.ts', 'export const prodOnly = 1;\n');
    write(fx, 'src/prod.test.ts', 'export const testOnly = 1;\n');
    const intent = { names: ['prodOnly'], shapes: [], files: [], dependencies: [], plannedTypeEscapes: [] };
    const intentPath = path.join(out, 'intent.json');
    writeFileSync(intentPath, JSON.stringify(intent));
    ...
    assert('T5. audit-repo --pre-write --production exits 0',
      res.status === 0, `status=${res.status}, stderr=${res.stderr.slice(0, 300)}`);

    const manifest = JSON.parse(readFileSync(path.join(out, 'manifest.json'), 'utf8'));
    const advisory = JSON.parse(readFileSync(path.join(out, 'pre-write-advisory.latest.json'), 'utf8'));
    const symbols = JSON.parse(readFileSync(path.join(out, 'symbols.json'), 'utf8'));
    const defFiles = Object.keys(symbols.defIndex ?? {});

    assert('T5b. orchestrator manifest records production scan range',
      manifest.scanRange?.includeTests === false && manifest.scanRange?.production === true,
      JSON.stringify(manifest.scanRange));
    assert('T5c. pre-write advisory records the same production scan range',
      advisory.scanRange?.includeTests === false,
      JSON.stringify(advisory.scanRange));
    assert('T5d. pre-write child/cold-cache graph excludes test files',
      !defFiles.some((f) => f.includes('.test.')),
      JSON.stringify(defFiles));
    assert('T5e. production names-only pre-write still avoids base triage/topology',
      !existsSync(path.join(out, 'triage.json')) &&
      !existsSync(path.join(out, 'topology.json')),
...

{
  ...
  try {
    write(fx, 'package.json', JSON.stringify({ name: 'au-pre-rust-inventory', type: 'module' }));
    ...
    const intent = { language: 'rust', files: ['src/lib.rs'], names: [], shapes: [], dependencies: [], plannedTypeEscapes: [] };
    const intentPath = path.join(out, 'intent.json');
    writeFileSync(intentPath, JSON.stringify(intent));
    ...
    assert('T6. rust pre-write exits 0 with fake analyzer',
      res.status === 0, `status=${res.status}, stderr=${res.stderr.slice(0, 500)}`);
    const advisory = JSON.parse(readFileSync(path.join(out, 'pre-write-advisory.latest.json'), 'utf8'));
    assert('T6b. rust pre-write inventory includes existing Rust file',
      advisory.preWrite?.fileInventory?.files?.includes('src/lib.rs'),
      JSON.stringify(advisory.preWrite?.fileInventory));
    assert('T6c. rust pre-write inventory keeps JS/TS files for mixed repo deltas',
