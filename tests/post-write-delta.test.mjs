...
let keySeq = 0;
function freshKey() {
  return `sha256:${String(++keySeq).padStart(64, "0")}`;
}
...

{
  ...
  for (const pat of forbidden) {
    assert(
      `T_PURE_3. post-write-delta.mjs does not use ${pat}`,
      !src.includes(pat),
...

{
  ...
  assert(
    "T_BASE_1a. baseline available, unchanged → all pre-existing",
    d.entries.length === 1 && d.entries[0].label === "pre-existing",
  );
  assert(
    "T_BASE_1b. baseline.status === available",
    d.baseline.status === "available",
  );
  assert(
    "T_BASE_1c. summary.preExisting === 1, silentNew === 0",
...

{
  ...
  const after = makeInventory({
    typeEscapes: [makeEscape({ occurrenceKey: "sha256:NEW" })],
  });
  const d = run({ advisory, before, after });
  assert(
    "T_BASE_2. baseline available, new as-any → silent-new",
...

{
  ...
  const d = run({ advisory, before, after });
  assert(
    "T_BASE_2b. duplicate occurrenceKey is compared as a multiset: one pre-existing and one silent-new",
    d.summary.preExisting === 1 && d.summary.silentNew === 1,
    JSON.stringify(d.summary),
  );
  assert(
    "T_BASE_2c. requiredAcknowledgements includes the duplicate-key silent-new",
    requiredAcknowledgements(d).length === 1 &&
      requiredAcknowledgements(d)[0].occurrenceKey === dupKey,
  );
  assert(
    "T_BASE_2d. duplicate-key silent-new carries localization ambiguity diagnostic",
    (requiredAcknowledgements(d)[0].diagnostics ?? []).includes(
...

{
  const advisory = makeAdvisory();
  const before = makeInventory({
    typeEscapes: [makeEscape({ occurrenceKey: "sha256:OLD" })],
  });
  const after = makeInventory({ typeEscapes: [] });
  const d = run({ advisory, before, after });
  const removed = d.entries.filter((e) => e.label === "removed");
  assert(
    "T_BASE_3. baseline available, gone from after → removed",
    removed.length === 1,
  );
}

{
  ...
  const after = makeInventory({
    typeEscapes: [makeEscape({ occurrenceKey: "sha256:O1" })],
  });
  const d = run({ advisory, before: null, after });
  assert(
    "T_BASE_4a. baseline missing → all observed-unbaselined",
    d.entries.every((e) => e.label === "observed-unbaselined"),
  );
  assert(
    "T_BASE_4b. baseline missing → no silent-new emitted",
    d.summary.silentNew === 0,
  );
  assert(
    "T_BASE_4c. baseline missing → no removed emitted",
    d.summary.removed === 0,
  );
}

{
  ...
  const d = run({ advisory, before, after });
  assert(
    "T_BASE_5. baseline unusable (supports.typeEscapes !== true) → baseline.status=missing",
...

{
  ...
  const before = makeInventory({
    complete: false,
    filesWithParseErrors: [
      { file: "src/foo.ts", message: "Unexpected token", line: 1 },
    ],
  ...
  assert(
    "T_BASE_INCOMPLETE_1a. after-occ in before-parse-error file → observed-unbaselined (NOT silent-new)",
    match?.label === "observed-unbaselined",
    `got label: ${match?.label}`,
  );
  assert(
    'T_BASE_INCOMPLETE_1b. carry diagnostic "before-file-parse-error" attached',
    (match?.diagnostics ?? []).includes("before-file-parse-error"),
  );
  assert(
    "T_BASE_INCOMPLETE_1c. summary.silentNew === 0 (the potential false silent-new was downgraded)",
    d.summary.silentNew === 0,
  );
  assert(
    "T_BASE_INCOMPLETE_1d. requiredAcknowledgements empty (no silent-new emission)",
    requiredAcknowledgements(d).length === 0,
  );
  assert(
    'T_BASE_INCOMPLETE_1e. baseline.status still "available" — a partial before is still a baseline',
    d.baseline.status === "available",
  );
}

{
  ...
  const fooEntry = d.entries.find(
    (e) => e.occurrenceKey === "sha256:FOO-IN-ERR",
  );
  const barEntry = d.entries.find(
    (e) => e.occurrenceKey === "sha256:BAR-CLEAN",
  );
  assert(
    "T_BASE_INCOMPLETE_2a. parse-error file → observed-unbaselined",
    fooEntry?.label === "observed-unbaselined",
  );
  assert(
    "T_BASE_INCOMPLETE_2b. clean-parse file → silent-new (narrowly scoped downgrade)",
    barEntry?.label === "silent-new",
  );
  assert(
    "T_BASE_INCOMPLETE_2c. requiredAcknowledgements has only the clean-file silent-new",
    requiredAcknowledgements(d).length === 1 &&
...

{
  ...
  const after = makeInventory({
    typeEscapes: [
      makeEscape({
        insideExportedIdentity: "src/a.ts::foo",
        occurrenceKey: "sha256:P1",
      }),
    ],
  });
  const d = run({
    advisory,
    before: makeInventory({ typeEscapes: [] }),
    after,
  });
  assert(
    "T_PLAN_1. planned at insideExportedIdentity matches → planned",
...

{
  const advisory = makeAdvisory({
    plannedTypeEscapes: [
      { escapeKind: "as-any", locationHint: "unknown", reason: "r" },
    ],
  });
  const after = makeInventory({
    typeEscapes: [makeEscape({ occurrenceKey: "sha256:P2" })],
  });
  const d = run({
    advisory,
    before: makeInventory({ typeEscapes: [] }),
  ...
  assert(
    'T_PLAN_2. planned with "unknown" + 1 candidate → planned',
...

{
  const advisory = makeAdvisory({
    plannedTypeEscapes: [
      { escapeKind: "as-any", locationHint: "src/vendor/", reason: "r" },
    ],
  });
  const after = makeInventory({
    typeEscapes: [
  ...
  assert(
    'T_PLAN_3. planned at file-prefix "src/vendor/" + file in that dir → planned',
    d.entries[0].label === "planned",
  );
}

{
  const advisory = makeAdvisory({
    plannedTypeEscapes: [
      { escapeKind: "as-any", locationHint: "src/foo", reason: "r" },
    ],
  });
  const after = makeInventory({
    typeEscapes: [
  ...
  assert(
    'T_PLAN_4a. planned at "src/foo" (no trailing /) does NOT match src/foobar.ts',
    !d.entries.some((e) => e.label === "planned"),
  );
  assert(
    "T_PLAN_4b. unmatched after escape falls to silent-new (baseline present, absent from before)",
...

{
  const advisory = makeAdvisory({
    plannedTypeEscapes: [
      { escapeKind: "ts-ignore", locationHint: "src/missing.ts", reason: "r" },
    ],
  });
  const after = makeInventory({ typeEscapes: [] });
  const d = run({
  ...
  assert(
    "T_PLAN_5. planned with no candidate → planned-not-observed",
    d.entries[0].label === "planned-not-observed" &&
      d.entries[0].occurrenceKey === null,
  );
}
...

{
  ...
  assert(
    "T_ABS_1a. baseline ok + unknown + 2 candidates → planned picks absent-from-before",
    planned?.occurrenceKey === "sha256:NEW",
    `got: ${planned?.occurrenceKey}`,
  );
  assert(
    "T_ABS_1b. the pre-existing candidate classifies as pre-existing",
    d.entries.some(
      (e) => e.label === "pre-existing" && e.occurrenceKey === "sha256:PRE",
    ),
  );
}

{
    ...
    plannedTypeEscapes: [
      {
        ...
        reason: "intentional temporary shim",
      },
    ],
  });
  ...
  const planned = d.entries.find((e) => e.label === "planned");
  assert(
    "T_ABS_1c. planned matching treats duplicate occurrenceKey as separate instances",
    planned?.line === 20 &&
      d.summary.preExisting === 1 &&
      d.summary.silentNew === 0,
...

{
  ...
  const before = makeInventory({
    includeTests: false,
    typeEscapes: [preExisting],
  });
  const after = makeInventory({
    includeTests: true,
    typeEscapes: [preExisting, newOne],
  });
  const d = run({ advisory, before, after });
  assert(
    "T_ABS_2a. scan-range mismatch → scanRangeParity: mismatch",
    d.scanRangeParity.status === "mismatch",
  );
  // Under mismatch remainder → observed-unbaselined (not silent-new).
  assert(
    "T_ABS_2b. under mismatch: unmatched remainders degrade to observed-unbaselined",
    d.entries.filter((e) => e.label === "observed-unbaselined").length >= 1,
  );
  assert(
    "T_ABS_2c. under mismatch: removed NOT computed",
    d.summary.removed === 0,
  );
  assert(
    "T_ABS_2d. under mismatch: requiredAcknowledgements empty",
    requiredAcknowledgements(d).length === 0,
  );
}

{
  ...
  assert(
    "T_ABS_3a. all candidates pre-existing → planned picks first (deterministic)",
    plannedEntry?.occurrenceKey === "sha256:PE1",
  );
  assert(
    "T_ABS_3b. the other pre-existing remains pre-existing",
    d.entries.some(
      (e) => e.label === "pre-existing" && e.occurrenceKey === "sha256:PE2",
    ),
  );
}
...

{
  ...
  assert(
    "T_AMB_1a. remainder in baseline → pre-existing (NOT silent-new)",
    preExist.length === 1,
  );
  assert(
    "T_AMB_1b. remainder carries ambiguous-planned-match diagnostic",
    preExist[0].diagnostics.includes("ambiguous-planned-match"),
  );
  assert(
    "T_AMB_1c. requiredAcknowledgements empty (remainder was pre-existing)",
    requiredAcknowledgements(d).length === 0,
  );
}

{
  ...
  assert(
    "T_AMB_2a. remainder NOT in before → silent-new",
    silentNew !== undefined,
  );
  assert(
    "T_AMB_2b. silent-new remainder carries ambiguous-planned-match diagnostic",
    silentNew?.diagnostics.includes("ambiguous-planned-match"),
  );
  assert(
    "T_AMB_2c. requiredAcknowledgements includes silent-new entry",
...

{
  ...
  assert(
    "T_AMB_3a. baseline missing → remainder → observed-unbaselined",
    obs !== undefined,
  );
  assert(
    "T_AMB_3b. observed-unbaselined remainder carries ambiguous-planned-match diagnostic",
    obs?.diagnostics.includes("ambiguous-planned-match"),
  );
  assert(
    "T_AMB_3c. requiredAcknowledgements empty (baseline-missing)",
    requiredAcknowledgements(d).length === 0,
  );
}
...

{
  ...
  const planned = d.entries.find((e) => e.label === "planned");
  assert(
    "T_SHAPE_1. codeShape tiebreak picks the string-literal-preserving match",
...

{
  ...
  assert(
    "T_CAP_1a. afterInventory null → capabilityParity.status: missing",
    d.capabilityParity.status === "missing",
  );
  assert("T_CAP_1b. entries: []", d.entries.length === 0);
  assert(
    "T_CAP_1c. capabilityFailures has after-inventory-missing",
    d.capabilityFailures.some((f) => f.kind === "after-inventory-missing"),
  );
  assert(
    "T_CAP_1d. requiredAcknowledgements empty",
    requiredAcknowledgements(d).length === 0,
  );
}

{
  ...
  const d = run({ advisory, before: null, after });
  assert(
    "T_CAP_2a. afterInventory supports.typeEscapes !== true → mismatch",
    d.capabilityParity.status === "mismatch",
  );
  assert("T_CAP_2b. entries: []", d.entries.length === 0);
}

{
  ...
  const d = run({ advisory, before: null, after });
  assert(
    "T_CAP_3. escapeKinds drift → mismatch",
    d.capabilityParity.status === "mismatch",
  );
}

{
  ...
  for (const caps of [undefined, null, {}, { typeEscapes: false }]) {
    ...
    const after = makeInventory({
      typeEscapes: [
        makeEscape({ occurrenceKey: "sha256:CAP4-" + String(caps) }),
      ],
    });
    const d = run({
      advisory,
    ...
    assert(
      `T_CAP_4. advisory.capabilities=${JSON.stringify(caps)} does NOT block classification`,
...
{
  const planned = makeEscape({
    insideExportedIdentity: "src/a.ts::foo",
    occurrenceKey: "sha256:SR1-P",
  });
  const unplanned = makeEscape({ line: 99, occurrenceKey: "sha256:SR1-U" });
  const advisory = makeAdvisory({
    plannedTypeEscapes: [
      { escapeKind: "as-any", locationHint: "src/a.ts::foo", reason: "r" },
    ],
  });
  const before = makeInventory({ includeTests: false, typeEscapes: [] });
  const after = makeInventory({
    includeTests: true,
    typeEscapes: [planned, unplanned],
  });
  const d = run({ advisory, before, after });
  assert(
    "T_SR_1a. mismatch: planned STILL emitted",
    d.entries.some(
      (e) => e.label === "planned" && e.occurrenceKey === "sha256:SR1-P",
    ),
  );
  assert(
    "T_SR_1b. mismatch: unplanned → observed-unbaselined",
    d.entries.some(
      (e) =>
        e.label === "observed-unbaselined" &&
        e.occurrenceKey === "sha256:SR1-U",
    ),
  );
  assert("T_SR_1c. mismatch: removed NOT computed", d.summary.removed === 0);
  assert(
    "T_SR_1d. mismatch: requiredAcknowledgements empty",
    requiredAcknowledgements(d).length === 0,
  );
}

{
  ...
  const d = run({ advisory, before, after });
  assert(
    "T_SR_2. exclude order differs but sorted equal → scanRangeParity: ok",
...

{
  ...
  const d = run({ advisory, before: null, after });
  assert(
    "T_COMP_1a. afterComplete === true",
    d.inventoryCompleteness.afterComplete === true,
  );
  assert(
    "T_COMP_1b. filesWithParseErrors empty",
...

{
  ...
  const d = run({ advisory, before: null, after });
  assert(
    "T_COMP_2a. afterComplete === false",
    d.inventoryCompleteness.afterComplete === false,
  );
  assert(
    'T_COMP_2b. filesWithParseErrors has entry with side: "after"',
    d.inventoryCompleteness.filesWithParseErrors.some(
      (e) => e.side === "after" && e.file === "src/bad.ts",
    ),
  );
}

{
  ...
  const d = run({ advisory, before, after });
  assert(
    "T_COMP_3a. beforeComplete === false (baseline available)",
    d.inventoryCompleteness.beforeComplete === false,
  );
  assert(
    'T_COMP_3b. filesWithParseErrors has entry with side: "before"',
    d.inventoryCompleteness.filesWithParseErrors.some(
      (e) => e.side === "before",
    ),
  );
...

{
  ...
  const d = run({ advisory, before: null, after });
  assert(
    "T_COMP_4a. baseline missing → beforeComplete: null",
    d.inventoryCompleteness.beforeComplete === null,
  );
  assert(
    "T_COMP_4b. no before entries in filesWithParseErrors",
    !d.inventoryCompleteness.filesWithParseErrors.some(
      (e) => e.side === "before",
    ),
  );
...

{
  ...
  assert(
    "T_REQ_1a. returns exactly 1 entry from a 6-label delta",
    req.length === 1,
  );
  assert(
    "T_REQ_1b. returned entry is the silent-new one",
    req[0].label === "silent-new",
  );
  assert(
    "T_REQ_2. observed-unbaselined NEVER in output",
    !req.some((e) => e.label === "observed-unbaselined"),
  );
  assert(
    "T_REQ_3. planned-not-observed NEVER in output",
  ...
  assert(
    "T_REQ_4. silent-new + ambiguous-planned-match IS included",
...
{
  assert(
    "T_EXPORT_1. CANONICAL_ESCAPE_KINDS matches canonical list",
    Array.isArray(CANONICAL_ESCAPE_KINDS) &&
      CANONICAL_ESCAPE_KINDS.length === 11 &&
      CANONICAL_ESCAPE_KINDS.every((k, i) => k === CANON[i]),
  );
  assert(
    "T_EXPORT_2. inventoryUsable exported",
    typeof inventoryUsable === "function",
  );
  assert(
    "T_EXPORT_3. inventoryUsable(null) === false",
    inventoryUsable(null) === false,
  );
  assert(
    "T_EXPORT_4. inventoryUsable(valid) === true",
