...
assert('T1. canonical §3 LOW_INFO_NAMES block parsed',
  Array.isArray(parsedLowInfo) && parsedLowInfo.length > 0);

assert('T2. canonical LOW_INFO_NAMES has exactly 16 entries',
  parsedLowInfo && parsedLowInfo.length === 16,
  `got ${parsedLowInfo?.length}`);

assert('T3. canonical LOW_INFO_NAMES matches expected list AND order',
  parsedLowInfo && JSON.stringify(parsedLowInfo) === JSON.stringify(EXPECTED_LOW_INFO_NAMES),
...
assert('T4. canonical §9 label enumeration parses to exactly 9 labels',
  parsedLabels && parsedLabels.length === 9,
  `got ${parsedLabels?.length}: ${JSON.stringify(parsedLabels)}`);

assert('T5. canonical §9 label set equals expected 9-label set',
  parsedLabels &&
...
assert('T6. _lib/canon-draft.mjs exports LOW_INFO_NAMES',
  Array.isArray(canonDraft.LOW_INFO_NAMES));

assert('T7. LOW_INFO_NAMES mirror is frozen (Object.freeze)',
  Object.isFrozen(canonDraft.LOW_INFO_NAMES));

assert('T8. LOW_INFO_NAMES mirror is byte-equal to canonical §3 (names + order)',
  JSON.stringify([...canonDraft.LOW_INFO_NAMES]) === JSON.stringify(EXPECTED_LOW_INFO_NAMES),
  `mirror=${JSON.stringify(canonDraft.LOW_INFO_NAMES)}`);

assert('T9. classifyTypeNameGroup exported',
  typeof canonDraft.classifyTypeNameGroup === 'function');

assert('T10. classifySingleIdentity exported',
...

{
  ...
  emitted.add(canonDraft.classifyTypeNameGroup({
    name: 'Foo',
    identities: [id1, id2],
  ...
  emitted.add(canonDraft.classifySingleIdentity({
    identity: id1, fanIn: 5, kind: 'TSTypeAliasDeclaration',
    contamination: { label: 'severely-any-contaminated' },
  }).label);
  emitted.add(canonDraft.classifySingleIdentity({
    identity: id1, fanIn: 2, kind: 'TSTypeAliasDeclaration',
    contamination: null,
  }).label);  // name-len check needs single-char name:
  emitted.add(canonDraft.classifySingleIdentity({
    identity: 'a.ts::X', fanIn: 2, kind: 'TSTypeAliasDeclaration',
    contamination: null,
  }).label);
  emitted.add(canonDraft.classifySingleIdentity({
    identity: id1, fanIn: 5, kind: 'TSInterfaceDeclaration',
    contamination: null,
  }).label);
  emitted.add(canonDraft.classifySingleIdentity({
    identity: id1, fanIn: 2, kind: 'TSInterfaceDeclaration',
    contamination: null,
  }).label);
  emitted.add(canonDraft.classifySingleIdentity({
    identity: id1, fanIn: 0, kind: 'TSInterfaceDeclaration',
    contamination: null,
  }).label);

  const strayLabels = [...emitted].filter((l) => !expected.has(l));
  assert('T11. emitted classifier labels ⊆ canonical 9-label set',
...

{
  ...
  assert('T12. _lib/canon-draft.mjs does NOT key identity/Map/Set on typeName',
    flagged.length === 0,
...
assert('T13. canonical/classification-gates.md §9 mentions all 9 labels with backticks',
...
assert('H1. canonical §10.4 LOW_INFO_HELPER_NAMES block parsed',
  Array.isArray(parsedHelperLowInfo) && parsedHelperLowInfo.length > 0);

assert('H2. canonical LOW_INFO_HELPER_NAMES matches expected 15-name list AND order',
  parsedHelperLowInfo &&
  JSON.stringify(parsedHelperLowInfo) === JSON.stringify(EXPECTED_LOW_INFO_HELPER_NAMES),
...
assert('H3. canonical §9 helper-label enumeration parses to exactly 9 labels',
  parsedHelperLabels && parsedHelperLabels.length === 9,
  `got ${parsedHelperLabels?.length}: ${JSON.stringify(parsedHelperLabels)}`);

assert('H4. canonical §9 helper-label set equals expected 9-label set',
  parsedHelperLabels &&
...
assert('H5. _lib/canon-draft.mjs exports LOW_INFO_HELPER_NAMES',
  Array.isArray(canonDraft.LOW_INFO_HELPER_NAMES));

assert('H6. LOW_INFO_HELPER_NAMES mirror is frozen (Object.freeze)',
  Object.isFrozen(canonDraft.LOW_INFO_HELPER_NAMES));

assert('H7. LOW_INFO_HELPER_NAMES mirror is byte-equal to canonical §10.4 (names + order)',
  canonDraft.LOW_INFO_HELPER_NAMES &&
  JSON.stringify([...canonDraft.LOW_INFO_HELPER_NAMES]) === JSON.stringify(EXPECTED_LOW_INFO_HELPER_NAMES),
  `mirror=${JSON.stringify(canonDraft.LOW_INFO_HELPER_NAMES)}`);

assert('H8. classifyHelperGroup exported',
  typeof canonDraft.classifyHelperGroup === 'function');

assert('H9. classifyHelperIdentity exported',
...

{
  ...
  emitted.add(canonDraft.classifyHelperGroup({
    name: 'parseJson',
    identities: [id1, id2],
    fanInByIdentity: { [id1]: 5, [id2]: 5 },
    contaminationByIdentity: {
  ...
  emitted.add(canonDraft.classifyHelperIdentity({
    identity: id1, fanIn: 5, kind: 'FunctionDeclaration',
    contamination: { label: 'severely-any-contaminated' },
  }).label);

  // Single Rule 1: low-signal-helper-name (name ∈ low-info helper names, fanIn < 3)
  emitted.add(canonDraft.classifyHelperIdentity({
    identity: 'a.ts::get', fanIn: 2, kind: 'FunctionDeclaration',
    contamination: null, exportedName: 'get',
  }).label);

  // Single Rule 2: central-helper (fanIn ≥ 3)
  emitted.add(canonDraft.classifyHelperIdentity({
    identity: id1, fanIn: 5, kind: 'FunctionDeclaration',
    contamination: null, exportedName: 'parseJson',
  }).label);

  // Single Rule 3: shared-helper (fanIn 1 or 2)
  emitted.add(canonDraft.classifyHelperIdentity({
    identity: id1, fanIn: 2, kind: 'FunctionDeclaration',
    contamination: null, exportedName: 'parseJson',
  }).label);

  // Single Rule 4: zero-internal-fan-in-helper
  emitted.add(canonDraft.classifyHelperIdentity({
    identity: id1, fanIn: 0, kind: 'FunctionDeclaration',
    contamination: null, exportedName: 'parseJson',
  }).label);

  const strayLabels = [...emitted].filter((l) => !expected.has(l));
  assert('H10. emitted helper-classifier labels ⊆ canonical 9-label helper set',
    strayLabels.length === 0,
...

{
  ...
  assert('H11. group of has-any-only helpers does NOT trigger ANY_COLLISION_HELPER',
  ...
  assert('H12. group of unknown-surface-only helpers does NOT trigger ANY_COLLISION_HELPER',
  ...
  assert('H13. mixed contaminated+clean helper group does NOT trigger ANY_COLLISION_HELPER',
...

{
  ...
  assert('H14. group `parse` (low-info name) + fanIn ≥ 3 → HELPER_DUPLICATE_STRONG (Rule 1 wins over Rule 2)',
...

{
  ...
  const r1 = canonDraft.classifyHelperIdentity({
    identity: 'a.ts::get', fanIn: 3, kind: 'FunctionDeclaration',
    contamination: null, exportedName: 'get',
  });
  assert('H15. single `get` + fanIn 3 → central-helper (Rule 2 fires over Rule 1 at threshold)',
  ...
  assert('H16. single `get` + fanIn 2 → low-signal-helper-name (Rule 1 fires below threshold)',
...

{
  ...
  const kinds = ['FunctionDeclaration', 'ArrowFunctionExpression', 'FunctionExpression'];
  ...
  assert(`H17. severely-any-contaminated-helper never appears across 144 classifier runs when contamination=undefined`,
    severeSeen === 0,
...
{
  const names = ['parseJson', 'get', 'fetchData', 'format', 'doThing', 'a'];
  const fanInPairs = [[5, 5], [3, 0], [1, 2], [0, 0], [10, 10]];
  let collisionSeen = 0;
  for (const name of names) {
  ...
  assert(`H18. ANY_COLLISION_HELPER never appears across 30 group-classifier runs when contaminationByIdentity is empty`,
    collisionSeen === 0,
...

{
  ...
  const flagged = suspect.filter((re) => re.test(stripped));
  assert('H19. _lib/canon-draft.mjs does NOT key identity/Map/Set on helperName or calleeName',
    flagged.length === 0,
    `flagged patterns: ${flagged.map((r) => r.toString()).join(', ')}`);
}
...

{
  ...
  const flagged = suspect.filter((re) => re.test(stripped));
  assert('H20. _lib/canon-draft.mjs does NOT assign topCallees.count to fanIn (PF-4)',
    flagged.length === 0,
    `flagged patterns: ${flagged.map((r) => r.toString()).join(', ')}`);
}
...

{
  ...
  assert('H21. _lib/canon-draft.mjs exports UNCERTAIN_REASONS enum with exactly 4 values',
    Array.isArray(exported) &&
    Object.isFrozen(exported) &&
    JSON.stringify([...exported].sort()) === JSON.stringify([...EXPECTED_REASONS].sort()),
...
assert('TP1. canonical §9 topology-label enumeration parses to exactly 8 labels',
  parsedTopologyLabels && parsedTopologyLabels.length === 8,
  `got ${parsedTopologyLabels?.length}: ${JSON.stringify(parsedTopologyLabels)}`);

assert('TP2. canonical §9 topology-label set equals expected 8-label set',
...
assert('TP3. canonical §11.4 label set block has all 8 expected labels',
  EXPECTED_TOPOLOGY_LABELS.every((l) => new RegExp(`^${l}$`, 'm').test(gatesText)));

assert('TP4. canonical §11 Topology classification section exists',
  /## 11\. Topology classification/.test(gatesText));

// ── TP5–TP9. Code-side mirror + classifier exports ──

assert('TP5. _lib/canon-draft.mjs exports TOPOLOGY_LABELS',
  Array.isArray(canonDraft.TOPOLOGY_LABELS));

assert('TP6. TOPOLOGY_LABELS mirror is frozen',
  Object.isFrozen(canonDraft.TOPOLOGY_LABELS));

assert('TP7. TOPOLOGY_LABELS byte-equal to canonical §11.4 (names + order)',
  canonDraft.TOPOLOGY_LABELS &&
  JSON.stringify([...canonDraft.TOPOLOGY_LABELS]) === JSON.stringify(EXPECTED_TOPOLOGY_LABELS),
  `mirror=${JSON.stringify(canonDraft.TOPOLOGY_LABELS)}`);

assert('TP8. classifyTopologySubmodule exported',
  typeof canonDraft.classifyTopologySubmodule === 'function');

assert('TP9. classifyTopologyScc + classifyTopologyFile exported',
...

{
  ...
  emitted.add(canonDraft.classifyTopologySubmodule({
    name: 'a', inDegree: 10, outDegree: 5, sccMember: true, crossEdgeSource: 'full-list',
  }).label);
  // Rule 1: isolated-submodule (only fires in full-list mode)
  emitted.add(canonDraft.classifyTopologySubmodule({
    name: 'b', inDegree: 0, outDegree: 0, sccMember: false, crossEdgeSource: 'full-list',
  }).label);
  // Rule 2: shared-submodule
  emitted.add(canonDraft.classifyTopologySubmodule({
    name: 'c', inDegree: 7, outDegree: 2, sccMember: false, crossEdgeSource: 'full-list',
  }).label);
  // Rule 3: leaf-submodule
  emitted.add(canonDraft.classifyTopologySubmodule({
    name: 'd', inDegree: 1, outDegree: 5, sccMember: false, crossEdgeSource: 'full-list',
  }).label);
  // Rule 4: scoped-submodule
  emitted.add(canonDraft.classifyTopologySubmodule({
    name: 'e', inDegree: 2, outDegree: 2, sccMember: false, crossEdgeSource: 'full-list',
  }).label);

  // SCC + file classifiers
  emitted.add(canonDraft.classifyTopologyScc({ sccIndex: 0, members: ['a.ts', 'b.ts'] }).label);
  ...
  const strayLabels = [...emitted].filter((l) => !expected.has(l));
  assert('TP10. emitted topology-classifier labels ⊆ canonical 8-label set',
    strayLabels.length === 0,
...

{
  const r = canonDraft.classifyTopologySubmodule({
    name: 'hub-cyclic', inDegree: 20, outDegree: 0, sccMember: true, crossEdgeSource: 'full-list',
  });
  assert('TP11. SCC member with high inDegree → cyclic-submodule (Rule 0 wins over Rule 2)',
    r.label === 'cyclic-submodule', `got=${r.label}`);
}
...

{
  ...
  assert('TP12a. full-list mode + zero in/out → isolated-submodule',
  ...
  assert('TP12b. top-30-only mode + zero in/out → scoped-submodule (NOT isolated; §11.1 Rule 1 guard)',
    rDegraded.label === 'scoped-submodule', `got=${rDegraded.label}`);
}
...

{
  ...
  const exact1000 = canonDraft.classifyTopologyFile({ file: 'b.ts', loc: 1000 });
  const huge = canonDraft.classifyTopologyFile({ file: 'c.ts', loc: 5000 });

  assert('TP13a. loc < 400 → no label (returns null)',
    small === null);
  assert('TP13b. loc === 400 → oversize',
    exact400?.label === 'oversize');
  assert('TP13c. loc === 1000 → extreme-oversize (Rule 0 wins at threshold)',
    exact1000?.label === 'extreme-oversize');
  assert('TP13d. loc >> 1000 → extreme-oversize',
...

{
  ...
  const flagged = suspect.filter((re) => re.test(stripped));
  assert('TP14. _lib/canon-draft.mjs does NOT use file-keyed Map as submodule index',
    flagged.length === 0,
    `flagged patterns: ${flagged.map((r) => r.toString()).join(', ')}`);
}
...

{
  ...
  const flagged = suspect.filter((re) => re.test(stripped));
  assert('TP15. _lib/canon-draft.mjs does NOT derive inDegree/outDegree from crossSubmoduleTop (PF-6)',
    flagged.length === 0,
    `flagged patterns: ${flagged.map((r) => r.toString()).join(', ')}`);
}
...

{
  ...
  assert('TP16a. TOPOLOGY_UNCERTAIN_REASONS exported + frozen',
    Array.isArray(exported) && Object.isFrozen(exported));
  assert('TP16b. TOPOLOGY_UNCERTAIN_REASONS has exactly 3 values matching spec',
    exported &&
    JSON.stringify([...exported].sort()) === JSON.stringify([...EXPECTED_TOPOLOGY_UNCERTAIN_REASONS].sort()),
    `got=${JSON.stringify(exported)}`);
  assert('TP16c. helper UNCERTAIN_REASONS stays at 4 values (NOT widened by topology addition)',
    Array.isArray(canonDraft.UNCERTAIN_REASONS) && canonDraft.UNCERTAIN_REASONS.length === 4);
}
...

{
  const r1 = canonDraft.classifyTopologySubmodule({
    name: 'z', inDegree: 0, outDegree: 0, sccMember: false, crossEdgeSource: 'full-list',
  });
  const r2 = canonDraft.classifyTopologySubmodule({
    name: 'z', inDegree: 0, outDegree: 0, sccMember: false, crossEdgeSource: 'top-30-only',
  });
  assert('TP17. classifier branches on crossEdgeSource string literal (isolated vs scoped)',
...

{
  ...
  if (idx >= 0) {
    ...
    if (fenceStart >= 0) {
      ...
      const fenceEnd = fenceContent.indexOf('```');
      if (fenceEnd >= 0) {
        parsed = fenceContent.slice(0, fenceEnd)
          .split('\n')
          .map((s) => s.trim())
          .filter(Boolean);
      }
  ...
  assert('TP18. canonical §11.4 label-set block parses to exactly 8 entries matching EXPECTED_TOPOLOGY_LABELS',
    parsed &&
...
assert('TN1. canonical §9 naming-label enumeration parses to exactly 10 labels',
  parsedNamingLabels && parsedNamingLabels.length === 10,
  `got ${parsedNamingLabels?.length}: ${JSON.stringify(parsedNamingLabels)}`);

assert('TN2. canonical §9 naming-label set equals expected 10-label set',
  parsedNamingLabels &&
...
assert('TN3. canonical §12 Naming classification section exists',
...
// Parse §12.3 fenced block — all 10 labels one per line.
{
  ...
  assert('TN4. canonical §12.3 label-set block parses to 10 entries matching EXPECTED_NAMING_LABELS',
    parsed &&
    JSON.stringify(parsed) === JSON.stringify(EXPECTED_NAMING_LABELS),
    `parsed=${JSON.stringify(parsed)}`);
}

// ── TN5–TN8. Code-side mirror + classifier exports ──

assert('TN5. _lib/canon-draft.mjs exports NAMING_LABELS',
  Array.isArray(canonDraft.NAMING_LABELS));

assert('TN6. NAMING_LABELS byte-equal to canonical §12.3',
  canonDraft.NAMING_LABELS &&
  Object.isFrozen(canonDraft.NAMING_LABELS) &&
  JSON.stringify([...canonDraft.NAMING_LABELS]) === JSON.stringify(EXPECTED_NAMING_LABELS),
  `mirror=${JSON.stringify(canonDraft.NAMING_LABELS)}`);

assert('TN7. NAMING_CONVENTIONS exported + frozen + matches canonical §12.5 (6 values)',
...
assert('TN8. naming classifier surface — classifyNamingCohort + classifyNamingItem + detectConvention + normalizeFileBasename',
...

{
  ...
  emitted.add(canonDraft.classifyNamingCohort({
    cohortId: 'x', members: [{ name: 'a' }, { name: 'b' }], kind: 'file',
    lowInfoExclusions: new Set(),
  }).label);
  // Rule 1: camelCase-dominant
  emitted.add(canonDraft.classifyNamingCohort({
    cohortId: 'x',
    members: [{ name: 'fooBar' }, { name: 'fooBaz' }, { name: 'fooQux' }, { name: 'fooZap' }, { name: 'FooBar' }],
    kind: 'symbol', lowInfoExclusions: new Set(),
  }).label);
  // Rule 2: mixed-convention
  emitted.add(canonDraft.classifyNamingCohort({
    cohortId: 'x',
    members: [
      { name: 'fooBar' }, { name: 'FooBar' }, { name: 'foo-bar' },
  ...
  emitted.add(canonDraft.classifyNamingItem({
    name: 'get', convention: 'camelCase', dominantConvention: 'camelCase', isLowInfo: true,
  }).label);  // Rule 0: low-info-excluded
  emitted.add(canonDraft.classifyNamingItem({
    name: 'foo', convention: 'camelCase', dominantConvention: null, isLowInfo: false,
  }).label);  // Rule 1: convention-match (no dominant)
  emitted.add(canonDraft.classifyNamingItem({
    name: 'fooBar', convention: 'camelCase', dominantConvention: 'camelCase', isLowInfo: false,
  }).label);  // Rule 2: convention-match
  emitted.add(canonDraft.classifyNamingItem({
    name: 'FooBar', convention: 'PascalCase', dominantConvention: 'camelCase', isLowInfo: false,
  }).label);  // Rule 3: convention-outlier

  const strayLabels = [...emitted].filter((l) => !expected.has(l));
  assert('TN9. emitted naming-classifier labels ⊆ canonical 10-label set',
    strayLabels.length === 0,
...

{
  ...
  const members = [
    ...
    { name: 'get' }, { name: 'set' }, { name: 'parse' },
    { name: 'format' }, { name: 'fetch' },
    { name: 'get' }, { name: 'set' }, { name: 'parse' },
  ];
  const r = canonDraft.classifyNamingCohort({
    cohortId: 'x', members, kind: 'symbol', lowInfoExclusions: lowInfo,
  });
  assert('TN10. raw 10 members with 8 low-info → effective size 2 → insufficient-evidence',
    r.label === 'insufficient-evidence', `got=${r.label}`);
}
...

{
  ...
  assert('TN11a. low-info item + matches dominant → low-info-excluded (Rule 0 wins over Rule 2)',
    r1.label === 'low-info-excluded', `got=${r1.label}`);

  // Item is low-info AND cohort has no dominant → still low-info-excluded
  const r2 = canonDraft.classifyNamingItem({
    name: 'get', convention: 'camelCase', dominantConvention: null, isLowInfo: true,
  });
  assert('TN11b. low-info item + no dominant → low-info-excluded (Rule 0 wins over Rule 1)',
    r2.label === 'low-info-excluded', `got=${r2.label}`);

  // Item is low-info AND differs from dominant → still low-info-excluded
  const r3 = canonDraft.classifyNamingItem({
    name: 'get', convention: 'camelCase', dominantConvention: 'PascalCase', isLowInfo: true,
  });
  assert('TN11c. low-info item + differs from dominant → low-info-excluded (Rule 0 wins over Rule 3)',
    r3.label === 'low-info-excluded', `got=${r3.label}`);
}

// ── TN12. Basename normalization (P0-2) ──

assert('TN12a. `_lib/canon-draft.mjs` normalizes to `canon-draft`',
  canonDraft.normalizeFileBasename('_lib/canon-draft.mjs') === 'canon-draft');
assert('TN12b. `src/UserCard.tsx` normalizes to `UserCard`',
  canonDraft.normalizeFileBasename('src/UserCard.tsx') === 'UserCard');
assert('TN12c. `tests/user-profile.test.tsx` strips .test + .tsx → `user-profile`',
  canonDraft.normalizeFileBasename('tests/user-profile.test.tsx') === 'user-profile');
assert('TN12d. `src/api.d.ts` strips .d + .ts → `api`',
  canonDraft.normalizeFileBasename('src/api.d.ts') === 'api');
assert('TN12e. `src/Foo.stories.tsx` strips .stories + .tsx → `Foo`',
  canonDraft.normalizeFileBasename('src/Foo.stories.tsx') === 'Foo');

// ── TN13. detectConvention sanity ──

assert('TN13a. detectConvention(`fooBar`) === `camelCase`',
  canonDraft.detectConvention('fooBar') === 'camelCase');
assert('TN13b. detectConvention(`FooBar`) === `PascalCase`',
  canonDraft.detectConvention('FooBar') === 'PascalCase');
assert('TN13c. detectConvention(`foo-bar`) === `kebab-case`',
  canonDraft.detectConvention('foo-bar') === 'kebab-case');
assert('TN13d. detectConvention(`foo_bar`) === `snake_case`',
  canonDraft.detectConvention('foo_bar') === 'snake_case');
assert('TN13e. detectConvention(`FOO_BAR`) === `UPPER_SNAKE`',
  canonDraft.detectConvention('FOO_BAR') === 'UPPER_SNAKE');
assert('TN13f. detectConvention(`Foo_bar`) === `mixed`',
  canonDraft.detectConvention('Foo_bar') === 'mixed');

// ── TN14. NAMING_UNCERTAIN_REASONS + CANON_DRAFT_SOURCES ──

assert('TN14a. NAMING_UNCERTAIN_REASONS frozen + matches expected (2 values)',
...
assert('TN14b. `cohort-insufficient-evidence` reason string distinct from `insufficient-evidence` label',
  canonDraft.NAMING_UNCERTAIN_REASONS?.includes('cohort-insufficient-evidence') &&
  !canonDraft.NAMING_UNCERTAIN_REASONS?.includes('insufficient-evidence'));

assert('TN14c. CANON_DRAFT_SOURCES exported + frozen + 4 values',
...

{
  ...
  const flagged = suspect.filter((re) => re.test(stripped));
  assert('TN15. _lib/canon-draft.mjs does NOT key naming cohorts on ownerFile::exportedName',
    flagged.length === 0,
    `flagged patterns: ${flagged.map((r) => r.toString()).join(', ')}`);
}
...

{
  ...
  function scan(dir) {
    ...
    for (const e of entries) {
      ...
      if (e.isDirectory()) {
        ...
        if (rel.startsWith('tests') || rel.startsWith('output') ||
            rel.startsWith('canonical-draft') || rel.startsWith('canonical') ||
            rel.startsWith('node_modules') || rel.startsWith('p1') ||
            rel.startsWith('p2') || rel.startsWith('p3')) continue;
        scan(full);
        ...
        const re = /(?:from\s+|await\s+import\s*\(\s*)['"](?:[^'"]*[\\/])?canon-draft\.mjs['"]/g;
        if (re.test(src)) {
  ...
  for (const sub of searchRoots) {
    ...
    } catch { /* dir may not exist — benign */ }
  }

  assert('FACADE-PIN-1. production code does NOT import from _lib/canon-draft.mjs facade (leaf modules only)',
    offenders.length === 0,
...
try {
  ...
  driftExists = true;
} catch { /* RED until step 1 lands canon-drift.md */ }

assert('DC-1. canonical/canon-drift.md exists (step 1 makes this GREEN)',
  driftExists,
...
function extractDriftCategoryMap(text) {
  ...
  const kindHeaderRe = /^(type-drift|helper-drift|topology-drift|naming-drift)\s*:\s*$/;
  const categoryLineRe = /^\s+([a-z][a-z-]+)\s*\(family:\s*([a-z][a-z-]+)\s*\)\s*$/;

  let currentKind = null;
  for (const line of body.split(/\r?\n/)) {
    const kh = line.trim().match(kindHeaderRe);
    if (kh) { currentKind = kh[1]; continue; }
...
assert('DC-9. §2 names every one of the 4 drift kinds',
...
assert('DC-10. §3 category enum count equals 20 (4 + 5 + 6 + 5)',
  Object.keys(parsedMap).length === 20,
  `parsed ${Object.keys(parsedMap).length} categories; expected 20`);

assert('DC-11. §3 per-kind category enum matches expected',
...
assert('DC-12. §3 family tag set equals exactly 5 canonical values',
  parsed && parsed.allFamilies.size === 5 &&
    EXPECTED_FAMILY_TAGS.every((f) => parsed.allFamilies.has(f)),
  `got families: ${parsed ? [...parsed.allFamilies].sort().join(',') : '(none)'}`);

assert('DC-13. §3 category → family mapping is 1:1 per canonical table',
...
assert('DC-14. §4 identity contract mentions each of the 4 kinds',
...
assert('DC-15. §5 parser contract names all 4 source files',
...
assert('DC-16. §6 JSON shape lists all 5 perSource.status domain values',
  EXPECTED_PERSOURCE_STATUS_DOMAIN.every((v) => section6Slice.includes('`' + v + '`')),
  `missing status values in §6: ${EXPECTED_PERSOURCE_STATUS_DOMAIN.filter((v) => !section6Slice.includes('`' + v + '`')).join(', ')}`);

assert('DC-17. §6 JSON shape shows drifts[] carrying {kind, category, family, identity}',
  ['"kind"', '"category"', '"family"', '"identity"'].every((k) => section6Slice.includes(k)),
  'expected JSON example keys kind/category/family/identity in §6');

assert('DC-18. §5 parser contract cross-refs classification-gates.md label sets',
