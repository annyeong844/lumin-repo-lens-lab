...
const findingC = {
  file: 'src/foo.ts', line: 10, symbol: 'Bar', kind: 'FunctionDeclaration', bucket: 'C',
};
const findingA = { ...findingC, bucket: 'A', fileInternalUses: 1 };
const findingB = { ...findingC, bucket: 'B', predicatePartner: { file: 'x', line: 1, symbol: 'y' } };
...
const findingSpecSafe = { ...findingSpec, safeAction: safeAction('remove_export_specifier') };

// R1: full evidence convergence → SAFE_FIX
{
  const { tier, reason } = tierForFinding(findingCSafe, {
    runtime: { status: 'dead-confirmed', grounding: 'grounded', confidence: 'high' },
    staleness: { tier: 'fossil', grounding: 'grounded', lineLastTouchedDaysAgo: 900 },
    policy: { excluded: false },
  });
  assert('R1. C + runtime-dead + fossil → SAFE_FIX',
    tier === 'SAFE_FIX',
    `got ${tier} (${reason})`);
}
...
// R2: runtime executed → DEGRADED (must override anything else)
{
  const { tier } = tierForFinding(findingCSafe, {
    runtime: { status: 'executed', grounding: 'grounded', confidence: 'high', hitsInSymbol: 42 },
    staleness: { tier: 'fossil' },
    policy: { excluded: false },
  });
  assert('R2. runtime executed → DEGRADED (overrides SAFE_FIX)',
    tier === 'DEGRADED',
    `got ${tier}`);
}
...
// R3: policy-excluded → MUTED regardless of other evidence
{
  ...
  assert('R3. policy-excluded → MUTED regardless of evidence',
    tier === 'MUTED',
    `got ${tier}`);
}
...
{
  const { tier, reason } = tierForFinding(findingCSafe, { policy: { excluded: false } });
  assert('R4. C with safeAction and without runtime/staleness → SAFE_FIX under static graph',
...
{
  const { tier, reason } = tierForFinding(findingC, { policy: { excluded: false } });
  assert('R4b. C without safeAction proof → REVIEW_FIX',
...
// R4c: bounded classify outputs must degrade, not review or safe-fix.
{
  const { tier, reason } = tierForFinding({
    ...findingC,
    bucket: 'unprocessed',
    action: 'classification incomplete; rerun with a larger classify time budget',
  }, { policy: { excluded: false } });
  assert('R4c. classify-incomplete bucket → DEGRADED',
...
// R5: resolver blindness >= 15% → DEGRADED (global gate)
{
  ...
  assert('R5. resolver unresolved >= 15% → DEGRADED even with strong local evidence',
    tier === 'DEGRADED',
    `got ${tier}`);
}
...
// not a contradiction. Runtime-executed and local taint remain blockers.
{
  const { tier, reason } = tierForFinding(findingCSafe, {
    runtime: { status: 'dead-confirmed', grounding: 'grounded', confidence: 'high' },
    staleness: { tier: 'recent', grounding: 'grounded', lineLastTouchedDaysAgo: 3 },
    policy: { excluded: false },
  });
  assert('R6. recent edits do not block static SAFE_FIX',
...
// static graph: keep the definition, remove only the unused export.
{
  ...
  assert('R7. A bucket export demotion can be SAFE_FIX when safeAction proof exists',
    tier === 'SAFE_FIX' && reason.includes('safe-action'),
    `got ${tier} (${reason})`);
}
...
// R8: specifier bucket with strong evidence → SAFE_FIX (mechanical)
{
  ...
  assert('R8. specifier + strong evidence → SAFE_FIX',
    tier === 'SAFE_FIX',
    `got ${tier}`);
}
...
// must not block a weaker selected safe action such as demotion.
{
  ...
  assert('R8b. strongerActionBlockers do not block selected safeAction',
    tier === 'SAFE_FIX' && reason.includes('safe-action'),
    `got ${tier} (${reason})`);
}
...
// R8c: selected-action blockers do block SAFE_FIX.
{
  ...
  assert('R8c. actionBlockers force REVIEW_FIX',
...
// reason to demote to review.
{
  ...
  assert('R8d. requiresModuleMarker keeps SAFE_FIX when proof is complete',
    tier === 'SAFE_FIX' && reason.includes('safe-action'),
    `got ${tier} (${reason})`);
}
...
// only. It must not create high confidence by itself.
{
  ...
  assert('R8e. entry-unreachable gives SAFE_FIX medium_with_evidence, not high',
    tier === 'SAFE_FIX' &&
      confidence === 'medium' &&
...
// independent call-graph support can become high.
{
  ...
  assert('R8e2. entry + call graph evidence gives SAFE_FIX high confidence',
    tier === 'SAFE_FIX' &&
      confidence === 'high' &&
...
// R8f: positive evidence cannot override relevant soft taint.
{
  const { tier, reason } = tierForFinding({
    ...findingC,
    supportedBy: [{ kind: 'entry-unreachable' }],
    taintedBy: [{ kind: TAINT.PARSE_ERRORS_ELSEWHERE, file: 'src/parse-error.ts' }],
    safeAction: safeAction('demote_export_declaration'),
  }, { policy: { excluded: false } });
  assert('R8f. soft taint still blocks entry-unreachable SAFE_FIX boost',
...

{
  ...
  assert('R8f2. resolver soft taint reason is not mislabeled as parse errors',
    tier === 'REVIEW_FIX' &&
...

{
  ...
  assert('R8f3. generated artifact soft taint returns structured blocking diagnostics',
    tier === 'REVIEW_FIX' &&
...

{
  ...
  assert('R8f4. generated consumer blind-zone returns structured blocking diagnostics',
    tier === 'REVIEW_FIX' &&
      blockedPromotion === true &&
...
// removes an externally observable export contract.
{
  ...
  assert('R8g. public deep-import risk blocks SAFE_FIX',
...
// R9: B bucket always needs review regardless of evidence
{
  ...
  assert('R9. B bucket (predicate partner) always REVIEW_FIX',
    tier === 'REVIEW_FIX',
    `got ${tier}`);
}

// R10: TIER_TO_SARIF_LEVEL shape
assert('R10. SAFE_FIX → warning; REVIEW_FIX/DEGRADED → note; MUTED → null',
...
// binding/type in place while removing the export edge.
{
  ...
  assert('R11. exported declaration dependency without safe action → REVIEW_FIX',
...

{
  ...
  assert('R11b. local declaration dependency + demote safe action → SAFE_FIX',
    tier === 'SAFE_FIX' && reason.includes('safe-action'),
    `got ${tier} (${reason})`);
}

{
  ...
  assert('R11c. exported declaration dependency still blocks delete action',
    tier === 'REVIEW_FIX' && reason.startsWith('declaration-dependency-not-preserved'),
    `got ${tier} (${reason})`);
}
...
writeFileSync(path.join(OUT, 'dead-classify.json'), JSON.stringify({
  ...
  proposal_C_remove_symbol: [
    { file: 'src/dead.ts', line: 10, symbol: 'Fossil', kind: 'FunctionDeclaration',
      action: '정의 자체 제거 가능.' },
    { file: 'src/recent.ts', line: 5, symbol: 'Active', kind: 'FunctionDeclaration',
      action: '정의 자체 제거 가능.' },
  ],
  proposal_A_demote_to_internal: [],
  proposal_B_review: [
    { file: 'src/public-types.ts', line: 2, symbol: 'PublicDependency', kind: 'TSInterfaceDeclaration',
      action: '파일 내 중심 타입.',
      declarationExportDependency: true,
  ...
  excludedCandidates: [
    { file: 'eslint.config.mjs', line: 1, symbol: 'default', kind: 'default',
      reason: 'config_FP22' },
  ],
}));

writeFileSync(path.join(OUT, 'runtime-evidence.json'), JSON.stringify({
  meta: { tool: 'test' },
  summary: {},
  merged: [
    { file: 'src/dead.ts', line: 10, symbol: 'Fossil', kind: 'FunctionDeclaration',
      runtimeStatus: 'dead-confirmed', grounding: 'grounded', confidence: 'high', hitsInSymbol: 0 },
    { file: 'src/recent.ts', line: 5, symbol: 'Active', kind: 'FunctionDeclaration',
      runtimeStatus: 'dead-confirmed', grounding: 'grounded', confidence: 'high', hitsInSymbol: 0 },
  ],
}));

writeFileSync(path.join(OUT, 'staleness.json'), JSON.stringify({
  meta: {}, summary: {},
  enriched: [
    { file: 'src/dead.ts', line: 10, symbol: 'Fossil',
      stalenessTier: 'fossil', grounding: 'grounded', lineLastTouchedDaysAgo: 900 },
    { file: 'src/recent.ts', line: 5, symbol: 'Active',
      stalenessTier: 'recent', grounding: 'grounded', lineLastTouchedDaysAgo: 3 },
  ],
}));

// Low resolver blindness so the gate doesn't trip the fixture
writeFileSync(path.join(OUT, 'symbols.json'), JSON.stringify({
  meta: {}, totalUsesResolved: 1000, unresolvedUses: 10,
  files: {}, totalDefs: 0, deadTotal: 0, trulyDead: 0,
  deadInProd: 0, deadInTest: 0, topSymbolFanIn: [], deadProdList: [], reExportsByFile: {},
  fanInByIdentity: {
...
writeFileSync(path.join(OUT, 'export-action-safety.json'), JSON.stringify({
...
writeFileSync(path.join(OUT, 'module-reachability.json'), JSON.stringify({
  meta: {
...
writeFileSync(path.join(OUT, 'entry-surface.json'), JSON.stringify({
...
assert('I1. fix-plan.json has 4-tier summary + total (includes MUTED from excludedCandidates)',
...
assert('I1b. excludedCandidates materialize as MUTED findings (not silently dropped)',
...
assert('I2. Fossil (C + runtime-dead + fossil) ranks SAFE_FIX',
  !!fossilEntry,
...
assert('I3. Active (recent staleness) ranks SAFE_FIX under static graph',
...
assert('I4. Active is not review-only when static evidence is clean',
  !activeEntry,
  `reviewFixes: ${JSON.stringify(plan.reviewFixes.map((s) => s.finding.symbol))}`);

// I5: meta records which inputs were present
assert('I5. fix-plan.meta.inputs flags every optional input',
...
assert('I6. resolverBlindness gate = ok on healthy fixture',
...
const publicDependency = plan.safeFixes.find((s) => s.finding.symbol === 'PublicDependency');
assert('I6b. local declaration export dependency + demote action ranks SAFE_FIX',
  !!publicDependency &&
...
assert('I6c. two evidence lenses give unreachable file high confidence',
  fossilEntry?.finding?.supportedBy?.some((s) => s.kind === 'entry-unreachable') &&
...
assert('I6d. reachable file does not get entry-unreachable support',
  !activeInSafe?.finding?.supportedBy?.some((s) => s.kind === 'entry-unreachable') &&
    activeInSafe?.confidence === 'medium' &&
    activeInSafe?.confidenceDetail === undefined,
  JSON.stringify(activeInSafe));

assert('I6e. call graph no observed callers adds independent support',
  fossilEntry?.finding?.supportedBy?.some((s) =>
    s.kind === 'call-graph-no-observed-callers' &&
    s.artifact === 'call-graph.json') &&
    fossilEntry?.reason.includes('no-observed-callers') &&
    !activeInSafe?.finding?.supportedBy?.some((s) => s.kind === 'call-graph-no-observed-callers'),
...
writeFileSync(path.join(OUT_B, 'dead-classify.json'), JSON.stringify({
  summary: { total: 1, category_C: 1 },
  proposal_C_remove_symbol: [{ file: 'src/x.ts', line: 1, symbol: 'Hit', kind: 'FunctionDeclaration', action: '' }],
  proposal_A_demote_to_internal: [], proposal_B_review: [], proposal_remove_export_specifier: [],
}));
writeFileSync(path.join(OUT_B, 'runtime-evidence.json'), JSON.stringify({
  meta: {}, summary: {},
  merged: [{ file: 'src/x.ts', line: 1, symbol: 'Hit', kind: 'FunctionDeclaration',
    runtimeStatus: 'executed', grounding: 'grounded', confidence: 'high', hitsInSymbol: 7 }],
}));
writeFileSync(path.join(OUT_B, 'symbols.json'), JSON.stringify({
  totalUsesResolved: 1000, unresolvedUses: 10,
}));
execSync(`node rank-fixes.mjs --root ${OUT_B} --output ${OUT_B}`,
  { cwd: DIR, stdio: ['ignore', 'pipe', 'pipe'] });
const planB = JSON.parse(readFileSync(path.join(OUT_B, 'fix-plan.json'), 'utf8'));
assert('I7. runtime-hit findings never reach SAFE_FIX (CI-noise prevention)',
...
execSync(`node rank-fixes.mjs --root ${OUT_D} --output ${OUT_D}`,
  { cwd: DIR, stdio: ['ignore', 'pipe', 'pipe'] });
const planD = JSON.parse(readFileSync(path.join(OUT_D, 'fix-plan.json'), 'utf8'));
assert('I7b. rank-fixes keeps public deep-import risk as REVIEW_FIX',
  planD.summary.SAFE_FIX === 0 &&
    ...
    planD.reviewFixes?.[0]?.evidence?.contract?.publicDeepImportRiskDetail?.packageName === 'public-no-exports',
  JSON.stringify({
...
writeFileSync(path.join(OUT_D, 'dead-classify.json'), JSON.stringify({
  summary: { total: 1, category_C: 1 },
  proposal_C_remove_symbol: [
    { file: 'src/dead-truly.ts', line: 1, symbol: 'neverUsed1', kind: 'FunctionDeclaration', action: '' },
  ],
  proposal_A_demote_to_internal: [],
  proposal_B_review: [],
  proposal_remove_export_specifier: [],
...
execSync(`node rank-fixes.mjs --root ${OUT_D} --output ${OUT_D}`,
  { cwd: DIR, stdio: ['ignore', 'pipe', 'pipe'] });
const planD2 = JSON.parse(readFileSync(path.join(OUT_D, 'fix-plan.json'), 'utf8'));
assert('I7c. no package name means no public deep-import risk blanket review',
...
writeFileSync(path.join(OUT_E, 'dead-classify.json'), JSON.stringify({
  summary: { total: 3, category_C: 3 },
  proposal_C_remove_symbol: [
    { file: 'src/components/Button.tsx', line: 1, symbol: 'Button', kind: 'FunctionDeclaration', action: '' },
    { file: 'src/hooks/useThing.ts', line: 1, symbol: 'useThing', kind: 'FunctionDeclaration', action: '' },
    { file: 'src/app/api/users/route.ts', line: 1, symbol: 'default', kind: 'FunctionDeclaration', action: '' },
  ],
  proposal_A_demote_to_internal: [],
  proposal_B_review: [],
  proposal_remove_export_specifier: [],
}));
writeFileSync(path.join(OUT_E, 'symbols.json'), JSON.stringify({
...
writeFileSync(path.join(OUT_E, 'export-action-safety.json'), JSON.stringify({
  meta: { tool: 'export-action-safety.mjs' },
  findings: [
    {
      ...
      symbol: 'Button',
      safeAction: safeAction('demote_export_declaration', {
        target: { definitionId: 'src/components/Button.tsx#FunctionDeclaration:1-40' },
      }),
      actionBlockers: [],
    },
    {
      ...
      symbol: 'useThing',
      safeAction: safeAction('demote_export_declaration', {
        target: { definitionId: 'src/hooks/useThing.ts#FunctionDeclaration:1-40' },
      }),
      actionBlockers: [],
    },
    {
      ...
      symbol: 'default',
      safeAction: safeAction('demote_export_declaration', {
        target: { definitionId: 'src/app/api/users/route.ts#FunctionDeclaration:1-40' },
      }),
      actionBlockers: [],
    },
  ],
}, null, 2));
writeFileSync(path.join(OUT_E, 'call-graph.json'), JSON.stringify({
  meta: {
    tool: 'build-call-graph.mjs',
    supports: {
      callFanInByDefinitionId: true,
...
execSync(`node rank-fixes.mjs --root ${OUT_E} --output ${OUT_E}`,
  { cwd: DIR, stdio: ['ignore', 'pipe', 'pipe'] });
const planE = JSON.parse(readFileSync(path.join(OUT_E, 'fix-plan.json'), 'utf8'));
assert('I7d. framework callback-like exports do not get call-graph support',
  planE.safeFixes?.length === 3 &&
    planE.safeFixes.every((s) =>
      !s.finding.supportedBy?.some((support) => support.kind === 'call-graph-no-observed-callers') &&
...
writeFileSync(path.join(OUT_G, 'dead-classify.json'), JSON.stringify({
  summary: { total: 1, category_C: 1 },
  proposal_C_remove_symbol: [
...
writeFileSync(path.join(OUT_G, 'symbols.json'), JSON.stringify({
  totalUsesResolved: 1000,
  unresolvedUses: 0,
  fanInByIdentity: { 'src/worker.ts::Worker': 0 },
}));
writeFileSync(path.join(OUT_G, 'export-action-safety.json'), JSON.stringify({
  meta: { tool: 'export-action-safety.mjs' },
  findings: [{
...
writeFileSync(path.join(OUT_G, 'call-graph.json'), JSON.stringify({
  meta: {
    tool: 'build-call-graph.mjs',
...
execSync(`node rank-fixes.mjs --root ${OUT_G} --output ${OUT_G}`,
  { cwd: DIR, stdio: ['ignore', 'pipe', 'pipe'] });
const planG = JSON.parse(readFileSync(path.join(OUT_G, 'fix-plan.json'), 'utf8'));
assert('I7d2. call graph support is withheld when bounded member-call stats are absent',
  planG.safeFixes?.length === 1 &&
    !planG.safeFixes[0].finding.supportedBy?.some((support) => support.kind === 'call-graph-no-observed-callers') &&
...
writeFileSync(path.join(OUT_F, 'dead-classify.json'), JSON.stringify({
  summary: { total: 1, category_C: 1 },
  proposal_C_remove_symbol: [
    {
...
writeFileSync(path.join(OUT_F, 'symbols.json'), JSON.stringify({
  totalUsesResolved: 1000,
  unresolvedUses: 0,
}, null, 2));
writeFileSync(path.join(OUT_F, 'export-action-safety.json'), JSON.stringify({
  meta: { tool: 'export-action-safety.mjs' },
  findings: [
    {
...
execSync(`node rank-fixes.mjs --root ${OUT_F} --output ${OUT_F}`,
  { cwd: DIR, stdio: ['ignore', 'pipe', 'pipe'] });
const planF = JSON.parse(readFileSync(path.join(OUT_F, 'fix-plan.json'), 'utf8'));
const generatedReview = planF.reviewFixes?.[0];
assert('I7e. fix-plan review entry carries generated blind-zone blocking diagnostics',
  planF.summary.SAFE_FIX === 0 &&
    planF.summary.REVIEW_FIX === 1 &&
    generatedReview?.blockedPromotion === true &&
    generatedReview?.blockedBy?.[0]?.reason === 'workspace-generated-artifact-missing' &&
...
writeFileSync(path.join(OUT_C, 'dead-classify.json'), JSON.stringify({
  summary: { total: 3, category_C: 3 },
  proposal_C_remove_symbol: [
    { file: 'apps/server/src/repository.ts', line: 10, symbol: 'CreateTurnInput', kind: 'TSTypeAliasDeclaration', action: '' },
    { file: 'apps/server/src/repository.ts', line: 20, symbol: 'ListLibraryDocsOptions', kind: 'TSTypeAliasDeclaration', action: '' },
    { file: 'apps/server/src/other.ts', line: 5, symbol: 'OtherInput', kind: 'TSTypeAliasDeclaration', action: '' },
  ],
  proposal_A_demote_to_internal: [],
  proposal_B_review: [],
  proposal_remove_export_specifier: [],
}));
writeFileSync(path.join(OUT_C, 'symbols.json'), JSON.stringify({
  totalUsesResolved: 1000, unresolvedUses: 0,
}));
writeFileSync(path.join(OUT_C, 'export-action-safety.json'), JSON.stringify({
  meta: { tool: 'export-action-safety.mjs' },
  findings: [
    {
...
execSync(`node rank-fixes.mjs --root ${OUT_C} --output ${OUT_C}`,
  { cwd: DIR, stdio: ['ignore', 'pipe', 'pipe'] });
const planC = JSON.parse(readFileSync(path.join(OUT_C, 'fix-plan.json'), 'utf8'));
const repoGroup = planC.safeFixGroups?.find((g) =>
  g.file === 'apps/server/src/repository.ts' &&
  g.actionKind === 'demote_export_declaration');
assert('I8. fix-plan groups SAFE_FIX by file and safeAction kind',
  ...
  JSON.stringify({ summary: planC.summary, safeFixGroups: planC.safeFixGroups }));
rmSync(OUT_C, { recursive: true, force: true });
rmSync(OUT, { recursive: true, force: true });
