...
function buildFixture(fx) {
  write(
    fx,
    "package.json",
    JSON.stringify({ name: "ar-cc-fx", type: "module" }),
  );
...

{
  ...
  const res = spawnSync(
    NODE,
    [AUDIT_CLI, "--root", fx, "--output", out, "--check-canon"],
    { encoding: "utf8" },
  );
  assert(
    "CK-1a. --check-canon alone → exit 0 (advisory, no canon → all skipped, but ran=true)",
    res.status === 0,
    `exit=${res.status}, stderr=${(res.stderr ?? "").slice(0, 400)}`,
  );
  const m = readManifest(out);
  assert(
    "CK-1b. manifest.checkCanon.requested === true",
    m.checkCanon?.requested === true,
    `block=${JSON.stringify(m.checkCanon)}`,
  );
  assert(
    "CK-1c. manifest.checkCanon has all 4 perSource keys",
    m.checkCanon?.perSource &&
      ["type-ownership", "helper-registry", "topology", "naming"].every(
        (s) => s in m.checkCanon.perSource,
      ),
    `perSource keys=${Object.keys(m.checkCanon?.perSource ?? {})}`,
  );
  assert(
    "CK-1d. summary.sourcesRequested === 4",
    m.checkCanon?.summary?.sourcesRequested === 4,
    `summary=${JSON.stringify(m.checkCanon?.summary)}`,
  );
  assert(
    "CK-1e. summary has sourcesChecked / sourcesSkipped / sourcesFailed / driftCount fields",
  ...
  assert(
    "CK-1f. driftCounts has all 4 keys",
    m.checkCanon?.driftCounts &&
      ["type-ownership", "helper-registry", "topology", "naming"].every(
        (s) => typeof m.checkCanon.driftCounts[s] === "number",
      ),
  ...
  assert(
    "CK-1g. first-read summary surfaces check-canon command result",
    summaryMd.includes("## Command Result") &&
      summaryMd.includes("Check-canon could not compare promoted canon yet"),
    summaryMd,
  );
  assert(
    "CK-1h. console preview surfaces check-canon result before JSON spelunking",
...

{
  ...
  assert(
    "CK-2a. unknown --sources value → orchestrator exit 1",
    res.status === 1,
    `exit=${res.status}`,
  );
  const m = readManifest(out);
  assert(
    "CK-2b. manifest.checkCanon.ran === false",
    m.checkCanon?.ran === false,
    `ran=${m.checkCanon?.ran}`,
  );
  assert(
    "CK-2c. manifest.checkCanon.reason mentions unknown sources",
    typeof m.checkCanon?.reason === "string" &&
      /unknown/i.test(m.checkCanon.reason),
...

{
  ...
  assert(
    "CK-3a. --sources naming valid value → exit 0 advisory",
    res.status === 0,
    `exit=${res.status}, stderr=${(res.stderr ?? "").slice(0, 400)}`,
  );
  const m = readManifest(out);
  assert(
    "CK-3b. requestedSources reflects subset",
  ...
  assert(
    "CK-3c. perSource has only the requested source",
    m.checkCanon?.perSource &&
...

{
  ...
  assert(
    "CK-3d. --source alias scopes check-canon",
    res.status === 0,
    `exit=${res.status}, stderr=${(res.stderr ?? "").slice(0, 400)}`,
  );
  const m = readManifest(out);
  assert(
    "CK-3e. requestedSources reflects --source alias",
    Array.isArray(m.checkCanon?.requestedSources) &&
      m.checkCanon.requestedSources.length === 1 &&
      m.checkCanon.requestedSources[0] === "naming",
...

{
  ...
  assert(
    "CK-4a. all sources missing canon → orchestrator advisory exit 0",
    res.status === 0,
    `exit=${res.status}`,
  );
  const m = readManifest(out);
  assert(
    "CK-4b. every perSource entry has ran=true (structured result, not spawn failure)",
    m.checkCanon?.perSource &&
      Object.values(m.checkCanon.perSource).every((e) => e.ran === true),
    `perSource=${JSON.stringify(m.checkCanon?.perSource)}`,
  );
  assert(
    "CK-4c. every perSource.status === skipped-missing-canon",
    Object.values(m.checkCanon?.perSource ?? {}).every(
      (e) => e.status === "skipped-missing-canon",
    ),
    `statuses=${Object.values(m.checkCanon?.perSource ?? {})
      .map((e) => e.status)
      .join(",")}`,
  );
  assert(
    "CK-4d. every perSource.exitCode === 2",
    Object.values(m.checkCanon?.perSource ?? {}).every((e) => e.exitCode === 2),
    "",
  );
  assert(
    "CK-4e. summary.sourcesSkipped === 4",
...

{
  ...
  assert(
    "CK-5. --strict-check-canon + all sources missing → exit 2 (no checked source)",
    res.status === 2,
    `exit=${res.status}, stderr=${(res.stderr ?? "").slice(0, 400)}`,
  );
  const m = readManifest(out);
  assert(
    "CK-5b. manifest.checkCanon.strict === true",
    m.checkCanon?.strict === true,
    `strict=${m.checkCanon?.strict}`,
  );
}
...

{
  ...
  assert(
    "CK-6. pre-write + post-write + check-canon → exit 2 (pre/post mutex preserved)",
    res.status === 2,
    `exit=${res.status}, stderr=${(res.stderr ?? "").slice(0, 400)}`,
  );
}
...

{
  ...
  assert(
    "CK-7a. --canon-draft + --check-canon → exit 0 advisory",
    res.status === 0,
    `exit=${res.status}, stderr=${(res.stderr ?? "").slice(0, 400)}`,
  );
  const m = readManifest(out);
  assert(
    "CK-7b. both manifest blocks present",
    m.canonDraft?.requested === true && m.checkCanon?.requested === true,
    `canonDraft=${!!m.canonDraft}, checkCanon=${!!m.checkCanon}`,
  );
  assert(
    "CK-7c. check-canon sees no promoted canonical/*.md even though canon-draft wrote canonical-draft/*.md",
...

{
  ...
  assert(
    "CK-8a. type-ownership with empty canon + empty fresh → exit 0",
    res.status === 0,
    `exit=${res.status}, stderr=${(res.stderr ?? "").slice(0, 400)}`,
  );
  const m = readManifest(out);
  ...
  assert(
    'CK-8b. perSource["type-ownership"].status = clean (no drift)',
    entry?.status === "clean" &&
      entry?.exitCode === 0 &&
      entry?.driftCount === 0,
  ...
  assert(
    "CK-8c. summary.sourcesChecked === 1",
...

{
  ...
  assert(
    "CK-9. ran === true when sources all skipped-missing-canon (structured outcomes still count)",
...

{
  ...
  assert(
    "CK-10a. --sources all → exit 0 (advisory, all 4 skipped-missing-canon)",
    res.status === 0,
    `exit=${res.status}, stderr=${(res.stderr ?? "").slice(0, 400)}`,
  );
  const m = readManifest(out);
  assert(
    "CK-10b. manifest.checkCanon.ran === true",
    m.checkCanon?.ran === true,
    `ran=${m.checkCanon?.ran}`,
  );
  assert(
    "CK-10c. requestedSources expanded to 4 named sources",
    Array.isArray(m.checkCanon?.requestedSources) &&
      m.checkCanon.requestedSources.length === 4 &&
      ["type-ownership", "helper-registry", "topology", "naming"].every((s) =>
        m.checkCanon.requestedSources.includes(s),
      ),
    `requestedSources=${JSON.stringify(m.checkCanon?.requestedSources)}`,
  );
  assert(
    "CK-10d. perSource has all 4 entries",
    m.checkCanon?.perSource && Object.keys(m.checkCanon.perSource).length === 4,
    "",
  );
  // NOT a false "unknown --sources" rejection.
  assert(
    'CK-10e. no "unknown --sources" reason recorded',
    !m.checkCanon?.reason || !/unknown/i.test(m.checkCanon.reason),
    "",
  );
  assert(
    "CK-10f. all requested sources use single check-canon child invocation",
...
// CK-11: --sources "all,naming" dedupes
{
  ...
  assert(
    'CK-11a. --sources "all,naming" → still 4 named sources (dedupe)',
    res.status === 0,
    `exit=${res.status}`,
  );
  const m = readManifest(out);
  assert(
    "CK-11b. requestedSources deduped to 4",
    m.checkCanon?.requestedSources?.length === 4,
    `length=${m.checkCanon?.requestedSources?.length}`,
  );
  assert(
    "CK-11c. deduped all-source request stays single-invocation",
    m.checkCanon?.executionMode === "single-invocation-all" &&
      m.checkCanon?.childInvocations === 1,
    `executionMode=${m.checkCanon?.executionMode}, childInvocations=${m.checkCanon?.childInvocations}`,
  );
...

{
  ...
  const helper = readFileSync(
    path.join(DIR, "_lib", "audit-check-canon.mjs"),
    "utf8",
  );
  const stripped = src
    .replace(/\/\*[\s\S]*?\*\//g, "")
  ...
  assert(
    "CK-12a. audit-repo.mjs delegates check-canon lifecycle to helper",
    /runCheckCanonLifecycle/.test(stripped) &&
      /audit-check-canon\.mjs/.test(stripped),
  );
  assert(
    "CK-12b. audit-check-canon.mjs owns CHECK_CANON_SOURCES",
