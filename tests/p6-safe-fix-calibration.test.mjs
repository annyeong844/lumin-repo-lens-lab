...
function bySymbol(list, symbol) {
  return list.find(
    (entry) => entry.symbol === symbol || entry.finding?.symbol === symbol,
  );
}
...
try {
  ...
  write(
    root,
    "src/executed-runtime.ts",
    `export function runtimeSeen() {\n` + `  return 'dynamic-use';\n` + `}\n`,
  );
  write(
    root,
    "src/uncovered-runtime.ts",
  ...
  assert(
    "P6S-1a. calibration corpus emits the runtime-dead candidate",
    !!stale,
    `deadList=${JSON.stringify(deadList)}`,
  );
  assert(
    "P6S-1b. calibration corpus emits the runtime-hit candidate",
    !!seen,
    `deadList=${JSON.stringify(deadList)}`,
  );
  assert(
    "P6S-1c. calibration corpus emits the uncovered candidate",
    !!uncovered,
    `deadList=${JSON.stringify(deadList)}`,
  );
  assert(
    "P6S-1d. calibration corpus emits the type-only candidate",
  ...
  assert(
    "P6S-2a. covered zero-hit runtime symbol is dead-confirmed",
    rtStale?.runtimeStatus === "dead-confirmed" &&
      rtStale.grounding === "grounded",
    JSON.stringify(rtStale),
  );
  assert(
    "P6S-2b. runtime-hit static-dead symbol is marked executed",
    rtSeen?.runtimeStatus === "executed" && rtSeen.hitsInSymbol === 3,
    JSON.stringify(rtSeen),
  );
  assert(
    "P6S-2c. file absent from coverage is uncovered, not dead-confirmed",
    rtUncovered?.runtimeStatus === "uncovered",
    JSON.stringify(rtUncovered),
  );
  assert(
    "P6S-2d. erased interface is type-only runtime evidence",
    rtTypeOnly?.runtimeStatus === "type-only",
    JSON.stringify(rtTypeOnly),
  );

  const plan = readJson(path.join(out, "fix-plan.json"));
  const safe = plan.safeFixes.find(
    (entry) => entry.finding.symbol === "staleRuntimeDead",
  );
  const degraded = plan.degraded.find(
    (entry) => entry.finding.symbol === "runtimeSeen",
  );
  const reviewUncovered = plan.reviewFixes.find(
    (entry) => entry.finding.symbol === "uncoveredRuntime",
  );
  const reviewType = plan.reviewFixes.find(
    (entry) => entry.finding.symbol === "ColdTypeOnly",
  );

  assert(
    "P6S-3a. SAFE_FIX path is reachable with AST + runtime + stale evidence",
    plan.summary.SAFE_FIX === 1 &&
      safe?.reason.includes("runtime-dead-confirmed"),
    `summary=${JSON.stringify(plan.summary)} safe=${JSON.stringify(safe)}`,
  );
  assert(
    "P6S-3b. runtime-hit contradiction is DEGRADED, never SAFE_FIX",
    plan.summary.DEGRADED === 1 &&
      degraded?.reason.includes("runtime-executed"),
    `summary=${JSON.stringify(plan.summary)} degraded=${JSON.stringify(plan.degraded)}`,
  );
  assert(
    "P6S-3c. uncovered runtime range stays REVIEW_FIX",
    !!reviewUncovered && reviewUncovered.reason.includes("runtime=uncovered"),
    JSON.stringify(reviewUncovered),
  );
  assert(
    "P6S-3d. type-only export stays REVIEW_FIX",
  ...
  const reasonCodes = new Set(
    (measurement.readiness.reasons ?? []).map((r) => r.code),
  );
  assert(
  ...
  assert(
    "P6S-4b. SAFE_FIX adjudication denominator is known and zero-FP",
  ...
  assert(
    "P6S-4c. one tiny local corpus remains Yellow, not Green",
