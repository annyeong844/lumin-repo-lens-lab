...
check('HPI1. preimagePath rejects unsafe ids and builds session path', () => {
  ...
  try {
    assert.equal(
      ...
      path.join(fx.auditRoot, 'sessions', 'sid_123', 'preimages', 'tool_abc.json')
    );
    assert.throws(() => preimagePath(fx.auditRoot, '../bad', 'tool_abc'), /unsafe session id/);
    assert.throws(() => preimagePath(fx.auditRoot, 'sid_123', '../bad'), /unsafe tool use id/);
  } finally {
    rmSync(fx.root, { recursive: true, force: true });
  }
});

check('HPI2. capture existing file writes fingerprint without raw source text', () => {
  const fx = fixture();
  try {
    ...
    const raw = readFileSync(preimagePath(fx.auditRoot, 'sid_123', 'tool_abc'), 'utf8');
...
check('HPI3. capture missing file records absent preimage', () => {
  ...
  try {
    ...
    assert.deepEqual(readPreimage(fx.auditRoot, 'sid_123', 'tool_missing'), record);
  } finally {
    rmSync(fx.root, { recursive: true, force: true });
  }
});

check('HPI4. readPreimage returns null for missing, malformed, or unsafe records', () => {
  const fx = fixture();
  try {
    assert.equal(readPreimage(fx.auditRoot, 'sid_123', 'tool_none'), null);
    mkdirSync(path.dirname(preimagePath(fx.auditRoot, 'sid_123', 'tool_bad')), { recursive: true });
    writeFileSync(preimagePath(fx.auditRoot, 'sid_123', 'tool_bad'), '{ bad json');
...
check('HPI5. cleanupPreimage removes only the requested tool preimage', () => {
  const fx = fixture();
  try {
    capturePreimage({ auditRoot: fx.auditRoot, sid: 'sid_123', tid: 'tool_a', safe: fx.safe });
    capturePreimage({ auditRoot: fx.auditRoot, sid: 'sid_123', tid: 'tool_b', safe: fx.safe });
...
check('HPI6. cleanupOldPreimages removes only old json preimages', () => {
  const fx = fixture();
  try {
    capturePreimage({ auditRoot: fx.auditRoot, sid: 'sid_123', tid: 'tool_old', safe: fx.safe });
    capturePreimage({ auditRoot: fx.auditRoot, sid: 'sid_123', tid: 'tool_new', safe: fx.safe });
...
check('HPI8. capture re-derives absolute path from repo root and repoRel', () => {
  const fx = fixture();
  const outside = path.join(fx.root, '..', `${path.basename(fx.root)}-outside.ts`);
  try {
    writeFileSync(outside, 'const secret = "outside-file-with-different-size";\n');
    const record = capturePreimage({
      auditRoot: fx.auditRoot,
      sid: 'sid_123',
      tid: 'tool_rederive',
    ...
    const raw = readFileSync(preimagePath(fx.auditRoot, 'sid_123', 'tool_rederive'), 'utf8');
