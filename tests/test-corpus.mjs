...
function runProductionPipeline(fx, out) {
  run('build-symbol-graph.mjs', ['--root', fx, '--output', out, '--production']);
  run('classify-dead-exports.mjs', ['--root', fx, '--output', out, '--production']);
  run('export-action-safety.mjs', ['--root', fx, '--output', out]);
  run('rank-fixes.mjs', ['--root', fx, '--output', out]);
}
...
function cleanupIdentities(fixPlan) {
  return new Set(cleanupEntries(fixPlan)
    .map((s) => `${s.finding.file}::${s.finding.symbol}`));
}
...
// review-visible cleanup candidates.
{
  ...
  try {
    write(fx, 'package.json', JSON.stringify({ name: 'corpus-fp18b', type: 'module' }));
    write(fx, 'src/loader.ts',
      'export async function loadCommand(name: string) {\n' +
      '  return import(`./commands/${name}.js`);\n' +
      '}\n');
    write(fx, 'src/commands/build.ts',
      'export function runBuild() { return "build"; }\n');
    write(fx, 'src/private.ts',
    ...
    const mutedBuild = fixPlan.muted.find((s) => s.finding.symbol === 'runBuild');
    const cleanupDead = findCleanup(fixPlan, (s) => s.finding.symbol === 'trulyDead');

    assert('CASE-FP18B.1. symbols emits dynamic import opacity target directory',
      symbols.dynamicImportOpacity?.some((e) =>
        e.consumerFile === 'src/loader.ts' &&
        e.targetDir === 'src/commands/' &&
        e.kind === 'template-prefix'),
      JSON.stringify(symbols.dynamicImportOpacity));
    assert('CASE-FP18B.2. dynamic command export is not review-visible cleanup',
      !visibleSymbols.has('runBuild'),
      `cleanup candidates: ${[...visibleSymbols].join(', ')}`);
    assert('CASE-FP18B.3. dynamic command export is MUTED with FP-18 evidence',
      mutedBuild &&
        mutedBuild.evidence?.policy?.reason === 'dynamicImportOpacity_FP18' &&
        mutedBuild.evidence.policy.evidence?.some((e) =>
          e.consumerFile === 'src/loader.ts' && e.targetDir === 'src/commands/'),
      `muted: ${JSON.stringify(fixPlan.muted)}`);
    assert('CASE-FP18B.4. classifier summary counts dynamicImportOpacity_FP18',
      classify.summary.excluded.dynamicImportOpacity_FP18 === 1,
      JSON.stringify(classify.summary.excluded));
    assert('CASE-FP18B.5. unrelated private export remains review-visible',
...
// would have inflated the regex counter. AST counter must give 0.
{
  ...
  try {
    write(fx, 'package.json', JSON.stringify({ name: 'corpus-ast', type: 'module' }));
    ...
    assert('CASE-AST.1. deadOnly is in the classified list',
      !!deadOnly, `proposals: ${JSON.stringify(buckets.map((b) => b.symbol))}`);
    assert('CASE-AST.2. deadOnly is classified as Class C (0 refs, not inflated by comment/string)',
      deadOnly && deadOnly.fileInternalUses === 0,
      `fileInternalUses=${deadOnly?.fileInternalUses} (old regex would have said 2)`);
    assert('CASE-AST.3. deadOnly evidence label is AST, not regex',
...
// reach the tsconfig alias resolver before asset classification.
{
  ...
  const out = mkdtempSync(path.join(tmpdir(), 'corpus-route-group-alias-out-'));
  try {
    write(fx, 'package.json', JSON.stringify({
      name: 'route-group-alias',
      type: 'module',
    }));
    write(fx, 'tsconfig.json', JSON.stringify({
      compilerOptions: { baseUrl: '.', paths: { '@/*': ['./*'] } },
    }));
    write(fx, 'app/layout.config.ts',
      "export const baseOptions = { title: 'Docs' };\n");
    write(fx, 'app/(doc)/layout.tsx',
      "import { baseOptions } from '@/app/layout.config';\n" +
      'export const layoutProps = { ...baseOptions };\n');

    runPipeline(fx, out);
    const symbols = readSymbols(out);
    const edge = symbols.resolvedInternalEdges?.find((candidate) =>
      candidate.from === 'app/(doc)/layout.tsx' &&
      candidate.source === '@/app/layout.config' &&
      candidate.to === 'app/layout.config.ts');

    assert('CASE-ROUTE-GROUP-ALIAS.1. dotted alias resolves through the source graph',
      Boolean(edge),
      `resolved edges: ${JSON.stringify(symbols.resolvedInternalEdges)}`);
    assert('CASE-ROUTE-GROUP-ALIAS.2. resolved dotted alias is not resolver blindness',
...
// soon as unresolvedRatio >= 15%.
{
  ...
  try {
    write(fx, 'package.json', JSON.stringify({ name: 'corpus-p1', type: 'module' }));
    ...
    write(fx, 'apps/other/components/authControl.tsx',
    ...
    assert('CASE-P1.1. fixture has high resolver blindness (forces the scenario)',
      ratio >= 0.15,
    ...
    assert('CASE-P1.2. AuthControl outside known alias target is NOT DEGRADED',
      authCtl && authCtl._tier !== 'DEGRADED',
      `tier=${authCtl?._tier}, reason=${authCtl?.reason}`);
    assert('CASE-P1.3. AuthControl reason does not cite unresolved spec taint',
      authCtl && (
    ...
    assert('CASE-P1.4. logger.log (clean file) is NOT in DEGRADED — the P1 win',
...
// review-visible cleanup candidates.
{
  ...
  try {
    write(fx, 'package.json', JSON.stringify({ name: 'corpus-fp44', type: 'module' }));
    write(fx, 'src/contract.ts',
      `export function contractHelper() { return 1; }\n` +
      `export function manifestPinned() { return 2; }\n` +
      `export const trulyDead = 2;\n`);
    write(fx, 'tests/contract.test.ts',
    ...
    const mutedContract = fixPlan.muted.find((s) => s.finding.symbol === 'contractHelper');
    const mutedManifest = fixPlan.muted.find((s) => s.finding.symbol === 'manifestPinned');
    const cleanupDead = findCleanup(fixPlan, (s) => s.finding.symbol === 'trulyDead');

    assert('CASE-FP44.1. test-pinned contractHelper is not review-visible cleanup',
      !proposalSymbols.has('contractHelper'),
      `proposal symbols: ${[...proposalSymbols].join(', ')}`);
    assert('CASE-FP44.2. contractHelper materializes as MUTED testConsumer_FP44',
      mutedContract &&
        mutedContract.evidence?.policy?.reason === 'testConsumer_FP44' &&
        mutedContract.evidence.policy.evidence?.testFile === 'tests/contract.test.ts',
      `muted: ${JSON.stringify(fixPlan.muted)}`);
    assert('CASE-FP44.3. export-manifest pinned symbol also materializes as MUTED',
      mutedManifest &&
        mutedManifest.evidence?.policy?.reason === 'testConsumer_FP44' &&
        mutedManifest.evidence.policy.evidence?.importKind === 'test-export-manifest',
      `muted: ${JSON.stringify(fixPlan.muted)}`);
    assert('CASE-FP44.4. trulyDead remains review-visible cleanup',
...
// mapOutputToSource the barrel is correctly detected.
{
  ...
  try {
    ...
    const deadProdSymbols = new Set(symbols.deadProdList?.map((d) => d.symbol) ?? []);
    const proposalSymbols = new Set([
      ...(classify.proposal_C_remove_symbol ?? []).map((p) => p.symbol),
      ...(classify.proposal_A_demote_to_internal ?? []).map((p) => p.symbol),
      ...(classify.proposal_B_review ?? []).map((p) => p.symbol),
    ...
    assert('CASE-FP40.1. barrelExport (from ./dist/index.mjs entry) is NOT dead-listed',
      !deadProdSymbols.has('barrelExport'),
      `deadProdList symbols: ${[...deadProdSymbols].join(', ')}`);
    assert('CASE-FP40.2. barrelFn (from ./dist/index.mjs entry) is NOT dead-listed',
      !deadProdSymbols.has('barrelFn'));
    assert('CASE-FP40.3. barrelExport is NOT proposed for removal',
      !proposalSymbols.has('barrelExport'));
    assert('CASE-FP40.4. trulyDead IS dead-listed (sanity: fixture actually runs the analysis)',
...
for (const [label, exportsField] of [
  ...
  ['conditional-root', { import: './dist/index.mjs', types: './dist/index.d.ts' }],
]) {
  ...
  try {
    ...
    assert(`CASE-FP40B.${label}. root export form marks src/index.ts as barrel`,
...
// it counts 1 file-internal use and Trigger lands in Tier A.
{
  ...
  try {
    ...
    write(fx, 'src/components/alert-dialog.tsx',
    ...
    const classify = readClassify(out);
    const buckets = [
      ...(classify.proposal_C_remove_symbol ?? []).map((p) => ({ ...p, _bucket: 'C' })),
      ...(classify.proposal_A_demote_to_internal ?? []).map((p) => ({ ...p, _bucket: 'A' })),
      ...(classify.proposal_B_review ?? []).map((p) => ({ ...p, _bucket: 'B' })),
    ];
    ...
    // any dead bucket.
    assert('CASE-FP41.1. AlertDialog (live) is NOT in any dead bucket',
      !alert,
      `AlertDialog bucket=${alert?._bucket}`);

    // Trigger IS in the classifier output (no external consumer).
    assert('CASE-FP41.2. AlertDialogTrigger appears in the classifier output',
      !!trigger,
    ...
    // valueRefs hit.
    assert('CASE-FP41.3. AlertDialogTrigger fileInternalUses == 1 (JSX use counted)',
      trigger && trigger.fileInternalUses === 1,
      `fileInternalUses=${trigger?.fileInternalUses}`);
    assert('CASE-FP41.4. Trigger\'s JSX use is tracked as a value reference',
    ...
    assert('CASE-FP41.5. Trigger classified as A-remove-export, not C-completely-dead',
      trigger && trigger._bucket === 'A',
      `bucket=${trigger?._bucket}`);

    // Evidence label preserved through the JSX-aware path.
    assert('CASE-FP41.6. Evidence label remains ast-ident-ref-count',
...
// public API evidence.
{
  ...
  try {
    ...
    write(fx, 'src/index.ts',
      `export function publicEntry() { return 1; }\n`);
    write(fx, 'types/index.d.ts',
    ...
    const fixPlan = readFixPlan(out);
    const visibleSymbols = cleanupSymbols(fixPlan);
    const mutedPublicOptions = fixPlan.muted.find((s) =>
      s.finding.file === 'types/index.d.ts' && s.finding.symbol === 'PublicOptions');
    const cleanupDead = findCleanup(fixPlan, (s) => s.finding.symbol === 'trulyDead');

    assert('CASE-P6-1.1. root export publicEntry is not review-visible cleanup',
      !visibleSymbols.has('publicEntry'),
      `cleanup candidates: ${[...visibleSymbols].join(', ')}`);
    assert('CASE-P6-1.2. type-only public subpath is MUTED as publicApi_FP23',
      mutedPublicOptions &&
        mutedPublicOptions.evidence?.policy?.reason === 'publicApi_FP23' &&
        Array.isArray(mutedPublicOptions.evidence.policy.evidence) &&
        mutedPublicOptions.evidence.policy.evidence.some((e) =>
          e.source === 'package.exports' && e.subpath === './types'),
      `muted: ${JSON.stringify(fixPlan.muted)}`);
    assert('CASE-P6-1.3. unrelated private export remains review-visible',
      cleanupDead && cleanupDead.finding.file === 'src/private.ts',
      `cleanup candidates: ${JSON.stringify(cleanupEntries(fixPlan).map((s) => s.finding))}`);
  } finally {
    rmSync(fx, { recursive: true, force: true });
...
// reachable from the package entry.
{
  ...
  const out = mkdtempSync(path.join(tmpdir(), 'corpus-p6-identity-surface-out-'));
  try {
    write(fx, 'package.json', JSON.stringify({
    ...
    for (let moduleIndex = 0; moduleIndex < 20; moduleIndex++) {
      ...
      for (let deadIndex = 0; deadIndex < 3; deadIndex++) {
        ...
        expectedDeadIdentities.add(`src/module-${moduleIndex}.ts::${deadName}`);
      }
    ...
    const missingDead = [...expectedDeadIdentities]
      .filter((identity) => !cleanup.has(identity));
    const mutedDead = fixPlan.muted.filter((entry) =>
      expectedDeadIdentities.has(`${entry.finding.file}::${entry.finding.symbol}`));
    assert('CASE-P6-1g.1. all 60 unused sibling exports remain review-visible',
      missingDead.length === 0 && expectedDeadIdentities.size === 60,
      `missing dead identities: ${JSON.stringify(missingDead)}`);
    assert('CASE-P6-1g.2. no unused sibling is muted by file-level public API policy',
      mutedDead.length === 0,
      `muted dead siblings: ${JSON.stringify(mutedDead)}`);
    assert('CASE-P6-1g.3. direct package entry exports remain protected',
      !cleanup.has('src/index.ts::directEntry'),
      `cleanup candidates: ${JSON.stringify([...cleanup])}`);
    assert('CASE-P6-1g.4. named public re-exports do not become cleanup candidates',
      publicReExports.every((_, index) =>
        !cleanup.has(`src/module-${index}.ts::public${index}`)),
      `cleanup candidates: ${JSON.stringify([...cleanup])}`);
  } finally {
    rmSync(fx, { recursive: true, force: true });
    rmSync(out, { recursive: true, force: true });
...
// by `imports` must not be muted as publicApi_FP23.
{
  ...
  try {
    ...
    write(fx, 'src/internal.ts',
      `export const internalOnly = 1;\n`);

    runPipeline(fx, out);
    const classify = readClassify(out);
    const fixPlan = readFixPlan(out);
    const mutedInternal = fixPlan.muted.find((s) => s.finding.symbol === 'internalOnly');
    const cleanupInternal = findCleanup(fixPlan, (s) => s.finding.symbol === 'internalOnly');

    assert('CASE-P6-1a.1. package.imports exact target is NOT counted as publicApi_FP23',
      classify.summary.excluded.publicApi_FP23 === 0,
      JSON.stringify(classify.summary.excluded));
    assert('CASE-P6-1a.2. #imports-only dead export is not MUTED as public API',
      !(mutedInternal?.evidence?.policy?.reason === 'publicApi_FP23'),
      `muted: ${JSON.stringify(fixPlan.muted)}`);
    assert('CASE-P6-1a.3. #imports-only dead export remains review-visible',
...
// import consumer inside source, but the build tool consumes them by path.
{
  ...
  try {
    ...
    write(fx, 'scripts/build.ts',
      ...
      `]\n`);
    write(fx, 'src/client/dev/react.ts',
    ...
    const fixPlan = readFixPlan(out);
    const visibleSymbols = cleanupSymbols(fixPlan);
    const mutedUseRegister = fixPlan.muted.find((s) => s.finding.symbol === 'useRegisterSW');
    const mutedOptions = fixPlan.muted.find((s) => s.finding.symbol === 'RegisterSWOptions');
    const cleanupDead = findCleanup(fixPlan, (s) => s.finding.symbol === 'trulyDead');

    assert('CASE-P6-1b.1. script entrypoint function is not review-visible cleanup',
      !visibleSymbols.has('useRegisterSW'),
      `cleanup candidates: ${[...visibleSymbols].join(', ')}`);
    assert('CASE-P6-1b.2. script entrypoint function is MUTED with FP45 evidence',
      mutedUseRegister &&
        mutedUseRegister.evidence?.policy?.reason === 'scriptEntrypoint_FP45' &&
        mutedUseRegister.evidence.policy.evidence?.some((e) =>
          e.source === 'script-string-literal' && e.scriptFile === 'scripts/build.ts'),
      `muted: ${JSON.stringify(fixPlan.muted)}`);
    assert('CASE-P6-1b.3. all exports in the script entry file are muted',
      mutedOptions && mutedOptions.evidence?.policy?.reason === 'scriptEntrypoint_FP45',
      `muted: ${JSON.stringify(fixPlan.muted)}`);
    assert('CASE-P6-1b.4. unrelated private export remains review-visible',
      cleanupDead && cleanupDead.finding.file === 'src/private.ts',
      `cleanup candidates: ${JSON.stringify(cleanupEntries(fixPlan).map((s) => s.finding))}`);
  } finally {
    rmSync(fx, { recursive: true, force: true });
...
// as dead even though a public entrypoint re-exports it.
{
  ...
  try {
    write(fx, 'package.json', JSON.stringify({
      ...
      type: 'module',
      scripts: {
        build: 'tsup src/client/dev/register.ts --format esm -d dist/client/dev',
      },
    }));
    write(fx, 'src/client/type.d.ts',
      `export interface RegisterSWOptions { immediate?: boolean }\n`);
    write(fx, 'src/client/dev/register.ts',
    ...
    const symbols = readSymbols(out);
    const fixPlan = readFixPlan(out);
    const deadIdentities = new Set((symbols.deadProdList ?? [])
      .map((d) => `${d.file}::${d.symbol}`));
    const visibleSymbols = cleanupIdentities(fixPlan);
    const mutedRegister = fixPlan.muted.find((s) =>
      s.finding.file === 'src/client/dev/register.ts' && s.finding.symbol === 'registerSW');
    const cleanupDead = findCleanup(fixPlan, (s) => s.finding.symbol === 'trulyDead');

    assert('CASE-P6-1d.1. sibling .d.ts type import contributes fan-in',
      symbols.fanInByIdentity?.['src/client/type.d.ts::RegisterSWOptions'] === 1,
      `fanIn=${symbols.fanInByIdentity?.['src/client/type.d.ts::RegisterSWOptions']}`);
    assert('CASE-P6-1d.2. source declaration type is not dead-listed',
      !deadIdentities.has('src/client/type.d.ts::RegisterSWOptions'),
      `dead=${[...deadIdentities].join(', ')}`);
    assert('CASE-P6-1d.3. source declaration type is not review-visible cleanup',
      !visibleSymbols.has('src/client/type.d.ts::RegisterSWOptions'),
      `cleanup candidates=${[...visibleSymbols].join(', ')}`);
    assert('CASE-P6-1d.4. public script entry still gets muted by entrypoint policy',
      mutedRegister?.evidence?.policy?.reason === 'scriptEntrypoint_FP45',
      `muted=${JSON.stringify(fixPlan.muted)}`);
    assert('CASE-P6-1d.5. unrelated private export remains review-visible',
      cleanupDead && cleanupDead.finding.file === 'src/private.ts',
      `cleanup candidates=${JSON.stringify(cleanupEntries(fixPlan).map((s) => s.finding))}`);
  } finally {
    rmSync(fx, { recursive: true, force: true });
...
// symbols look dead even though workspace consumers import the package.
{
  ...
  try {
    ...
    write(fx, 'packages/shared/package.json', JSON.stringify({
      name: '@scope/shared',
      type: 'module',
      exports: {
        '.': {
    ...
    const cleanupDead = findCleanup(fixPlan, (s) =>
      s.finding.file === 'packages/shared/src/private.ts' &&
      s.finding.symbol === 'trulyDead');

    assert('CASE-P6-1e.1. package import fan-in lands on source function',
      symbols.fanInByIdentity?.['packages/shared/src/index.ts::createShared'] === 1,
      `fanIn=${symbols.fanInByIdentity?.['packages/shared/src/index.ts::createShared']}, all=${JSON.stringify(symbols.fanInByIdentity)}`);
    assert('CASE-P6-1e.2. package type import fan-in lands on source interface',
      symbols.fanInByIdentity?.['packages/shared/src/index.ts::PublicOptions'] === 1,
      `fanIn=${symbols.fanInByIdentity?.['packages/shared/src/index.ts::PublicOptions']}, all=${JSON.stringify(symbols.fanInByIdentity)}`);
    assert('CASE-P6-1e.3. imported public source exports are not review-visible cleanup',
    ...
    assert('CASE-P6-1e.4. unreferenced export in package public source file is not review-visible cleanup',
      !visibleIdentities.has('packages/shared/src/index.ts::publicButUnreferenced'),
      `cleanup candidates=${[...visibleIdentities].join(', ')}`);
    assert('CASE-P6-1e.5. unrelated private export remains review-visible',
      cleanupDead,
      `cleanup candidates=${JSON.stringify(cleanupEntries(fixPlan).map((s) => s.finding))}`);
  } finally {
    rmSync(fx, { recursive: true, force: true });
...
// paired runtime file.
{
  ...
  try {
    ...
    const fixPlan = readFixPlan(out);
    const visibleIdentities = cleanupIdentities(fixPlan);
    const mutedSidecar = fixPlan.muted.find((s) =>
      s.finding.file === 'src/runtime-module-sources.d.ts' &&
      s.finding.symbol === 'RUNTIME_MODULE_SOURCES');
    const cleanupDead = findCleanup(fixPlan, (s) =>
      s.finding.file === 'src/private.ts' &&
      s.finding.symbol === 'trulyDead');

    assert('CASE-P6-1f.1. declaration sidecar is not review-visible cleanup',
      !visibleIdentities.has('src/runtime-module-sources.d.ts::RUNTIME_MODULE_SOURCES'),
      `cleanup candidates=${[...visibleIdentities].join(', ')}`);
    assert('CASE-P6-1f.2. declaration sidecar is MUTED with FP48 evidence',
      mutedSidecar?.evidence?.policy?.reason === 'declarationSidecar_FP48',
      `muted=${JSON.stringify(fixPlan.muted)}`);
    assert('CASE-P6-1f.3. unrelated private export remains review-visible',
      cleanupDead,
      `cleanup candidates=${JSON.stringify(cleanupEntries(fixPlan).map((s) => s.finding))}`);
  } finally {
    rmSync(fx, { recursive: true, force: true });
...
// Both are real consumers outside the import graph.
{
  ...
  try {
    ...
    write(fx, 'docs/package.json', JSON.stringify({
      name: 'docs',
      type: 'module',
      devDependencies: { vitepress: '1.0.0' },
    }));
    write(fx, 'docs/.vitepress/config.ts',
      `export default { title: 'Docs' };\n`);
    write(fx, 'docs/.vitepress/theme/index.ts',
      `export default { enhanceApp() {} };\n`);
    write(fx, 'docs/.vitepress/contributors.ts',
      `export const teamMembers = [];\n`);
    write(fx, 'app/package.json', JSON.stringify({ name: 'app', type: 'module' }));
    write(fx, 'app/index.html',
      `<div id="app"></div>\n<script type="module" src="/src/main.ts"></script>\n`);
    write(fx, 'app/src/main.ts',
      `export default { mounted: true };\n`);
    write(fx, 'app/src/private.ts',
      `export const trulyDead = 1;\n`);

    runPipeline(fx, out);
    const fixPlan = readFixPlan(out);
    const visibleSymbols = cleanupIdentities(fixPlan);
    const mutedConfig = fixPlan.muted.find((s) => s.finding.file === 'docs/.vitepress/config.ts');
    const mutedTheme = fixPlan.muted.find((s) => s.finding.file === 'docs/.vitepress/theme/index.ts');
    const mutedMain = fixPlan.muted.find((s) => s.finding.file === 'app/src/main.ts');
    const cleanupContrib = findCleanup(fixPlan, (s) => s.finding.file === 'docs/.vitepress/contributors.ts');
    const cleanupDead = findCleanup(fixPlan, (s) => s.finding.file === 'app/src/private.ts');

    assert('CASE-P6-1c.1. VitePress config is MUTED by convention',
      mutedConfig?.evidence?.policy?.reason === 'vitePress_FP46',
      `muted=${JSON.stringify(fixPlan.muted)}`);
    assert('CASE-P6-1c.2. VitePress theme index is MUTED by convention',
      mutedTheme?.evidence?.policy?.reason === 'vitePress_FP46',
      `muted=${JSON.stringify(fixPlan.muted)}`);
    assert('CASE-P6-1c.3. HTML module main entrypoint is MUTED with evidence',
      mutedMain?.evidence?.policy?.reason === 'htmlEntrypoint_FP47' &&
        mutedMain.evidence.policy.evidence?.some((e) =>
          e.source === 'html-module-script' && e.htmlFile === 'app/index.html'),
      `muted=${JSON.stringify(fixPlan.muted)}`);
    assert('CASE-P6-1c.4. unrelated .vitepress helper is still review-visible',
      cleanupContrib && visibleSymbols.has('docs/.vitepress/contributors.ts::teamMembers'),
      `cleanup candidates=${[...visibleSymbols].join(', ')}`);
    assert('CASE-P6-1c.5. unrelated app private export remains review-visible',
      cleanupDead && cleanupDead.finding.symbol === 'trulyDead',
      `cleanup candidates=${JSON.stringify(cleanupEntries(fixPlan).map((s) => s.finding))}`);
  } finally {
    rmSync(fx, { recursive: true, force: true });
...
// top-level Next middleware/proxy convention.
{
  ...
  try {
    ...
    write(fx, 'app/page.tsx',
      `export function RootPage() { return null; }\n`);
    write(fx, 'proxy.ts',
      `export function proxy() { return Response.json({ ok: true }); }\n` +
      `export const config = { matcher: ['/api/:path*'] };\n`);
    write(fx, 'app/foo/middleware.ts',
      `export function middleware() { return null; }\n`);
    write(fx, 'src/instrumentation-client.ts',
      `export default function clientInstrumentation() {}\n`);
    write(fx, 'packages/tool/package.json', JSON.stringify({
      name: '@fixture/tool',
      type: 'module',
    }));
    write(fx, 'packages/tool/app/page.tsx',
    ...
    const mutedSymbol = (file, symbol) => fixPlan.muted.find((s) =>
      s.finding.file === file && s.finding.symbol === symbol);
    const visibleSymbol = (file, symbol) => visible.find((s) =>
      s.finding.file === file && s.finding.symbol === symbol);

    assert('CASE-FRAMEWORK-POLICY-1.1. root Next app route is MUTED by framework policy',
      mutedSymbol('app/page.tsx', 'RootPage')?.evidence?.policy?.reason === 'frameworkSentinel_FP27',
      `muted=${JSON.stringify(fixPlan.muted)}`);
    assert('CASE-FRAMEWORK-POLICY-1.2. root Next proxy export is MUTED by framework policy',
      mutedSymbol('proxy.ts', 'proxy')?.evidence?.policy?.reason === 'frameworkSentinel_FP27',
      `muted=${JSON.stringify(fixPlan.muted)}\nvisible=${JSON.stringify(visible)}`);
    assert('CASE-FRAMEWORK-POLICY-1.3. nested non-Next app route stays visible',
      !mutedSymbol('packages/tool/app/page.tsx', 'ToolPage') &&
        visibleSymbol('packages/tool/app/page.tsx', 'ToolPage'),
      `muted=${JSON.stringify(fixPlan.muted)}\nvisible=${JSON.stringify(visible)}`);
    assert('CASE-FRAMEWORK-POLICY-1.4. nested app middleware path stays visible',
      !mutedSymbol('app/foo/middleware.ts', 'middleware') &&
        visibleSymbol('app/foo/middleware.ts', 'middleware'),
      `muted=${JSON.stringify(fixPlan.muted)}\nvisible=${JSON.stringify(visible)}`);
    assert('CASE-FRAMEWORK-POLICY-1.5. phase-1 framework counters are emitted',
      classify.summary.frameworkPolicy?.mutedFindings?.next >= 2 &&
        classify.summary.frameworkPolicy?.reviewHintFindings?.next >= 1 &&
        classify.summary.frameworkPolicy?.pathShapedCandidatesKeptVisible?.middleware >= 1,
...
// export names should become MUTED.
{
  ...
  try {
    ...
    write(fx, 'packages/hono/package.json', JSON.stringify({
      name: '@fixture/hono-app',
      type: 'module',
      dependencies: { hono: '^4.0.0' },
    }));
    write(fx, 'packages/hono/src/server.ts',
      `import { Hono } from 'hono';\n` +
      `const app = new Hono();\n` +
      `export function health(c) { return c.text('ok'); }\n` +
      `export function looseHelper() { return 'loose'; }\n` +
      `app.get('/health', health);\n`);
    write(fx, 'packages/svelte/package.json', JSON.stringify({
      name: '@fixture/svelte-app',
      type: 'module',
      dependencies: { '@sveltejs/kit': '^2.0.0' },
    }));
    write(fx, 'packages/svelte/src/routes/blog/[slug]/+page.server.ts',
      `export function load() { return {}; }\n` +
      `export const entries = () => [{ slug: 'a' }];\n` +
      `export function privateHelper() { return 1; }\n`);
    write(fx, 'packages/astro/package.json', JSON.stringify({
      name: '@fixture/astro-app',
      type: 'module',
      dependencies: { astro: '^5.0.0' },
    }));
    write(fx, 'packages/astro/src/pages/api/user.ts',
      `export function GET() { return new Response('ok'); }\n` +
      `export function helper() { return 'helper'; }\n`);
    write(fx, 'packages/router/package.json', JSON.stringify({
      name: '@fixture/router-app',
      type: 'module',
      dependencies: { '@react-router/dev': '^7.0.0' },
    }));
    write(fx, 'packages/router/app/routes/home.tsx',
    ...
    assert('CASE-FRAMEWORK-POLICY-2.1. Hono local route handler is MUTED by route fact',
      mutedSymbol('packages/hono/src/server.ts', 'health')?.evidence?.policy?.reason === 'frameworkSentinel_FP27',
      `muted=${JSON.stringify(fixPlan.muted)}\nvisible=${JSON.stringify(visible)}`);
    assert('CASE-FRAMEWORK-POLICY-2.2. unrelated Hono helper remains visible',
      visibleSymbol('packages/hono/src/server.ts', 'looseHelper') &&
        !mutedSymbol('packages/hono/src/server.ts', 'looseHelper'),
      `muted=${JSON.stringify(fixPlan.muted)}\nvisible=${JSON.stringify(visible)}`);
    assert('CASE-FRAMEWORK-POLICY-2.3. SvelteKit load and dynamic entries are MUTED',
      mutedSymbol('packages/svelte/src/routes/blog/[slug]/+page.server.ts', 'load') &&
        mutedSymbol('packages/svelte/src/routes/blog/[slug]/+page.server.ts', 'entries'),
      `muted=${JSON.stringify(fixPlan.muted)}`);
    assert('CASE-FRAMEWORK-POLICY-2.4. ordinary SvelteKit route helper remains visible',
      visibleSymbol('packages/svelte/src/routes/blog/[slug]/+page.server.ts', 'privateHelper') &&
        !mutedSymbol('packages/svelte/src/routes/blog/[slug]/+page.server.ts', 'privateHelper'),
      `muted=${JSON.stringify(fixPlan.muted)}\nvisible=${JSON.stringify(visible)}`);
    assert('CASE-FRAMEWORK-POLICY-2.5. Astro endpoint GET is MUTED but helper remains visible',
      ...
      `muted=${JSON.stringify(fixPlan.muted)}\nvisible=${JSON.stringify(visible)}`);
    assert('CASE-FRAMEWORK-POLICY-2.6. React Router loader is MUTED while clientLoader is review-visible',
      mutedSymbol('packages/router/app/routes/home.tsx', 'loader') &&
        ...
        classify.summary.frameworkPolicy?.reviewHintFindings?.['react-router'] >= 1,
...
{
  const fx = mkdtempSync(path.join(tmpdir(), 'corpus-framework-policy-non-next-'));
  const out = mkdtempSync(path.join(tmpdir(), 'corpus-framework-policy-non-next-out-'));
  try {
    write(fx, 'package.json', JSON.stringify({
    ...
    write(fx, 'apps/svelte/package.json', JSON.stringify({
      name: '@fixture/nested-svelte',
      type: 'module',
      dependencies: { '@sveltejs/kit': '^2.0.0' },
    }));
    write(fx, 'apps/svelte/src/routes/blog/[slug]/+page.server.ts',
      `export function load() { return {}; }\n` +
      `export const entries = () => [{ slug: 'a' }];\n` +
      `export function privateHelper() { return 1; }\n`);
    write(fx, 'apps/astro/package.json', JSON.stringify({
      name: '@fixture/nested-astro',
      type: 'module',
      dependencies: { astro: '^5.0.0' },
    }));
    write(fx, 'apps/astro/src/pages/api/user.ts',
      `export function GET() { return new Response('ok'); }\n` +
      `export function helper() { return 'helper'; }\n`);
    ...
    const svelteLoad = mutedSymbol('apps/svelte/src/routes/blog/[slug]/+page.server.ts', 'load');
    const svelteEntries = mutedSymbol('apps/svelte/src/routes/blog/[slug]/+page.server.ts', 'entries');
    const astroGet = mutedSymbol('apps/astro/src/pages/api/user.ts', 'GET');

    assert('CASE-FRAMEWORK-POLICY-2b.1. non-workspace nested SvelteKit load is MUTED by nearest package',
      svelteLoad?.evidence?.policy?.reason === 'frameworkSentinel_FP27' &&
        svelteLoad.evidence.policy.evidence?.packageRoot === 'apps/svelte' &&
        svelteLoad.evidence.policy.evidence?.activation?.includes('dependency:@sveltejs/kit'),
      `muted=${JSON.stringify(fixPlan.muted)}\nvisible=${JSON.stringify(visible)}`);
    assert('CASE-FRAMEWORK-POLICY-2b.2. non-workspace nested SvelteKit dynamic entries are MUTED',
      ...
      `muted=${JSON.stringify(fixPlan.muted)}`);
    assert('CASE-FRAMEWORK-POLICY-2b.3. ordinary nested SvelteKit route helper remains visible',
      visibleSymbol('apps/svelte/src/routes/blog/[slug]/+page.server.ts', 'privateHelper') &&
        !mutedSymbol('apps/svelte/src/routes/blog/[slug]/+page.server.ts', 'privateHelper'),
      `muted=${JSON.stringify(fixPlan.muted)}\nvisible=${JSON.stringify(visible)}`);
    assert('CASE-FRAMEWORK-POLICY-2b.4. non-workspace nested Astro endpoint GET is MUTED by nearest package',
      astroGet?.evidence?.policy?.reason === 'frameworkSentinel_FP27' &&
        astroGet.evidence.policy.evidence?.packageRoot === 'apps/astro' &&
        astroGet.evidence.policy.evidence?.activation?.includes('dependency:astro'),
      `muted=${JSON.stringify(fixPlan.muted)}\nvisible=${JSON.stringify(visible)}`);
    assert('CASE-FRAMEWORK-POLICY-2b.5. ordinary nested Astro helper remains visible',
...
// other server projects where exports may be ordinary review candidates.
{
  ...
  try {
    ...
    write(fx, 'src/nest-application.ts',
    ...
    const classify = readClassify(out);
    const fixPlan = readFixPlan(out);
    const mutedMiddleware = fixPlan.muted.find((s) =>
    ...
    assert('CASE-FP30-SCOPE.1. h3 alone does not activate Nuxt/Nitro mute policy',
      classify.summary.excluded.nuxtNitro_FP30 === 0,
      JSON.stringify(classify.summary.excluded));
    assert('CASE-FP30-SCOPE.2. Nest-style middleware helper is not MUTED as nuxtNitro_FP30',
      !mutedMiddleware,
      `muted=${JSON.stringify(fixPlan.muted)}`);
    assert('CASE-FP30-SCOPE.3. ordinary middleware helper remains review-visible',
      !!visibleMiddleware &&
...
// fan-in.
{
  ...
  try {
    write(fx, 'package.json', JSON.stringify({ name: 'corpus-decl-surface', type: 'module' }));
    write(fx, 'src/api.ts',
    ...
    runPipeline(fx, out);
    const fixPlan = readFixPlan(out);
    const cleanupHidden = findCleanup(fixPlan, (s) =>
      s.finding.file === 'src/api.ts' && s.finding.symbol === 'HiddenResult');
    const degradedHidden = fixPlan.degraded.find((s) =>
      s.finding.file === 'src/api.ts' && s.finding.symbol === 'HiddenResult');
    const cleanupDead = findCleanup(fixPlan, (s) =>
      s.finding.file === 'src/private.ts' && s.finding.symbol === 'trulyDead');

    assert('CASE-DECL-1.1. exported class/const signature type is demote-only SAFE_FIX',
      cleanupHidden?.finding?.declarationExportDependency === true &&
        cleanupHidden.finding.safeAction?.kind === 'demote_export_declaration' &&
        cleanupHidden.finding.safeAction?.strongerActionBlockers?.includes('local-refs-present'),
      `cleanup candidates=${JSON.stringify(cleanupEntries(fixPlan).map((s) => s.finding))}`);
    assert('CASE-DECL-1.2. exported class/const signature type is not DEGRADED when demote preserves binding',
      !degradedHidden,
      `degraded=${JSON.stringify(fixPlan.degraded.map((s) => ({ reason: s.reason, finding: s.finding })))}`);
    assert('CASE-DECL-1.3. unrelated private export remains review-visible',
      cleanupDead,
      `cleanup candidates=${JSON.stringify(cleanupEntries(fixPlan).map((s) => s.finding))}`);
  } finally {
    rmSync(fx, { recursive: true, force: true });
...
const FP_BUDGET = 0;
const fpCount = failed;

console.log(`\n── FP budget gate ──`);
console.log(`  precision failures: ${fpCount}`);
console.log(`  budget:             ${FP_BUDGET}`);
console.log(`  gate:               ${fpCount <= FP_BUDGET ? 'PASS' : 'FAIL'}`);
