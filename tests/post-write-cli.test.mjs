...

{
  ...
  try {
    ...
    assert(
      'T1b. stdout contains "post-write delta" header',
      stdout.includes("## post-write delta"),
    );
    assert(
      "T1c. delta JSON latest written",
      existsSync(path.join(out, "post-write-delta.latest.json")),
    );
    const deltaFiles = readdirSync(out).filter(
      (n) => n.startsWith("post-write-delta.") && !n.endsWith(".latest.json"),
    );
    assert("T1d. specific delta file exists", deltaFiles.length === 1);
    const delta = JSON.parse(
      readFileSync(path.join(out, "post-write-delta.latest.json"), "utf8"),
    );
    assert(
      "T1e. preWriteInvocationId matches advisory",
      delta.preWriteInvocationId ===
        JSON.parse(readFileSync(advisory, "utf8")).invocationId,
    );
    assert(
      "T1f. deltaInvocationId populated and well-formed",
      typeof delta.deltaInvocationId === "string" &&
        delta.deltaInvocationId.length > 0,
    );
    assert(
      "T1g. deltaInvocationId !== preWriteInvocationId (freshly generated)",
...

{
  ...
  assert(
    "T2a. post-write.mjs imports generateDeltaInvocationId",
    src.includes("generateDeltaInvocationId"),
  );
  assert(
    "T2b. post-write.mjs calls computeDelta with deltaInvocationId in arg object",
...

{
  ...
  try {
    ...
    const after1 = readdirSync(out).filter(
      (n) => n.startsWith("post-write-delta.") && !n.endsWith(".latest.json"),
    );
    postWrite(fx, out, advisory);
    const after2 = readdirSync(out).filter(
      (n) => n.startsWith("post-write-delta.") && !n.endsWith(".latest.json"),
    );
    assert(
      "T3. second post-write run produces a NEW specific file (prior preserved)",
...

{
  ...
  try {
    ...
    assert(
      'T4a. capabilityParity.status === "missing"',
      delta.capabilityParity.status === "missing",
    );
    assert("T4b. entries: []", delta.entries.length === 0);
    assert(
      "T4c. capabilityFailures carries after-inventory-missing",
      (delta.capabilityFailures ?? []).some(
        (f) => f.kind === "after-inventory-missing",
      ),
    );
  } finally {
    rmSync(fx, { recursive: true, force: true });
...

{
  ...
  try {
    ...
    const invInc = JSON.parse(
      readFileSync(path.join(out, "any-inventory.json"), "utf8"),
    );
    assert(
      "T5a. --include-tests: after-inventory meta.includeTests === true",
    ...
    const invProd = JSON.parse(
      readFileSync(path.join(out, "any-inventory.json"), "utf8"),
    );
    assert(
      "T5b. --production: after-inventory meta.includeTests === false",
...

{
  ...
  try {
    ...
    const stdout = postWrite(fx, out, advisory);
    assert(
      "T6. path with spaces + $ survives end-to-end",
      stdout.includes("## post-write delta") &&
        existsSync(path.join(out, "post-write-delta.latest.json")),
    );
  } finally {
...

{
  ...
  try {
    ...
    assert("T7a. missing --pre-write-advisory → exit 1", res.status === 1);
    assert(
      "T7b. stderr mentions advisory",
...

{
  ...
  try {
    ...
    assert("T8a. non-existent advisory → exit 1", res.status === 1);
    assert(
      "T8b. stderr mentions not found",
...

{
  ...
  try {
    ...
    assert(
      "T9pre. advisory has no preWrite.anyInventoryPath (pre-condition)",
      !advContent.preWrite ||
        !("anyInventoryPath" in (advContent.preWrite ?? {})),
    );
...

{
  ...
  try {
    ...
    const stdout = postWrite(fx, out, advisory);
    assert(
      "T10a. stdout mentions after-inventory incomplete",
      stdout.includes("after-inventory incomplete"),
    );
    assert(
      'T10b. stdout does NOT contain "No silent new any in the scan range."',
...

{
  ...
  try {
    ...
    assert(
      'T11b. stdout starts with "## post-write delta"',
      res.stdout.startsWith("## post-write delta"),
    );
    assert(
      "T11c. stderr contains [post-write] diagnostic (or empty)",
      res.stderr.length === 0 || /\[post-write\]/.test(res.stderr),
    );
    assert(
      "T11d. stdout has NO [post-write] diagnostic prefix",
...

{
  ...
  try {
    ...
    const delta = JSON.parse(
      readFileSync(path.join(postOut, "post-write-delta.latest.json"), "utf8"),
    );
    assert(
      "T12a. before-inventory found via advisory dir or scanRange.output",
      delta.baseline.status === "available",
      `baseline=${JSON.stringify(delta.baseline)}`,
    );
    assert(
      "T12b. scan range parity can be evaluated",
...

{
  ...
  try {
    ...
    assert(
      "T13. absolute preWrite.anyInventoryPath loads before-inventory",
      delta.baseline.status === "available",
      `baseline=${JSON.stringify(delta.baseline)}`,
    );
  } finally {
