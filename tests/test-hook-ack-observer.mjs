...
check('HAO1. parseAuditAckLines parses a valid standalone ACK line', () => {
  assert.deepEqual(parseAuditAckLines('ok\nAUDIT_ACK evt_abc123 fixed\nthanks'), [
...
check('HAO4. parseAuditAckLines ignores ACK inside unclosed fence through EOF', () => {
  assert.deepEqual(parseAuditAckLines('before\n```md\nAUDIT_ACK evt_abc123 fixed'), []);
});
...
check('HAO7. parseAuditAckLines ignores ACK inside leading-space blockquote', () => {
  assert.deepEqual(parseAuditAckLines('  > AUDIT_ACK evt_abc123 fixed'), []);
});

check('HAO8. observeStopAcknowledgements marks matching event acknowledged', () => {
  const fx = fixture();
  try {
...
check('HAO9. observer prefers last_assistant_message over transcriptText fallback', () => {
  const fx = fixture();
  try {
    ...
    assert.equal(entries.find((entry) => entry.id === first.eventId).acknowledged, true);
    assert.equal(entries.find((entry) => entry.id === second.eventId).acknowledged, false);
  } finally {
    rmSync(fx.root, { recursive: true, force: true });
  }
});

check('HAO10. observer uses transcriptText fallback when last_assistant_message is absent', () => {
  const fx = fixture();
  try {
    const appended = appendEventIfNotDeduped(fx.auditRoot, fx.sid, event());
    const result = observeStopAcknowledgements(
    ...
    assert.equal(readEventStoreState(fx.auditRoot, fx.sid).entries[0].ack_source, 'noted');
  } finally {
    rmSync(fx.root, { recursive: true, force: true });
  }
});
