...

{
  const root = fresh();
  try {
    const cacheRoot = defaultAuditCacheRoot({
      root,
      output: path.join(root, ".audit", "runs", "r1"),
    });
    assert(
      "default cache root is stable .audit/.cache sibling",
...

{
  ...
  try {
    ...
    assert(
      "strict reusable fact hit requires matching current content hash",
    ...
    assert(
      "different current content hash misses",
...

{
  ...
  try {
    ...
    assert(
      "read-error entries never become clean hits",
...

{
  ...
  try {
    ...
    writeFileSync(
      path.join(store.repoCacheDir, "any-inventory.cache.json"),
      "{not-json",
    );
    const cache = loadProducerCache(store, "any-inventory");
    assert(
      "malformed cache loads as empty with invalidated reason",
...

{
  ...
  try {
    ...
    assert(
      "saveProducerCache writes cache file atomically",
      existsSync(file) &&
        JSON.parse(readFileSync(file, "utf8")).schemaVersion === 1,
    );
    clearIncrementalCache(store);
    assert(
      "clearIncrementalCache removes repo cache dir contents",
      !existsSync(file),
    );
  } finally {
    rmSync(root, { recursive: true, force: true });
