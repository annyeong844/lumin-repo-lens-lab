...
try {
  const build = spawnSync(
    NODE,
    [path.join(ROOT, "scripts/build-skill.mjs"), "--out", OUT],
    {
      cwd: ROOT,
      encoding: "utf8",
    },
  ...
  assert(
    "SP1. build-skill exits 0",
    build.status === 0,
    `${build.stdout}\n${build.stderr}`,
  );

  const publicScripts = existsSync(path.join(OUT, "scripts"))
    ? readdirSync(path.join(OUT, "scripts"))
        .filter((f) => f.endsWith(".mjs"))
        .sort()
    : [];

  assert(
    "SP2. generated skill exposes public script wrappers plus smoke test",
  ...
  assert(
    "SP3. generated skill includes shipping contract files",
    existsSync(path.join(OUT, "SKILL.md")) &&
      existsSync(
        path.join(path.dirname(OUT), "lumin-repo-lens-lab-codex/SKILL.md"),
      ) &&
      existsSync(
        path.join(path.dirname(OUT), "lumin-repo-lens-lab-write-gate/SKILL.md"),
      ) &&
      existsSync(
        path.join(path.dirname(OUT), "lumin-repo-lens-lab-canon/SKILL.md"),
      ) &&
  ...
  const generatedRefactorTemplate = readFileSync(
    path.join(OUT, "templates/refactor-plan-template.md"),
    "utf8",
  );
  const generatedRefactorPolicy = readFileSync(
    path.join(OUT, "references/refactor-plan-policy.md"),
    "utf8",
  );
  assert(
    "SP3c. generated refactor-plan policy and template split behavior from output shape",
    generatedRefactorPolicy.includes(
      ...
      generatedRefactorTemplate.includes(
        "Default length target: 20 to 35 lines",
      ) &&
      generatedRefactorTemplate.includes(
        "Only include machine-readable scope JSON when the user asks for it",
      ) &&
      !generatedRefactorTemplate.includes(
        "imply the user should have known better",
      ),
    `${generatedRefactorPolicy}\n${generatedRefactorTemplate}`,
  );

  const generatedShortChecklist = readFileSync(
    path.join(OUT, "templates/REVIEW_CHECKLIST_SHORT.md"),
    "utf8",
  );
  assert(
    "SP3d. generated skill includes short chat-facing checklist",
    generatedShortChecklist.includes("Gentle Structural Review") &&
      ...
      generatedShortChecklist.includes(
        "full checklist walk, formal report, or",
      ) &&
      ...
      /Do not omit\s+it after full-profile short answers/.test(
        generatedShortChecklist,
      ) &&
      generatedShortChecklist.includes(
        "Use the long `templates/REVIEW_CHECKLIST.md` only",
      ) &&
      generatedShortChecklist.includes("chat-facing structural reviews"),
    generatedShortChecklist,
  );

  const generatedLongChecklist = readFileSync(
  ...
  assert(
    "SP3b. generated SKILL.md stays slim enough for progressive disclosure",
    skillLines <= 165,
  ...
  const generatedReviewWorkflow = readFileSync(
    path.join(OUT, "references/structural-review-workflow.md"),
    "utf8",
  );
  const generatedFpStub = readFileSync(
    path.join(OUT, "references/false-positive-patterns.md"),
    "utf8",
  );
  assert(
    "SP3f. generated audit surface preserves full-baseline then quick-incremental cadence",
    generatedSkill.includes("references/structural-review-workflow.md") &&
      generatedSkill.includes(
      ...
      generatedReviewWorkflow.includes(
        "Short chat output is still allowed after a full run",
      ) &&
      generatedReviewWorkflow.includes(
        "Do not delegate this closeout to a string heuristic",
      ) &&
      generatedReviewWorkflow.includes(
        "Checklist gate and output density are separate",
      ) &&
      generatedReviewWorkflow.includes("The Core Contract makes") &&
      generatedReviewWorkflow.includes(
        "required review step, not merely a template",
      ) &&
      generatedCommandRouting.includes("choose the profile by cadence") &&
      generatedCommandRouting.includes(
        "--root . --output .audit --profile full",
      ) &&
      generatedCommandRouting.includes(
        "--root . --output .audit --profile quick",
      ) &&
      generatedCommandRouting.includes(
        "Checklist gate: the checklist is a required review step",
      ) &&
      generatedCommandRouting.includes(
        "Short output is not permission to skip it",
      ) &&
  ...
  assert(
    "SP3f3. generated package keeps the historical FP ledger out of normal context",
    existsSync(path.join(OUT, "references/false-positive-index.md")) &&
      existsSync(path.join(OUT, "references/false-positive-patterns.md")) &&
      !existsSync(
        path.join(OUT, "docs/maintainer/false-positive-patterns-ledger.md"),
      ) &&
  ...
  const packagedCanonText = collectMarkdownFiles(path.join(OUT, "canonical"))
    ...
    .join("\n");
  assert(
    "SP3f4. generated canonical spine omits maintainer patch-note metadata",
    !/Last updated|docs\/history|docs\/spec|rustlike3-clone|Generated:|v\d+(?:\.\d+)? change|promoted \d{4}-\d{2}-\d{2}|Previous sessions/.test(
      packagedCanonText,
    ) &&
      packagedCanonText.includes(
        "NO STRUCTURAL CLAIM WITHOUT MACHINE EVIDENCE",
      ) &&
      packagedCanonText.includes(
        "Drift records carry three classifying fields",
      ),
    packagedCanonText,
  );

  assert(
    "SP3f2. generated routing offers full expansion after short full-profile output",
    generatedCommandRouting.includes("one required feature-discovery tail") &&
      /full checklist walk, formal report, or due-diligence\s+handoff/.test(
        generatedCommandRouting,
      ) &&
      ...
      /Do not\s+omit it after full-profile short answers/.test(
        generatedCommandRouting,
      ) &&
      /Do not add this tail after\s+quick incremental checks/.test(
        generatedCommandRouting,
      ) &&
      generatedCommandRouting.includes(
        "Do not treat that offer as a required next step",
      ),
    generatedCommandRouting,
  );

  assert(
    "SP3g. generated routing explains first-run dependency setup without making users write install steps",
    generatedCommandRouting.includes(
      "First audit may install parser dependencies locally once",
    ) &&
      ...
      generatedCommandRouting.includes(
        "Do not tell first-time users that they must write intent JSON",
      ) &&
      generatedCommandRouting.includes(
        "npm ci --omit=dev --ignore-scripts --no-audit --fund=false",
      ) &&
      /Do not ask[\s\S]*install packages unless that\s+guard still fails/.test(
        generatedCommandRouting,
      ),
    generatedCommandRouting,
  );
  ...
  assert(
    ...
    existsSync(path.join(OUT, "_engine/_README.md")) &&
      ...
      existsSync(
        path.join(
          OUT,
          "_engine/producers/build-framework-resource-surfaces.mjs",
        ),
      ) &&
  ...
  const staleLibRefs = generatedDocs
    ...
    .flatMap((file) => {
      ...
      return text.includes("_lib/") ? [path.relative(OUT, file)] : [];
    });
  assert(
    "SP4b. generated package markdown normalizes maintainer _lib/ paths to _engine/lib/",
    staleLibRefs.length === 0 &&
      readFileSync(
        path.join(OUT, "canonical/any-contamination.md"),
        "utf8",
  ...
  assert(
    "SP5. generated skill excludes maintainer/lab surfaces",
  ...
  assert(
    "SP6. generated package bin points at public audit wrapper",
    pkg.name === "lumin-repo-lens-lab-skill" &&
      ...
      pkg.scripts?.["canon-draft"] ===
        "node scripts/audit-repo.mjs --canon-draft" &&
      pkg.scripts?.["check-canon"] ===
        "node scripts/audit-repo.mjs --check-canon" &&
  ...
  assert(
    "SP6a. generated package pins Node to parser-supported engines",
    pkg.engines?.node === "^20.19.0 || >=22.12.0",
    JSON.stringify(pkg.engines),
  );

  assert(
    "SP6b. generated package does not expose platform-specific oxc binding as a direct dependency",
    !Object.hasOwn(
      pkg.dependencies ?? {},
  ...
  assert(
    "SP6c. generated package-lock excludes maintainer-only dev dependencies",
    generatedLock.packages?.[""]?.engines?.node === "^20.19.0 || >=22.12.0" &&
      generatedLock.packages?.[""]?.bin?.["lumin-repo-lens-lab"] ===
        "scripts/audit-repo.mjs" &&
      !Object.hasOwn(generatedLock.packages?.[""]?.bin ?? {}, "lumin-audit") &&
      !Object.hasOwn(
        generatedLock.packages?.[""]?.bin ?? {},
        "grounded-audit",
      ) &&
      !lockPackages.includes("node_modules/eslint") &&
      !lockPackages.some((name) => name.startsWith("node_modules/@eslint/")),
    lockPackages
      .filter((name) => name.includes("eslint"))
      .slice(0, 20)
      .join("\n"),
  );

  const auditProducer = readFileSync(
    path.join(OUT, "_engine/producers/audit-repo.mjs"),
    "utf8",
  );
  assert(
  ...
  const checkWrapper = spawnSync(
    NODE,
    ["--check", path.join(OUT, "scripts/audit-repo.mjs")],
    {
      encoding: "utf8",
    },
  );
  const checkProducer = spawnSync(
    NODE,
    ["--check", path.join(OUT, "_engine/producers/audit-repo.mjs")],
    {
      encoding: "utf8",
    },
  );
  assert(
    "SP8. generated public wrapper and rewritten producer pass node --check",
    checkWrapper.status === 0 && checkProducer.status === 0,
    `${checkWrapper.stderr}\n${checkProducer.stderr}`,
  );

  const help = spawnSync(
    NODE,
    [path.join(OUT, "scripts/audit-repo.mjs"), "--help"],
    {
      cwd: OUT,
      encoding: "utf8",
    },
  ...
  assert(
    "SP9. generated audit wrapper reaches the engine help text",
    help.status === 0 &&
  ...
  const missingDeps = spawnSync(
    ...
    [
      ...
      OUT,
      "--output",
      path.join(TMP, "missing-deps-audit"),
    ],
    {
      cwd: OUT,
      encoding: "utf8",
  ...
  assert(
    "SP9a. generated audit wrapper uses luminRepoLens metadata for skill-safe dependency setup",
    missingDeps.status === 2 &&
      missingDeps.stderr.includes(
        "setup required: runtime dependencies are not ready",
      ) &&
  ...
  let selfDrift = null;
  try {
    selfDrift = JSON.parse(
      readFileSync(path.join(selfAuditOut, "canon-drift.json"), "utf8"),
    );
  } catch {
    /* assertion below reports stdout/stderr */
  }
  assert(
    "SP9b2. generated package check-canon skips absent self-audit fact canon instead of shipping it",
    selfCheck.status === 0 &&
      selfDrift?.summary?.sourcesSkipped === 4 &&
      selfDrift?.summary?.driftCount === 0 &&
      Object.values(selfDrift?.perSource ?? {}).every(
        (entry) => entry.status === "skipped-missing-canon",
      ),
  ...
  assert(
    "SP9f. generated package includes a runnable smoke test",
    smoke.status === 0 &&
  ...
  writeFileSync(
    goFile,
    ["package main", "", 'const Version = "1"', "", "func main() {}", ""].join(
      "\n",
    ),
  );

  const treeSitterModuleUrl = pathToFileURL(
    path.join(OUT, "_engine/lib/tree-sitter-langs.mjs"),
  ).href;
  ...
  assert(
    "SP9c. generated package resolves tree-sitter WASM dependencies from package root",
    tsCheck.status === 0 && tsResult?.available === true,
    `${tsCheck.stdout}\n${tsCheck.stderr}`,
  );

  assert(
    "SP9d. generated package extracts Go symbols through tree-sitter",
    tsCheck.status === 0 &&
      tsResult?.files === 1 &&
  ...
  const shippedHistoryRefs = listFiles(
    path.join(OUT, "_engine"),
    ".mjs",
  ).flatMap((file) => {
    const text = readFileSync(file, "utf8");
  ...
  assert(
    "SP9e. generated engine comments do not point at maintainer-only docs/history or docs/spec paths",
  ...
  assert(
    "SP10. plugin wrapper uses default skills/commands discovery and marketplace source",
  ...
  const commands = existsSync(path.join(ROOT, "commands"))
    ...
    : [];
  assert(
    "SP11. plugin exposes default command, welcome, stable capability commands, and refactor-plan coaching command",
    JSON.stringify(commands) ===
      JSON.stringify(COMMANDS.map((c) => `${c}.md`).sort()) &&
      COMMANDS.every((name) => {
        ...
        const expectedMode = name === "lumin-repo-lens-lab" ? "default" : name;
        return (
          text.includes(
            `\${CLAUDE_PLUGIN_ROOT}/skills/${skillTarget}/SKILL.md`,
          ) &&
          text.includes(
      ...
      readFileSync(
        path.join(ROOT, "references/command-routing.md"),
        "utf8",
      ).includes("do not ask which mode") &&
      readFileSync(
        path.join(ROOT, "references/command-routing.md"),
        "utf8",
      ).includes("Do not run a scan immediately") &&
      /short four-section\s+chat output/.test(
        readFileSync(path.join(ROOT, "references/command-routing.md"), "utf8"),
      ),
    JSON.stringify(commands),
  );

  const mainYaml = readFileSync(path.join(OUT, "agents/openai.yaml"), "utf8");
  const codexYaml = readFileSync(
    path.join(SKILLS_ROOT, "lumin-repo-lens-lab-codex", "agents/openai.yaml"),
    "utf8",
  );
  const codexSkill = readFileSync(
    path.join(SKILLS_ROOT, "lumin-repo-lens-lab-codex", "SKILL.md"),
    "utf8",
  );
  const writeGateYaml = readFileSync(
    path.join(SKILLS_ROOT, "lumin-repo-lens-lab-write-gate", "agents/openai.yaml"),
    "utf8",
  );
  const canonYaml = readFileSync(
    path.join(SKILLS_ROOT, "lumin-repo-lens-lab-canon", "agents/openai.yaml"),
    "utf8",
  );
  const generatedReadme = readFileSync(path.join(OUT, "README.md"), "utf8");
  assert(
    "SP12. build-skill keeps Codex discovery thin by adding metadata to shared generated skill surfaces",
  ...
  assert(
    "SP12b. Codex metadata names the shared surfaces and thin Codex wrapper without duplicating the engine",
    mainYaml.includes('display_name: "Lumin Repo Lens"') &&
      ...
      codexSkill.includes(
        "Set `<audit-repo>` to this path resolved relative",
      ) &&
      ...
      writeGateYaml.includes(
        'default_prompt: "Use $lumin-repo-lens-lab-write-gate',
      ) &&
  ...
  const generatedEnglishPublicDocs = [
    ...
    [
      "../lumin-repo-lens-lab-write-gate/SKILL.md",
      readFileSync(
        path.join(SKILLS_ROOT, "lumin-repo-lens-lab-write-gate/SKILL.md"),
        "utf8",
      ),
    ],
    [
      "../lumin-repo-lens-lab-canon/SKILL.md",
      readFileSync(
        path.join(SKILLS_ROOT, "lumin-repo-lens-lab-canon/SKILL.md"),
        "utf8",
      ),
    ],
    ...
    [
      ...
      readFileSync(path.join(OUT, "references/glossary.md"), "utf8"),
    ],
  ...
  for (const [name, text] of generatedEnglishPublicDocs) {
    for (const token of KOREAN_EPISTEMIC_TOKENS) {
      if (text.includes(token))
        generatedKoreanEpistemicOffenders.push(`${name}: ${token}`);
    }
  }
  assert(
    "SP12b2. generated public English docs use the English `unknown` evidence label",
  ...
  assert(
    "SP12c0. shipped README carries Claude Code marketplace install instructions before Codex link install",
    generatedReadme.includes("## First useful run") &&
      ...
      generatedReadme.indexOf("## First useful run") <
        generatedReadme.indexOf("### Codex-native install"),
    generatedReadme,
  );

  assert(
    "SP12c1. shipped README states conservative clone and shape evidence boundaries",
    generatedReadme.includes("Function-clone cues are review cues") &&
      ...
      generatedReadme.includes(
        "Start from `audit-summary.latest.md`, `manifest.json`, and `checklist-facts.json`",
      ) &&
      generatedReadme.includes(
        "open raw JSON artifacts only for the claim being cited",
      ),
    generatedReadme,
  );

  assert(
    "SP12c. shipped README carries Codex link-install instructions",
    generatedReadme.includes("### Codex-native install") &&
      generatedReadme.includes("$lumin-repo-lens-lab-codex") &&
      generatedReadme.includes(
        "ln -sfn ~/.codex/lumin-repo-lens-lab/skills/lumin-repo-lens-lab-codex",
      ) &&
      generatedReadme.includes(
        'cmd /c mklink /J "%USERPROFILE%\\.codex\\skills\\lumin-repo-lens-lab-codex"',
      ) &&
      generatedReadme.includes(
        "ln -sfn ~/.codex/lumin-repo-lens-lab/skills/lumin-repo-lens-lab",
      ) &&
      generatedReadme.includes(
        'cmd /c mklink /J "%USERPROFILE%\\.codex\\skills\\lumin-repo-lens-lab"',
      ) &&
  ...
  assert(
    "SP12d. shipped README warns that default .audit artifacts may be commit-sensitive",
