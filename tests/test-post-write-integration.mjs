...

{
  ...
  try {
    ...
    const intent = {
      ...
      plannedTypeEscapes: [
        { escapeKind: 'as-any', locationHint: 'src/adapters/', codeShape: 'r as any', reason: 'upstream SDK lacks type exports' },
      ],
    };
    ...
    // Step 1: pre-write.
    const pre = auditRepo(['--root', fx, '--output', out, '--profile', 'quick', '--pre-write', '--intent', intentPath]);
    assert('F1. pre-write step exit 0', pre.status === 0, `stderr=${pre.stderr.slice(0, 300)}`);
    const preManifest = JSON.parse(readFileSync(path.join(out, 'manifest.json'), 'utf8'));
    assert('F1b. manifest.preWrite.ran === true', preManifest.preWrite?.ran === true);

    // Step 2: edit — introduce escapes + parse-error file.
    write(fx, 'src/adapters/a.ts',
      `export const adaptA = (r) => {\n` +
      `  return r as any;\n` +
      `};\n`
    );
    write(fx, 'src/adapters/b.ts',
      `export const adaptB = (r) => {\n` +
      `  return r as any;\n` +
      `};\n`
    );
    write(fx, 'src/unplanned.ts',
      `export const unused = (u) => u as any;\n`
    );
    ...
    const post = auditRepo([
      ...
      '--post-write', '--pre-write-advisory', advisory,
    ]);
    assert('F1c. post-write step exit 0', post.status === 0, `stderr=${post.stderr.slice(0, 300)}`);
    assert('F1d. stdout contains "## post-write delta"',
    ...
    assert('F1f. manifest.postWrite.deltaPath exists on disk',
      typeof m.postWrite?.deltaPath === 'string' && existsSync(m.postWrite.deltaPath));
    ...
    assert('F1g. manifest.postWrite.silentNew === 2', m.postWrite?.silentNew === 2);
    assert('F1h. manifest.postWrite.silentNew matches delta.summary.silentNew',
      m.postWrite?.silentNew === delta.summary.silentNew);
    assert('F1i. manifest.postWrite.requiredAcknowledgementCount === 2',
      m.postWrite?.requiredAcknowledgementCount === 2);
    assert('F1j. manifest.postWrite.afterComplete === false (bad.ts parse error)',
      m.postWrite?.afterComplete === false);
    assert('F1k. manifest.postWrite.baselineStatus === "available"',
    ...
    assert('F1l. delta.preWriteInvocationId === advisory.invocationId',
    ...
    assert('F1o. exactly one "planned" entry on src/adapters/a.ts or b.ts',
      plannedEntries.length === 1 &&
      (plannedEntries[0].file === 'src/adapters/a.ts' || plannedEntries[0].file === 'src/adapters/b.ts'));

    // The OTHER adapter file is silent-new with ambiguous-planned-match diagnostic.
    const adapterEntries = delta.entries.filter((e) => e.file?.startsWith('src/adapters/'));
    const adapterAmbiguous = adapterEntries.filter((e) =>
      e.label === 'silent-new' && (e.diagnostics ?? []).includes('ambiguous-planned-match'));
    assert('F1p. exactly one silent-new adapter has ambiguous-planned-match diagnostic',
      adapterAmbiguous.length === 1);
    assert('F1q. planned and ambiguous remainder are on DIFFERENT adapter files',
      plannedEntries[0].file !== adapterAmbiguous[0].file);

    // unplanned.ts classifies as plain silent-new (no ambiguity).
    const unplannedEntries = delta.entries.filter((e) => e.file === 'src/unplanned.ts');
    assert('F1r. src/unplanned.ts → silent-new with empty diagnostics',
    ...
    assert('F1s. inventoryCompleteness.afterComplete === false',
      delta.inventoryCompleteness?.afterComplete === false);
    const badParseEntry = (delta.inventoryCompleteness?.filesWithParseErrors ?? [])
      .find((e) => e.side === 'after' && /bad\.ts$/.test(e.file));
    assert('F1t. filesWithParseErrors includes src/bad.ts on "after" side',
    ...
    assert('F1u. requiredAcknowledgements(delta).length === 2',
      req.length === 2);
    assert('F1v. every entry in requiredAcknowledgements has label === silent-new',
      req.every((e) => e.label === 'silent-new'));

    // Markdown stdout pinning.
    assert('F1w. stdout includes "silent-new — REQUIRE acknowledgment: 2 entries"',
    ...
    assert('F1x. stdout includes "Inventory completeness:" section with parse-error file listed',
      post.stdout.includes('Inventory completeness:') && /src\/bad\.ts/.test(post.stdout));

    // JSON round-trip.
    const roundTripped = JSON.parse(JSON.stringify(delta));
    assert('F1y. JSON round-trip produces structurally equal object',
...

{
  ...
  try {
    ...
    const pre = auditRepo(['--root', fx, '--output', out, '--profile', 'quick',
      '--pre-write', '--intent', intentPath, '--no-fresh-audit']);
    assert('F2. pre-write exit 0', pre.status === 0, `stderr=${pre.stderr.slice(0, 300)}`);

    const advisory = path.join(out, 'pre-write-advisory.latest.json');
    const adv = JSON.parse(readFileSync(advisory, 'utf8'));
    assert('F2b. advisory has no preWrite.anyInventoryPath (pre-condition)',
      !adv.preWrite || !('anyInventoryPath' in adv.preWrite));

    // Edit — add one as any.
    write(fx, 'src/a.ts', `export const foo = (x) => x as any;\n`);

    const post = auditRepo(['--root', fx, '--output', out, '--profile', 'quick',
      '--post-write', '--pre-write-advisory', advisory]);
    assert('F2c. post-write exit 0', post.status === 0, `stderr=${post.stderr.slice(0, 300)}`);

    const m = JSON.parse(readFileSync(path.join(out, 'manifest.json'), 'utf8'));
    assert('F2d. manifest.postWrite.ran === true', m.postWrite?.ran === true);
    assert('F2e. manifest.postWrite.baselineStatus === "missing"',
      m.postWrite?.baselineStatus === 'missing');
    assert('F2f. manifest.postWrite.silentNew === 0',
      m.postWrite?.silentNew === 0);
    assert('F2g. manifest.postWrite.requiredAcknowledgementCount === 0',
      m.postWrite?.requiredAcknowledgementCount === 0);

    const delta = JSON.parse(readFileSync(m.postWrite.deltaPath, 'utf8'));
    assert('F2h. delta.baseline.status === "missing"',
      delta.baseline.status === 'missing');
    assert('F2i. delta.baseline.reason mentions anyInventoryPath',
      /anyInventoryPath/.test(delta.baseline.reason ?? ''));

    const observedEntries = delta.entries.filter((e) => e.label === 'observed-unbaselined');
    assert('F2j. exactly one observed-unbaselined entry on src/a.ts',
      observedEntries.length === 1 && observedEntries[0].file === 'src/a.ts');
    assert('F2k. summary.silentNew === 0', delta.summary.silentNew === 0);

    assert('F2l. requiredAcknowledgements returns empty array',
      requiredAcknowledgements(delta).length === 0);

    assert('F2m. stdout summary mentions before-inventory missing',
      post.stdout.includes('No silent-new acknowledgements required') &&
      post.stdout.includes('before-inventory missing'));

    // JSON round-trip.
    const roundTripped = JSON.parse(JSON.stringify(delta));
    assert('F2n. JSON round-trip structurally equal',
      JSON.stringify(roundTripped) === JSON.stringify(delta));
  } finally {
    rmSync(fx, { recursive: true, force: true });
    rmSync(out, { recursive: true, force: true });
