...
use lumin_rust_common::{
    find_repo_root_with_fallback, parse_enum, take_path, take_string, usage_error, CliAction,
};
...
fn print_usage() {
    eprintln!(
        "Usage: lumin-rust-cargo-oracle --root <path> [--output <path>] [--cargo-bin <path>] [--features <csv>] [--package <name>] [--repo-root <path>] [--cargo-check-mode metadata-only|cargo-check|targeted-cargo-check] [--cargo-target-dir-mode isolated-temp|reusable-temp] [--target-path <path>...]"
    );
}
