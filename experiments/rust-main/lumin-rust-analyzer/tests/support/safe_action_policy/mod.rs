...
pub fn assert_safe_action_missing_schema_calibrated_artifact(artifact: &Value) -> Result<()> {
    summary::assert_safe_action_summary(artifact);
    action_policy::assert_safe_action_policy(artifact)?;
    finding::assert_safe_action_finding(artifact)?;
    bridge_calibration::assert_safe_action_missing_schema_calibrated_bridge(artifact)?;
    Ok(())
}
