...
#[test]
fn unified_cli_invalid_timeout_exits_2_before_writing_artifact() -> Result<()> {
    let temp = TempDir::new()?;
    let output_path = temp.path().join("rust-analyzer.json");
    ...
    assert!(output.stdout.is_empty());
    assert!(!output_path.exists());
    assert!(String::from_utf8_lossy(&output.stderr).contains("invalid --timeout-ms value: soon"));
    Ok(())
}

#[test]
fn unified_cli_invalid_targeted_package_cap_exits_2_before_writing_artifact() -> Result<()> {
    let temp = TempDir::new()?;
    let output_path = temp.path().join("rust-analyzer.json");
    ...
    assert!(!output_path.exists());
    assert!(String::from_utf8_lossy(&output.stderr)
        .contains("--targeted-package-cap must be greater than zero"));
    Ok(())
}
