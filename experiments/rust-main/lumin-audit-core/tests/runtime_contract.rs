...
fn cli_runtime_contract_reports_js_bridge_capabilities() -> Result<()> {
    ...
    assert_eq!(
        contract["features"]["jsTsPreWriteExactWorktreeByteCache"],
        true
    );
    assert_eq!(
        contract["features"]["jsTsPreWriteCanonicalSourceContainment"],
        true
    );
    assert_eq!(contract["features"]["jsTsPreWriteSingleFlight"], true);
    assert_eq!(contract["features"]["checklistFactsIncrementalCache"], true);
    ...
    assert_eq!(
        contract["features"]["jsTsPreWriteCurrentEvidenceOnly"],
        true
    );
    ...
    assert_eq!(
        contract["features"]["workspaceDependencyOwnerManifests"],
        true
    );
