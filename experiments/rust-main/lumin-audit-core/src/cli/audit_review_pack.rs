...
pub(super) fn run_audit_review_pack_render(args: Vec<String>) -> Result<()> {
    let mut input = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            _ => bail!("audit-review-pack-render: unknown argument '{arg}'\n{USAGE}"),
        }
    }

    let input = input.context("audit-review-pack-render: missing --input <path|->")?;
    let json = read_json_input(&input, "audit-review-pack-render")?;
    let request = serde_json::from_value::<AuditReviewPackRenderRequest>(json)
        .context("audit-review-pack-render: invalid request shape")?;
    let output_path = PathBuf::from(&request.output_path);
    let (markdown, result) = render_audit_review_pack_request(&request)?;
