...
use lumin_audit_core::symbol_graph::{build_symbol_graph_artifact, SymbolGraphRequest};

pub(super) fn run_symbol_graph_artifact(args: Vec<String>) -> Result<()> {
    let mut input = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            _ => bail!("symbol-graph-artifact: unknown argument '{arg}'\n{USAGE}"),
        }
    }

    let input = input.context("symbol-graph-artifact: missing --input <path|->")?;
    let json = read_json_input(&input, "symbol-graph-artifact")?;
    let request = serde_json::from_value::<SymbolGraphRequest>(json)
        .context("symbol-graph-artifact: invalid request shape")?;
