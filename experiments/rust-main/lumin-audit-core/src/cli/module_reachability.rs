...
use lumin_audit_core::module_reachability::{
    build_module_reachability_artifact, ModuleReachabilityRequest,
};
...
pub(super) fn run_module_reachability_artifact(args: Vec<String>) -> Result<()> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            _ => bail!("module-reachability-artifact: unknown argument '{arg}'\n{USAGE}"),
        }
    }

    let input = input.context("module-reachability-artifact: missing --input <path|->")?;
    let json = read_json_input(&input, "module-reachability-artifact")?;
    let request = serde_json::from_value::<ModuleReachabilityRequest>(json)
        .context("module-reachability-artifact: invalid request shape")?;
