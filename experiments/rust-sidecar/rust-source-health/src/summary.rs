...
pub(crate) fn summarize(files: &BTreeMap<String, FileHealth>) -> Summary {
    ...
    for file in files.values() {
        ...
        summary.function_signatures += file.ast.function_signatures.len();
        summary.function_body_fingerprints += file.ast.function_body_fingerprints.len();
        summary.inline_patterns += file.ast.inline_patterns.len();
