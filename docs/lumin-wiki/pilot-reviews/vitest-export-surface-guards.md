...
> - `tests/test-function-clone-export-surface.mjs`
...
This review decides whether four tiny export-surface guard suites can move
...
normalization, function-clone artifact generation, or classification policy
decisions.
...
| `tests/test-function-clone-export-surface.mjs`    | `node tests/test-function-clone-export-surface.mjs`    | `npm run test:vitest:function-clone-export-surface`    | `_lib/function-clone-artifact.mjs` |
...
- `_lib/function-clone-artifact.mjs` exports `buildFunctionCloneArtifact`;
- `_lib/function-clone-artifact.mjs` does not expose
...
- leaking function-clone schema or normalized version constants must fail;
- hiding the public function-clone artifact builder must fail;
...
- The mirror must not absorb resolver, generated, deadness/ranking,
  pre/post-write workflow, function-clone grouping, or classification behavior
  suites.
...
3. `tests/function-clone-export-surface.test.mjs`,
4. `tests/classify-policies-export-surface.test.mjs`,
