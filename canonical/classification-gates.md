...
> **Owner:** this file.
> **Status:** spine v1.3 (2026-04-21: §12 Naming classification added for P3-4; type §1-§9 + helper §10 + topology §11 unchanged).
...
Types defined in files classified as test-like (per `_lib/test-paths.mjs::isTestLikePath`) are NOT excluded from observation — they appear in the registry with a `tags: ["test-only"]` annotation. The finalize step (promotion to canon) typically drops them; the raw observation retains them so reviewers can decide.
...
Parallel to §2 (type group) + §4 (single type identity), adapted for exported helpers (top-level functions / arrows / const-functions). Consumed by the P3-2 canon draft generator (`canonical-draft/helper-registry.md`) per `docs/history/phases/p3/p3-2.md` v2.
...
A **group** is the set of helper identities sharing an `exportedName`. Fan-in per identity is **consumer-file-count**: `|distinctConsumerFiles|` — the number of distinct files that import the helper. A single consumer file calling the helper 10 times contributes 1 to fan-in, not 10. This differs from raw call-site count and is pinned by `docs/history/phases/p3/p3-2.md` v2 PF-4.
...
### 10.3 Label set (drift-locked — mirrored in `_lib/canon-draft.mjs`)
...
Any classifier output outside this set is a defect. `tests/test-classification-gates.mjs` pins the mirror constant in `_lib/canon-draft.mjs` byte-equal to this enumeration.
...
Parallel to §2/§4 (types) and §10 (helpers), adapted for topology. Unlike types/helpers, topology operates at the SUBMODULE level. Consumed by the P3-3 canon draft generator (`canonical-draft/topology.md`) per `docs/history/phases/p3/p3-3.md` v3.
...
### 11.7 Fan-in evidence source (honest-degradation per docs/history/phases/p3/p3-3.md v3 PF-6)
...
Parallel to §2/§4 (types), §10 (helpers), §11 (topology), adapted for naming conventions. Consumed by the P3-4 canon draft generator (`canonical-draft/naming.md`) per `docs/history/phases/p3/p3-4.md` v2.
...
|-------------|------------------------|---------|
| File cohort | `<ownerFile>` | `_lib/legacy_helper.mjs` |
| Symbol cohort | `<ownerFile>::<exportedName>` | `_lib/oldStyle.mjs::OLD_API` |
