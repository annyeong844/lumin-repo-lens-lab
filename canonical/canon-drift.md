...
> **Status:** canonical, v1.2 (landed 2026-04-21 via P5-0; clarified 2026-04-22 to include §5.c §2 cross-edge parser contract that was implicit in §3.1 categories).
...
> **Consumed by:** P5-1+ (`_lib/check-canon-*.mjs` + `check-canon.mjs`).
> **Last updated:** 2026-05-05.
>
> **v1.1 change (2026-04-22):** §5.c §2 Cross-submodule edges table contract added. Non-breaking clarification — the `cross-edge-added` / `cross-edge-removed` categories in §3.1 were always canonical but the parser contract for the source table was omitted in v1, making `cross-edge-*` unreachable for a pure canon consumer. v1.1 closes that gap without changing any existing contract.
>
> **v1.2 change (2026-05-05):** §5.a type ownership current renderer contract includes the display-only `Fan-in space` column. The drift parser keeps a legacy read-compatibility path for the prior 6-column table, but new drafts emit the 7-column shape.

---

## 1. Purpose

Canon drift is step 4 of the stickiness loop: after P3 emits fresh canon drafts and a human/LLM promotes selected drafts to `canonical/*.md`, subsequent audits must answer "does the promoted canon still match the code?". The drift detector **reports** that question; it NEVER writes to `canonical/`. Re-promotion stays human-driven. See `docs/history/phases/p5/session.md` v2 §2 ("Optimization target") for the loop closing rationale.
