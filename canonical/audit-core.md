...
| `experiments/rust-main/lumin-audit-core/src/block_clones/suffix_array.rs`      | Token-value compression with file sentinels plus deterministic suffix-array and LCP construction over already-tokenized inputs | clone thresholds, span materialization, noise policy, cap policy, or JSON projection |
...
| `experiments/rust-main/lumin-audit-core/src/block_clones/tests.rs`             | Focused artifact behavior fixtures for review groups, same-file muting, and schema rejection | production behavior or alternate policy implementations |
...
| `experiments/rust-main/lumin-audit-core/src/function_clones/near.rs`           | Checked near-function candidate policy constants and metadata, grouped-identity exclusion, token/name similarity, pair scoring, candidate ordering, and projection cap | new threshold calibration, source parsing, exact/structure/signature grouping, or final artifact assembly |
...
| `experiments/rust-main/lumin-audit-core/src/function_clones/tests.rs`          | Focused artifact behavior fixtures for grouping, near-candidate scoring, incomplete evidence, and schema rejection | production behavior or alternate policy implementations |
...
| `experiments/rust-main/lumin-audit-core/src/js_ts_extract/protocol.rs`         | Project-owned JS/TS extraction request, response, file-result, definition/use, opacity, CJS surface, re-export, class-method, and type-escape JSON shapes                                                                                                                               | parser traversal, extraction policy, source IO, relative resolution, or graph projection |
...
by `_lib/audit-core.mjs` and the Rust runtime-contract command. The strict
...
| `experiments/rust-main/lumin-audit-core/src/checklist_facts.rs` | `checklist-facts.json` facade: request validation, stable section ordering, metadata projection, and final artifact assembly from typed request facts and already-produced artifacts | request JSON shape, section-specific gate math, topology/shape analysis, citation wording, deferred-item vocabulary, source walking, OXC parsing, or Markdown rendering |
| `experiments/rust-main/lumin-audit-core/src/checklist_facts/protocol.rs` | Project-owned checklist-facts request, optional input-artifact, function-size, and silent-catch JSON shapes plus checked default vocabulary | gate computation, artifact reads, source extraction, or rendering |
...
`_lib/audit-core.mjs` is the single JS owner of runtime-contract validation and
