...
| `experiments/rust-main/lumin-audit-core/src/js_ts_pre_write/cache.rs`         | Strict per-file OXC fact cache for the shared JS/TS pre-write/post-write evidence pass: repository and source-set identity, Git clean-blob identity keyed by the actual root-relative source path, exact Git blob bytes for cache-miss parsing, content-SHA identity for dirty/untracked or Git-read-fallback files, cache load/store validation, and incremental observations | source discovery, parser traversal, final evidence reuse, absence claims, cue/ranking policy, artifact-path aliases as source identity, worktree reads under a Git-blob identity, or stat/mtime-only identity                                                         |
...
`_lib/extract-ts.mjs` as a fallback. Rust-reported per-file parse errors remain
artifact-visible parse failures; they are not helper-contract failures.

`_lib/symbol-graph-discovery.mjs` owns source snapshot construction,
...
`_lib/symbol-graph-resolver.mjs` owns the remaining JS-side resolution boundary:
...
`_lib/source-use-assembly-request.mjs` is the JS transport boundary for this
...
`_lib/source-use-record-builder.mjs` owns conversion from already-discovered
...
`_lib/symbol-graph-source-use-planner.mjs` owns deterministic partitioning of
...
`_lib/symbol-graph-finalizer.mjs` owns the transport-only finalizer boundary:
...
`_lib/symbol-graph-request.mjs` owns strict v2 request lowering: extraction
...
`_lib/symbol-graph-artifact.mjs` builder must not be restored; compatibility
...
| `experiments/rust-main/lumin-audit-core/src/cli/manifest.rs` | CLI runners for manifest metadata, root/evidence assembly and refresh, lifecycle patch application, final writes, companion policy/render sequencing, and producer-performance/manifest closeout. For base-skipped lifecycle runs it validates the plan skip reason, emits a standard `scan-gap`, suppresses stale base reads, and derives current lifecycle artifacts only from manifest-bound paths/IDs before performance and closeout projection. | producer orchestration, lifecycle child sequencing before closeout, blind-zone owner migration before parity, artifact-read observation outside manifest-evidence summary/refresh/root/finalize inputs |
...
repository. `_lib/js-ts-rust-evidence.mjs` owns command-specific request and
response path translation; `_lib/audit-core.mjs` owns current-contract
