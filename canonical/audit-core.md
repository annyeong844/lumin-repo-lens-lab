...
| Lifecycle child process execution | `preWrite` Rust engine selection: `pre_write_routing.rs`; `preWrite` Rust engine execution: `pre_write_lifecycle.rs`; `preWrite` JS/TS child execution: `pre_write_lifecycle.rs` spawning `pre-write.mjs`; `canonDraft`: `canon_draft_lifecycle.rs`; `checkCanon`: `check_canon_lifecycle.rs`; `postWrite`: `post_write_lifecycle.rs`; combined lifecycle sequencing: `cli/lifecycle.rs` through `execute-audit-lifecycle` | Rust audit-core owns the base audit profile executor, pre-write routing, Rust pre-write analyzer child execution, JS/TS pre-write child execution without owning JS/TS producer semantics, the canon-draft lifecycle child spawner, the check-canon lifecycle child spawner, the post-write child spawner, lifecycle sequencing across those blocks, and final companion/closeout coordination. JS still owns request construction from public CLI flags/files. | Migrate JS/TS pre-write producer semantics only after a dedicated parity plan. |
...
| `experiments/rust-main/lumin-audit-core/src/source_use_assembly/protocol.rs` | Project-owned source-use request/response JSON shapes and schema constants | compact transport decoding, resolution, graph projection, or filesystem access |
...
| `experiments/rust-main/lumin-audit-core/src/source_use_assembly/namespace.rs` | Namespace re-export map construction and deterministic re-export-chain resolution from caller-supplied facts | import extraction, source resolution, graph mutation, or diagnostic projection |
...
| `experiments/rust-main/lumin-audit-core/src/symbol_graph/sfc.rs` | Typed SFC style/template/global/generated-manifest/framework-convention projection and source-use target linkage | SFC discovery, source walking, source-use resolution, fan-in eligibility, or safe-fix policy |
...
The runtime bridge contract is `audit-core-js-runtime-bridge.v36` and requires
`symbolGraphStrictRequestV2`; a v35 helper or a helper missing that feature is
rejected before execution. Every advertised platform binary and the packaged
source fallback must be regenerated from the same contract.
