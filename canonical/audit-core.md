...
summary/refresh artifact reads,
...
| Final `manifest.json` assembly | Initial root/evidence projection: `manifest_root.rs` through `manifest-root-with-evidence`; final audit-run closeout/write: `cli/manifest.rs` through `finalize-audit-run`; remaining lifecycle/companion rendering sequence: `audit-repo.mjs` | Rust audit-core now owns the first manifest object from JS-supplied run metadata, typed `commandsRun`/`skipped`, Rust-owned evidence reads with artifact-read telemetry, final `producer-performance.json` construction, closeout patch application, and final `manifest.json` file emission. JS still sequences lifecycle blocks, renders companion Markdown, and passes typed observations/paths into audit-core. | Migrate the remaining sequencing only after those lifecycle/companion inputs have typed Rust owners or an explicit Rust orchestrator owns the full final manifest projection. |
