...
| Producer runtime observation values (`commandsRun`, `skipped`) | Base audit profile: `orchestration_executor.rs` through the `execute-base-plan` wrapper; lifecycle helpers: `audit-repo.mjs` | Rust now owns base-step skip decisions, statuses, stderr snippets, wall-clock measurements, and orchestrator memory snapshots. JS still builds the executor request, invokes the Rust CLI, owns lifecycle helper runtime observations, and assembles the final manifest. | Migrate lifecycle helpers only with their own raw-block parity plan; keep JS/TS producer internals outside audit-core. |
| Producer performance measurement inputs | `audit-repo.mjs` plus `_lib/artifacts.mjs`, except base execution observations and artifact-size measurement now owned by `lumin-audit-core` | Rust owns base child status/wall/stderr/memory observations and artifact-size measurement for the already-produced artifact names supplied by the JS runner. Artifact read metrics, phase timing reads, lifecycle observations, and final ledger assembly still stay with the JS runner in this slice. | Move phase timing and artifact-read measurement only with explicit Rust owners; migrate lifecycle observations separately. |
...
- Audit-core may own an orchestration plan separately from execution. The plan
  is declarative profile/lifecycle evidence; `orchestration_executor.rs` is the
  only current Rust owner allowed to evaluate base-step preconditions or spawn
  base producer children.
