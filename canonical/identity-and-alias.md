...
Identity resolution uses `importedName` (source-side name), not `localName`. `localName` only matters inside the consumer file for in-file reference counting (which is a different concern, handled by `_lib/classify-facts.mjs::countFileReferencesAst`).
...
Depth limit 8 hops (matches `_lib/alias-map.mjs::extractStringTarget` depth).
...
The downgrade is **per-identity**, not repo-wide. Only identities whose path shape matches an unresolved specifier prefix (same logic as `specifierCouldMatchFile` in `_lib/finding-provenance.mjs`) are downgraded. Identities in unaffected scopes keep their confidence.
