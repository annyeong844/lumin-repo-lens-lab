...
{
  "kind": "watchpoint",
  "target": "_lib/resolver-core.mjs::makeResolver",
...
Drift is defined as: a canonical declaration exists (promoted to `canonical/*.md`) that disagrees with a current fact of the same kind. Formal drift semantics live in `canonical/canon-drift.md` (landed 2026-04-21 via P5-0). Pre-write continues to emit one-line drift warnings as described in `pre-write-gate.md` §8; the formal drift detector (`check-canon.mjs`) is built out across P5-1..P5-4.
