...
- A scan range is the explicit set of files / languages / directories examined. "Checked all `*.mjs` except `node_modules/` and `_lib/vocab.mjs`, 0 hits for pattern `TAINT.*_MATCH`" is a valid scan range. "I looked around and didn't see it" is not.
...
4. **Boundary collapse.** Layer rules ignored; feature envy; `_lib/` reaching into app code.
