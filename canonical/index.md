...
Methodology borrowed from `rustlike3-clone`: invariants here, phases in `p{N}/session.md`. No phase proceeds without reading the canonical files it depends on.
...
- `canonical/lifecycle.md` — post-write regression + refresh cadence. Promoted when P2 starts.
- ~~`canonical/canon-drift.md`~~ — **promoted 2026-04-21 via P5-0.** Formal drift categories + parser contract + `canon-drift.json` shape now live in `canonical/canon-drift.md`.

Skeletons are intentional placeholders: listing these reserves the filename + topic so later promotion doesn't re-negotiate scope.

## 5. External reference material (not canonical; used as design input)

- `ouroboros-ts/canonical/*` — another project's canonical set. Format reference for `type-ownership.md`, `helper-registry.md`.
- `suyeon-daemon-followup/docs/current/reference/*` — function-role-intent inventories. Format reference for P3 canon draft outputs.
- `rustlike3-clone/canonical/*` + `p{N}/session.md` — methodology reference for this spine.

These are **never imported as truth**; they are patterns to learn from.

## 6. How to change the spine
